...
hex2bin (const char *string, void *buffer, size_t length)
{
  ...
  for (i=0; i < length; )
    {
      if (!((*(s) >= '0' && *(s) <= '9') || (*(s) >= 'A' && *(s) <= 'F') || (*(s) >= 'a' && *(s) <= 'f')) || !((*(s+1) >= '0' && *(s+1) <= '9') || (*(s+1) >= 'A' && *(s+1) <= 'F') || (*(s+1) >= 'a' && *(s+1) <= 'f')))
        return -1;
      ((unsigned char*)buffer)[i++] = (((*(s) <= '9'? (*(s)- '0'): *(s) <= 'F'? (*(s)-'A'+10):(*(s)-'a'+10)) * 16) + (*((s)+1) <= '9'? (*((s)+1)- '0'): *((s)+1) <= 'F'? (*((s)+1)-'A'+10):(*((s)+1)-'a'+10)));
      s += 2;
    }
  if (*s && (!(0 ? isascii(*s) : (unsigned)(*s) < 128) || !__isspace(*s)) )
    return -1;
  if (i != length)
    return -1;
  if (*s)
  ...
  for (s = buffer, p = stringbuf; length; length--, s++)
    {
      if (with_colon && s != buffer)
        *p++ = ':';
      *p++ = (((*s>>4)&15) < 10 ? (((*s>>4)&15) + '0') : ((((*s>>4)&15) - 10) + 'A'));
      *p++ = ((*s&15) < 10 ? ((*s&15) + '0') : (((*s&15) - 10) + 'A'));
    }
  *p = 0;
...
hex2str (const char *hexstring, char *buffer, size_t bufsize, size_t *buflen)
{
  ...
  for (s=hexstring, count=0; ((*(s) >= '0' && *(s) <= '9') || (*(s) >= 'A' && *(s) <= 'F') || (*(s) >= 'a' && *(s) <= 'f')) && ((*(s+1) >= '0' && *(s+1) <= '9') || (*(s+1) >= 'A' && *(s+1) <= 'F') || (*(s+1) >= 'a' && *(s+1) <= 'f')); s += 2, count++)
    ;
  if (*s && (!(0 ? isascii(*s) : (unsigned)(*s) < 128) || !__isspace(*s)) )
    return ((void*)0);
  ...
  if (buffer)
    {
      ...
      for (s=hexstring, idx=0; ((*(s) >= '0' && *(s) <= '9') || (*(s) >= 'A' && *(s) <= 'F') || (*(s) >= 'a' && *(s) <= 'f')) && ((*(s+1) >= '0' && *(s+1) <= '9') || (*(s+1) >= 'A' && *(s+1) <= 'F') || (*(s+1) >= 'a' && *(s+1) <= 'f')); s += 2)
        ((unsigned char*)buffer)[idx++] = (((*(s) <= '9'? (*(s)- '0'): *(s) <= 'F'? (*(s)-'A'+10):(*(s)-'a'+10)) * 16) + (*((s)+1) <= '9'? (*((s)+1)- '0'): *((s)+1) <= 'F'? (*((s)+1)-'A'+10):(*((s)+1)-'a'+10)));
      if (need_nul)
        buffer[idx] = 0;
    }
...
hex2str_alloc (const char *hexstring, size_t *r_count)
{
  ...
  tail = hex2str (hexstring, ((void*)0), 0, &nbytes);
  if (!tail)
    {
      if (r_count)
        *r_count = 0;
  ...
  if (!result)
    return ((void*)0);
  if (!hex2str (hexstring, result, nbytes+1, ((void*)0)))
    g10_log_bug0( "convert.c" , 244, __FUNCTION__ );
  return result;
}
