...
initialize( ARGPARSE_ARGS *arg, const char *filename, unsigned *lineno )
{
    if( !(arg->flags & (1<<15)) ) {
 arg->internal.idx = 0;
 arg->internal.last = ((void*)0);
 arg->internal.inarg = 0;
 arg->internal.stopped = 0;
 arg->internal.aliases = ((void*)0);
 arg->internal.cur_alias = ((void*)0);
 arg->internal.iio_list = ((void*)0);
 arg->err = 0;
 arg->flags |= 1<<15;
 if( *arg->argc < 0 )
     g10_log_bug("Invalid argument for ArgParse\n");
    }
 ...
 if( filename ) {
     if( arg->r_opt == -6 )
       g10_log_error("%s:%u: argument not expected\n", filename, *lineno );
     else if( arg->r_opt == -5 )
       g10_log_error("%s:%u: read error\n", filename, *lineno );
     else if( arg->r_opt == -4 )
       g10_log_error("%s:%u: keyword too long\n", filename, *lineno );
     else if( arg->r_opt == -3 )
       g10_log_error("%s:%u: missing argument\n", filename, *lineno );
     else if( arg->r_opt == -7 )
       g10_log_error("%s:%u: invalid command\n", filename, *lineno );
     else if( arg->r_opt == -10 )
       g10_log_error("%s:%u: invalid alias definition\n",filename,*lineno);
     else
       g10_log_error("%s:%u: invalid option\n", filename, *lineno );
 }
 else {
     if( arg->r_opt == -3 )
       g10_log_error("Missing argument for option \"%.50s\"\n",
   arg->internal.last? arg->internal.last:"[??]" );
     else if( arg->r_opt == -6 )
       g10_log_error("Option \"%.50s\" does not expect an argument\n",
   arg->internal.last? arg->internal.last:"[??]" );
     else if( arg->r_opt == -7 )
       g10_log_error("Invalid command \"%.50s\"\n",
   arg->internal.last? arg->internal.last:"[??]" );
     else if( arg->r_opt == -8 )
       g10_log_error("Option \"%.50s\" is ambiguous\n",
   arg->internal.last? arg->internal.last:"[??]" );
     else if( arg->r_opt == -9 )
       g10_log_error("Command \"%.50s\" is ambiguous\n",
   arg->internal.last? arg->internal.last:"[??]" );
     else
       g10_log_error("Invalid option \"%.50s\"\n",
   arg->internal.last? arg->internal.last:"[??]" );
 }
  ...
  while (!ready)
    ...
    again:
      switch (state)
        {
        case skipWS:
          if (!(0 ? isascii(c) : (unsigned)(c) < 128) || !__isspace(c))
            {
              namelen = 0;
              state = collectNAME;
              goto again;
        ...
        case collectNAME:
          ...
          else if (namelen < (sizeof(name)/sizeof((name)[0]))-1)
            name[namelen++] = c;
          else
            state = skipNAME;
          break;
        ...
        ARGPARSE_ARGS *arg, ARGPARSE_OPTS *opts)
{
 ...
 if( c == '\n' || c== (-1) ) {
  ...
  break;
     ...
     ;
 else if( !state && __isspace(c) )
     ;
 else if( !state && c == '#' )
     state = 1;
 else if( state == 1 )
     ;
 else if( state == 2 && __isspace(c) ) {
     keyword[i] = 0;
     for(i=0; opts[i].short_opt; i++ )
  if( opts[i].long_opt && !strcmp( opts[i].long_opt, keyword) )
      break;
  ...
  if( i < buflen-1 )
     ...
     else if( i < (sizeof(keyword)/sizeof((keyword)[0]))-1 )
  keyword[i++] = c;
     else {
  buflen = (sizeof(keyword)/sizeof((keyword)[0]))+50;
...
arg_parse( ARGPARSE_ARGS *arg, ARGPARSE_OPTS *opts)
{
    ...
    initialize( arg, ((void*)0), ((void*)0) );
    argc = *arg->argc;
    argv = *arg->argv;
    idx = arg->internal.idx;

    if( !idx && argc && !(arg->flags & (1<<4)) ) {
 argc--; argv++; idx++;
    }
...
set_opt_arg(ARGPARSE_ARGS *arg, unsigned flags, char *s)
{
    ...
    switch( arg->r_type = (flags & 7) ) {
      case 1:
 arg->r.ret_int = (int)strtol(s,((void*)0),base);
 return 0;
      case 3:
 arg->r.ret_long= strtol(s,((void*)0),base);
 return 0;
      case 4:
 arg->r.ret_ulong= strtoul(s,((void*)0),base);
 return 0;
      case 2:
      default:
 arg->r.ret_str = s;
