...
print_warn(void)
{
  if (!no_warning)
    {
      g10_log_info(gettext ("WARNING: using insecure memory!\n"));
      g10_log_info(gettext ("please see http://www.gnupg.org/documentation/faqs.html" " for more information\n"));
...
# 168 "secmem.c"
 ...
 if( setuid( uid ) || getuid() != geteuid() || !setuid(0) )
     g10_log_fatal("failed to reset uid: %s\n", strerror((*__errno_location())));
    }

    if( err ) {
 if( (*__errno_location()) != (63)
     ...
     g10_log_error ("can't lock memory: %s\n", strerror(err));
...
init_pool( size_t n)
{
    ...
    if( disable_secmem )
 g10_log_bug("secure memory is disabled");


    pgsize_val = sysconf (30);
    ...
    poolsize = (poolsize + pgsize -1 ) & ~(pgsize-1);
       ...
       pool = mmap( 0, poolsize, 1|2,
     0x02|0x20, -1, 0);
# 262 "secmem.c"
    if( pool == (void*)-1 )
 g10_log_info("can't mmap pool of %u bytes: %s - using malloc\n",
 ...
 if( !pool )
     g10_log_fatal("can't allocate memory pool of %u bytes\n",
             (unsigned)poolsize);
 else
     pool_okay = 1;
    }
...
secmem_init( size_t n )
{
 ...
 if( uid != geteuid() ) {
     if( setuid( uid ) || getuid() != geteuid() || !setuid(0) )
  g10_log_fatal("failed to drop setuid\n" );
 }
 ...
 if( !pool_okay )
     ...
     g10_log_error("Oops, secure memory pool already initialized\n");
    }
...
secmem_malloc( size_t size )
{
    ...
    if( !pool_okay ) {
 g10_log_info(
  gettext ("operation is not possible without initialized secure memory\n"));
 g10_log_info(gettext ("(you may have used the wrong program for this task)\n"));
 exit(2);
    }
...
secmexrealloc( void *p, size_t newsize )
{
    ...
    if (size < sizeof(MEMBLOCK))
      g10_log_bug ("secure memory corrupted at block %p\n", (void *)mb);
...
secmem_free( void *a )
{
 ...
 return;
    ...
    do { volatile char *_vptr=(volatile char *)(mb); size_t _vlen=(size); while(_vlen) { *_vptr=(0xff); _vptr++; _vlen--; } } while(0);
    do { volatile char *_vptr=(volatile char *)(mb); size_t _vlen=(size); while(_vlen) { *_vptr=(0xaa); _vptr++; _vlen--; } } while(0);
    do { volatile char *_vptr=(volatile char *)(mb); size_t _vlen=(size); while(_vlen) { *_vptr=(0x55); _vptr++; _vlen--; } } while(0);
    do { volatile char *_vptr=(volatile char *)(mb); size_t _vlen=(size); while(_vlen) { *_vptr=(0x00); _vptr++; _vlen--; } } while(0);
    mb->size = size;
...
secmem_term()
{
 ...
 return;
    ...
    do { volatile char *_vptr=(volatile char *)(pool); size_t _vlen=(poolsize); while(_vlen) { *_vptr=(0xff); _vptr++; _vlen--; } } while(0);
    do { volatile char *_vptr=(volatile char *)(pool); size_t _vlen=(poolsize); while(_vlen) { *_vptr=(0xaa); _vptr++; _vlen--; } } while(0);
    do { volatile char *_vptr=(volatile char *)(pool); size_t _vlen=(poolsize); while(_vlen) { *_vptr=(0x55); _vptr++; _vlen--; } } while(0);
    do { volatile char *_vptr=(volatile char *)(pool); size_t _vlen=(poolsize); while(_vlen) { *_vptr=(0x00); _vptr++; _vlen--; } } while(0);
