...
{
    if( name )
 g10_log_bug0( "logger.c" , 49, __FUNCTION__ );
 ...
 logfp = fdopen( fd, "a" );
    if( !logfp ) {
 logfp = (stderr);
 g10_log_fatal("can't open fd %d for logging: %s\n", fd, strerror((*__errno_location())));
    }
}
...
g10_log_warning( const char *fmt, ... )
{
    ...
    if(strict)
      {
 errorcount++;
 g10_log_print_prefix(gettext ("ERROR: "));
      }
    else
      g10_log_print_prefix(gettext ("WARNING: "));
...
{
    g10_log_bug(gettext ("... this is a bug (%s:%d:%s)\n"), file, line, func );
}
# 259 "logger.c"
void
