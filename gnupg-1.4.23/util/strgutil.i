...
static ushort *active_charset = ((void*)0);
...
strlist_prev( STRLIST head, STRLIST node )
{
    ...
    for(n=((void*)0); head && head != node; head = head->next )
 n = head;
    return n;
}
...
trim_spaces( char *str )
{
    ...
    for( p=string; *p && __isspace(*(byte*)p) ; p++ )
 ;

    for( (mark = ((void*)0)); (*string = *p); string++, p++ )
 if( __isspace(*(byte*)p) ) {
     if( !mark )
  mark = string ;
 }
...
trim_trailing_chars( byte *line, unsigned len, const char *trimchars )
{
    ...
    for(mark=((void*)0), p=line, n=0; n < len; n++, p++ ) {
  ...
  if ((*__errno_location()) == (28))
    {
      ...
      if (!x)
        g10_log_info (gettext ("conversion from `%s' to `%s' not available\n"),
                  from, to);
    }
  else
    {
      ...
      if (!shown)
        g10_log_info (gettext ("iconv_open failed: %s\n"), strerror ((*__errno_location())));
      shown = 1;
    }
...
native_to_utf8( const char *string )
{
  ...
  if (no_translation)
  ...
  else
    {
      ...
      outptr = buffer;
      outbytes = length;
      if ( iconv (cd, ( char **)&inptr, &inbytes,
                  &outptr, &outbytes) == (size_t)-1)
        {
          static int shown;

          if (!shown)
            g10_log_info (gettext ("conversion from `%s' to `%s' failed: %s\n"),
                      active_charset_name, "utf-8", strerror ((*__errno_location())));
          shown = 1;
 ...
 if( !buffer ) {
     buffer = p = xmalloc( n + 1 );
            ...
            *p = 0;
            ...
            outbytes = n * 4;
            if (outbytes / 4 != n)
                g10_log_bug0( "strgutil.c" , 1064, __FUNCTION__ );
            ...
            if ( iconv (cd, ( char **)&inptr, &inbytes,
                ...
                if (!shown)
                  g10_log_info (gettext ("conversion from `%s' to `%s' failed: %s\n"),
                            "utf-8", active_charset_name, strerror ((*__errno_location())));
                shown = 1;
...
xasprintf (const char *fmt, ...)
{
  ...
  if (estream_vasprintf (&buf, fmt, ap) < 0)
    g10_log_fatal ("estream_asprintf failed: %s\n", strerror ((*__errno_location())));
  __builtin_va_end(ap);
  return buf;
}
...
do_strconcat (const char *s1, va_list arg_ptr)
{
  ...
  argv[argc++] = s1;
  needed = strlen (s1);
  while (((argv[argc] = __builtin_va_arg(arg_ptr, const char *))))
    {
...
ascii_strlwr (char *s)
{
  ...
  for (p = s; *p; p++ )
    if ((0 ? isascii(*p) : (unsigned)(*p) < 128) && *p >= 'A' && *p <= 'Z')
      *p |= 0x20;
