...
{
    g10_log_error ("out of %s memory while allocating %u bytes\n",
               secure? "secure":"" ,(unsigned)n );
    if (secure) {

        g10_log_info ("(this may be caused by too many secret keys used "
...
m_size( const void *a )
{
    ...
    g10_log_debug("dummy m_size called\n");
    return 0;
# 633 "memory.c"
}
...
char *
xtrystrdup(const char *a )
{
    size_t n = strlen (a);
    char *p = xtrymalloc(n+1 );
    if (p)
      strcpy (p, a);
    return p;
}
