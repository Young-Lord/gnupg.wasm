...
mailbox_from_userid (const char *userid)
{
  ...
  return result? ascii_strlwr (result) : ((void*)0);
}
...
get_pka_info (const char *address, void *fprbuf, size_t fprbuflen)
{
  ...
  *domain++ = 0;
  ...
  if (!hash)
    goto leave;
  name = strconcat (hash, "._pka.", domain, ((void*)0));
  if (!name)
    goto leave;
  if (get_cert (name, 1, 16384, ((void*)0), &fpr, &fpr_len, &url))
    goto leave;
  if (!fpr)
    goto leave;
