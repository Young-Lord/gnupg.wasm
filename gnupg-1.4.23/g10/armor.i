  ...
  if (len > 7 && !memcmp (line, "FAILED ", 7))
    {
      line += 7;
      len -= 7;
      for (; len && (*(line) >= '0' && *(line) <= '9'); len--, line++)
        {
          code *= 10;
          code += (*(line) - '0' );
        }
...
# 444 "armor.c"
    p = strchr( line, ':');
    if( !p || ((opt.compliance==CO_RFC2440) && p[1]!=' ')
 || (!(opt.compliance==CO_RFC2440) && p[1]!=' ' && p[1]!='\n' && p[1]!='\r'))
      {
 g10_log_error(gettext ("invalid armor header: "));
 ...
 return -1;
    ...
    if( opt.verbose ) {
 g10_log_info(gettext ("armor header: "));
 print_string( (stderr), line, len, 0 );
 putc('\n', (stderr));
    }
 ...
 if( (hashes=parse_hash_header( line )) )
   ...
   {
     g10_log_error(gettext ("invalid clearsig header\n"));
     return -1;
   }
      }
 ...
 g10_log_info(gettext ("unknown armor header: "));
 print_string( (stderr), line, len, 0 );
 putc('\n', (stderr));
      }
...
check_input( armor_filter_context_t *afx, IOBUF a )
{
 ...
 if( i >= 0 && !(afx->only_keyblocks && i != 1 && i != 5 && i != 6 )) {
     ...
     if( hdr_line == 3 ) {
  if( afx->in_cleartext ) {
      g10_log_error(gettext ("nested clear text signatures\n"));
      rc = 15;
  }
  afx->in_cleartext = 1;
     }
 ...
 if( i <= 0 ) {
     if (i && (opt.compliance==CO_RFC2440))
  rc = 15;
     break;
 }
    }
      ...
      size_t *retn, byte *buf, size_t size )
{
 ...
 if( afx->faked == 1 )
 ...
 else
   {
     ...
     if(tempbuf_len==512)
       {
  buf[len++]=0xE0+9;
  memcpy(&buf[len],tempbuf,512);
  len+=512;
  tempbuf_len=0;
  continue;
       }

     while( tempbuf_len < 512
 ...
 if(p[0]=='-')
  ...
  if( afx->not_dash_escaped && type != 2 )
    ;
  else
    {
      if( type != 2 )
        {
   g10_log_info(gettext ("unexpected armor: "));
  ...
  g10_log_info(gettext ("invalid dash escaped line: "));
  print_string( (stderr), p, n, 0 );
  putc('\n', (stderr));
       }
   }
       ...
       byte *buf, size_t size )
{
 ...
 if( c == '\n' || c == ' ' || c == '\r' || c == '\t' )
  ...
  if( isxdigit(cc1) && isxdigit(cc2)
      ...
      c = (0 ? isdigit(cc1) : ((unsigned)(cc1)-'0') < 10)? (cc1 - '0'): (ascii_toupper(cc1)-'A'+10);
      c <<= 4;
      c |= (0 ? isdigit(cc2) : ((unsigned)(cc2)-'0') < 10)? (cc2 - '0'): (ascii_toupper(cc2)-'A'+10);
      afx->buffer_pos += 2;
      afx->qp_detected = 1;
      goto again;
  }
  ...
  buf[n++] = val;
     ...
     g10_log_error(gettext ("invalid radix64 character %02X skipped\n"), c2);
     continue;
 }
 switch(idx) {
 ...
 if( c == -1 )
     g10_log_error(gettext ("premature eof (no CRC)\n"));
 else {
     u32 mycrc = 0;
     idx = 0;
     do {
  ...
  if( !afx->buffer_len )
     ...
     if( c == -1 ) {
  g10_log_info(gettext ("premature eof (in CRC)\n"));
  rc = invalid_crc();
     }
     else if( idx == 0 ) {
  ...
  g10_log_info(gettext ("malformed CRC\n"));
  rc = invalid_crc();
     }
     else if( mycrc != afx->crc ) {
                g10_log_info (gettext ("CRC error; %06lX - %06lX\n"),
...
# 999 "armor.c"
    if( (opt.debug & 8) )
 g10_log_debug("armor-filter: control: %d\n", control );
    if( control == 3 && afx->inp_bypass ) {
 n = 0;
 if( afx->buffer_len ) {
 ...
 for(; n < size; n++ ) {
     if( (c=( ((a)->nofast || (a)->d.start >= (a)->d.len )? iobuf_readbyte((a)) : ( (a)->nbytes++, (a)->d.buf[(a)->d.start++] ) )) == -1 )
  break;
     buf[n] = c & 0xff;
 }
 if( !n )
 ...
 *ret_len = n;
 ...
 if( size < 512 +1 )
     g10_log_bug0( "armor.c" , 1023, __FUNCTION__ );
 ...
 if( afx->faked )
  ...
  if( !n )
      rc = -1;
         ...
         unsigned int hashes = afx->hashes;
                ...
                if ( sesmarklen > 20 )
                    g10_log_bug0( "armor.c" , 1044, __FUNCTION__ );
  ...
  n=0;
                ...
                buf[n++] = afx->not_dash_escaped? 0:1;
                if( hashes & 1 )
                    buf[n++] = 3;
                if( hashes & 2 )
                    buf[n++] = 2;
 ...
 *ret_len = n;
    ...
    else if( control == 4 && !afx->cancel ) {
 if( !afx->status ) {
     const char *s;
     STRLIST comment=opt.comments;

     if( afx->what >= (sizeof(head_strings)/sizeof((head_strings)[0])) )
  g10_log_bug("afx->what=%d", afx->what);
     iobuf_writestr(a, "-----");
  ...
  if( ++idx2 >= (64/4) )
    {
     ...
     if( afx->what >= (sizeof(tail_strings)/sizeof((tail_strings)[0])) )
  g10_log_bug("afx->what=%d", afx->what);
     iobuf_writestr(a, "-----");
     ...
     g10_log_error(gettext ("no valid OpenPGP data found.\n"));
     afx->no_openpgp_data = 1;
     write_status_text( 24, "1" );
 }
 if( afx->truncated )
     g10_log_info(gettext ("invalid armor: line longer than %d characters\n"),
        20000 );

 if( afx->qp_detected )
     g10_log_error(gettext ("quoted printable character in armor - " "probably a buggy MTA has been used\n") );
