...
static struct parse_options keyserver_opts[]=
  {
    ...
    {"max-cert-size",0,((void*)0),((void*)0)},
    {"include-revoked",0,((void*)0),("include revoked keys in search results")},
    {"include-subkeys",0,((void*)0),("include subkeys when searching by key ID")},
    {"use-temp-files",0,((void*)0),
     ("use temporary files to pass data to keyserver helpers")},
    {"keep-temp-files",(1<<1),((void*)0),
     ("do not delete temporary files after using them")},
    {"refresh-add-fake-v3-keyids",(1<<2),((void*)0),
     ((void*)0)},
    {"auto-key-retrieve",(1<<3),((void*)0),
     ("automatically retrieve keys when verifying signatures")},
    {"honor-keyserver-url",(1<<4),((void*)0),
     ("honor the preferred keyserver URL set on the key")},
    {"honor-pka-record",(1<<5),((void*)0),
...
static size_t max_cert_size=16384;
...
# 152 "keyserver.c"
 ...
 opt.keyserver_options.options|=(1<<0);
      ...
      &opt.keyserver_options.export_options,0))
 {
  ...
  if(max_cert)
    {
      max_cert_size=strtoul(max_cert,(char **)((void*)0),10);
      ...
      const char *configname,unsigned int configlineno)
{
  ...
  ((void)((string!=((void*)0)) || (__assert_fail("string!=NULL", "keyserver.c", 244, __func__),0)));
      ...
      if(*uri=='[')
 {
  ...
  && (((0 ? isascii(*idx) : (unsigned)(*idx) < 128) && isxdigit(*idx))
                    || *idx==':' || *idx=='.');idx++)
     count++;
  ...
  if(p && plen)
    {
      ...
      memcpy(dupe,p,plen);
      dupe[plen]='\0';
      spec=parse_keyserver_uri(dupe,1,((void*)0),0);
      xfree(dupe);
    }
...
print_keyrec(int number,struct keyrec *keyrec)
{
  ...
  printf("(%d)\t%s  ",number,( (keyrec->uidbuf)->d.buf ));
 ...
 printf("key %s",keystr(kid));
    ...
    default:
      g10_log_bug0( "keyserver.c" , 519, __FUNCTION__ );
      break;
    }
  ...
  if(keyrec->createtime>0)
    {
      printf(", ");
      printf(gettext ("created: %s"),strtimestamp(keyrec->createtime));
    }

  if(keyrec->expiretime>0)
    {
      printf(", ");
      printf(gettext ("expires: %s"),strtimestamp(keyrec->expiretime));
    }

  if(keyrec->flags&1)
    printf(" (%s)",gettext ("revoked"));
  if(keyrec->flags&2)
    printf(" (%s)",gettext ("disabled"));
  if(keyrec->flags&4)
    printf(" (%s)",gettext ("expired"));
...
{
  static struct keyrec *work=((void*)0);
  struct keyrec *ret=((void*)0);
  char *record;
  int i;
      ...
      else
 {
  ...
  for(i=strlen(keystring);i>0;i--)
    if(((keystring[i-1])==' ' || (keystring[i-1])=='\n' || (keystring[i-1])=='\r' || (keystring[i-1])=='\t'))
      keystring[i-1]='\0';
    else
      break;

  if((record=strsep(&keystring,":"))==((void*)0))
    return ret;
      ...
      if(work->desc.mode)
 {
   ...
   work->uidbuf=iobuf_temp();
      ...
      if((tok=strsep(&keystring,":"))==((void*)0))
 return ret;
...
show_prompt(KEYDB_SEARCH_DESC *desc,int numdesc,int count,const char *search)
{
  ...
  answer=cpr_get_no_help("keysearch.prompt",
    gettext ("Enter number(s), N)ext, or Q)uit > "));
  ...
  if(answer[0]=='q' || answer[0]=='Q')
  ...
  else if(atoi(answer)>=1 && atoi(answer)<=numdesc)
    {
      ...
      while((num=strsep(&split," ,"))!=((void*)0))
 if(atoi(num)>=1 && atoi(num)<=numdesc)
   keyserver_work(KS_GET,((void*)0),&desc[atoi(num)-1],1,
    ((void*)0),((void*)0),opt.keyserver);
...
keyserver_search_prompt(IOBUF buffer,const char *searchstr)
{
      ...
      if(!started && rl>0 && ascii_strncasecmp("info:",line,5)==0)
 {
  ...
  version=1;
       ...
       if(version!=1)
  {
    g10_log_error(gettext ("invalid keyserver protocol " "(us %d!=handler %d)\n"),1,version);

    break;
  }
     }

   if((tok=strsep(&str,":"))!=((void*)0) && sscanf(tok,"%d",&count)==1)
     {
       if(count==0)
  goto notfound;
       else if(count<0)
...
# 918 "keyserver.c"
  if(count==0)
    {
      if(localstr)
 g10_log_info(gettext ("key \"%s\" not found on keyserver\n"),localstr);
      else
 g10_log_info(gettext ("key not found on keyserver\n"));
    }
...
keyserver_errstr (int code)
{
  ...
  switch (code)
    ...
    case 1:s = "keyserver helper internal error"; break;
    case 2: s = "keyserver not supported"; break;
    case 3: s = "keyserver helper version mismatch";break;
    case 4: s = "keyserver helper general error"; break;
    case 5: s = "keyserver helper is out of core"; break;
    case 6: s = "key not found"; break;
    case 7: s = "key exists"; break;
    case 8:s = "key incomplete (EOF)"; break;
    case 9: s = "keyserver unreachable"; break;
    case 10: s = "keyserver timeout"; break;
    default: s = "?"; break;
    }
  return s;
}
  ...
  struct keyserver_spec *keyserver)
{
  ...
  char *command,*end,*searchstr=((void*)0);
  byte *line=((void*)0);
  struct exec_info *spawn;
  const char *scheme;
  const char *libexecdir = get_libexecdir ();

  ((void)((keyserver) || (__assert_fail("keyserver", "keyserver.c", 1081, __func__),0)));
...
# 1111 "keyserver.c"
  if(opt.exec_path_set)
    {
      ...
      command=xmalloc((strlen("gpgkeys_" "curl" ""))+strlen(scheme)+3+strlen("")+1);
      command[0]='\0';
    }
  else

    {
      ...
      command=xmalloc(strlen(libexecdir)+strlen("/")+
        (strlen("gpgkeys_" "curl" ""))+strlen(scheme)+3+strlen("")+1);
  ...
  if(path_access(command,1)!=0)
    strcpy(end,"gpgkeys_" "curl" "");

  if(opt.keyserver_options.options&(1<<0))
    {
      if(opt.keyserver_options.options&(1<<1))
 {
   command=xrealloc(command,strlen(command)+
       strlen(" -o \"%O\" \"%I\"")+1);
   strcat(command," -o \"%O\" \"%I\"");
 }
      else
 {
   command=xrealloc(command,strlen(command)+
   ...
   strcat(command," -o \"%o\" \"%i\"");
      ...
      ret=exec_write(&spawn,((void*)0),command,((void*)0),0,0);
    }
  else
    ret=exec_write(&spawn,command,((void*)0),((void*)0),0,0);
 ...
 for(i=0;i<count;i++)
  ...
  quiet=1;
       ...
       g10_log_bug0( "keyserver.c" , 1264, __FUNCTION__ );

     if(!quiet)
       {
  if(keyserver->host)
    g10_log_info(gettext ("requesting key %s from %s server %s\n"),
      keystr_from_desc(&desc[i]),
      keyserver->scheme,keyserver->host);
  else
    g10_log_info(gettext ("requesting key %s from %s\n"),
 ...
 for(key=list;key!=((void*)0);key=key->next)
   fprintf(spawn->tochild,"%s\n",key->d);

 fprintf(spawn->tochild,"\n");

 if(keyserver->host)
   g10_log_info(gettext ("searching for names from %s server %s\n"),
     keyserver->scheme,keyserver->host);
 else
   g10_log_info(gettext ("searching for names from %s\n"),keyserver->uri);
...
# 1407 "keyserver.c"
  ...
  if(keyserver->host)
    g10_log_info(gettext ("sending key %s to %s server %s\n"),
      keystr(block->pkt->pkt.public_key->keyid),
      keyserver->scheme,keyserver->host);
  else
    g10_log_info(gettext ("sending key %s to %s\n"),
      keystr(block->pkt->pkt.public_key->keyid),
      keyserver->uri);
 ...
 if(keyserver->host)
   g10_log_info(gettext ("searching for \"%s\" from %s server %s\n"),
     searchstr,keyserver->scheme,keyserver->host);
 else
   g10_log_info(gettext ("searching for \"%s\" from %s\n"),
 ...
 break;
    ...
    default:
      g10_log_fatal(gettext ("no keyserver action!\n"));
      break;
    }
      ...
      if(iobuf_read_line(spawn->fromchild,&line,&buflen,&maxlen)==0)
 {
   ...
   goto fail;
      ...
      plen=strlen(ptr);
      while(plen>0 && ((ptr[plen-1])==' ' || (ptr[plen-1])=='\n' || (ptr[plen-1])=='\r' || (ptr[plen-1])=='\t'))
 plen--;
      ptr[plen]='\0';
 ...
 {
   if(ascii_strncasecmp(&ptr[8],"1.4.23",strlen("1.4.23"))!=0)
     g10_log_info(gettext ("WARNING: keyserver handler from a different" " version of GnuPG (%s)\n"),&ptr[8]);
 ...
 outofband=1;
  ...
  if(!gotversion)
    {
      g10_log_error(gettext ("keyserver did not send VERSION\n"));
      goto fail;
    }
...
# 1628 "keyserver.c"
   ...
   import_release_stats_handle(stats_handle);
          if (gpgkeys_err)
            {
              g10_log_error (gettext ("keyserver communications error: %s\n"),
        ...
        struct keyserver_spec *keyserver)
{
  ...
  if(!keyserver)
    {
      g10_log_error(gettext ("no keyserver known (use option --keyserver)\n"));
      return 46;
    }
# 1691 "keyserver.c"
 ...
 {
 case 127:
   g10_log_error(gettext ("no handler for keyserver scheme `%s'\n"),
      keyserver->scheme);
   break;

 case 2:
   g10_log_error(gettext ("action `%s' not supported with keyserver " "scheme `%s'\n"),
 ...
 case 3:
   g10_log_error(gettext ("gpgkeys_" "%s does not support" " handler version %d\n"),
 ...
 case 10:
   g10_log_error(gettext ("keyserver timed out\n"));
   break;
 ...
 case 1:
 default:
   g10_log_error(gettext ("keyserver internal error\n"));
   break;
 }
...
keyserver_export(STRLIST users)
{
 ...
 {
   g10_log_error(gettext ("\"%s\" not a key ID: skipping\n"),users->d);
   continue;
 }
      else
 append_to_strlist(&sl,users->d);
  ...
  if(sl)
    {
      rc=keyserver_work(KS_SEND,sl,((void*)0),0,((void*)0),((void*)0),opt.keyserver);
      free_strlist(sl);
    }
...
keyserver_import(STRLIST users)
{
      ...
      if(count==num)
 {
  ...
  if(count>0)
    rc=keyserver_work(KS_GET,((void*)0),desc,count,((void*)0),((void*)0),opt.keyserver);
   ...
   struct keyserver_spec *keyserver)
{
  ...
  return keyserver_work(KS_GET,((void*)0),&desc,1,((void*)0),((void*)0),keyserver);
}
...
keyidlist(STRLIST users,KEYDB_SEARCH_DESC **klist,int *count,int fakev3)
{
      ...
      for (ndesc=0, sl=users; sl; sl = sl->next)
 {
  ...
  while (!(rc = keydb_search2((kdbhd),(desc),(ndesc),((void*)0))))
    {
      if (!users)
      ...
      if((node=find_kbnode(keyblock,PKT_PUBLIC_KEY)))
 {
   ...
   if(fakev3 && ((node->pkt->pkt.public_key->pubkey_algo)==1 || (node->pkt->pkt.public_key->pubkey_algo)==2 || (node->pkt->pkt.public_key->pubkey_algo)==3 ) &&
       ...
       if(*count==num)
  {
   ...
   (*klist)[*count].skipfncvalue=((void*)0);


   if(opt.keyserver_options.options&(1<<4))
     {
...
keyserver_refresh(STRLIST users)
{
  ...
  opt.keyserver_options.import_options|=(1<<4);





  opt.keyserver_options.import_options|=(1<<2);



  if((opt.keyserver_options.options&(1<<2)) && opt.keyserver
   ...
   if(desc[i].skipfncvalue)
     {
       ...
       rc=keyserver_work(KS_GET,((void*)0),&desc[i],1,((void*)0),((void*)0),keyserver);
       if(rc)
  g10_log_info(gettext ("WARNING: unable to refresh key %s" " via %s: %s\n"),keystr_from_desc(&desc[i]),
 ...
 {
   if(count==1)
     g10_log_info(gettext ("refreshing 1 key from %s\n"),opt.keyserver->uri);
   else
     g10_log_info(gettext ("refreshing %d keys from %s\n"),
       count,opt.keyserver->uri);
 }

      rc=keyserver_work(KS_GET,((void*)0),desc,numdesc,((void*)0),((void*)0),opt.keyserver);
    }
  ...
  if(!(opt.keyserver_options.import_options&(1<<2)))
...
{
  if(tokens)
    return keyserver_work(KS_SEARCH,tokens,((void*)0),0,((void*)0),((void*)0),opt.keyserver);
  else
    return 0;
}
  ...
  for(sl=urilist;sl;sl=sl->next)
    {
      ...
      spec=parse_keyserver_uri(sl->d,1,((void*)0),0);
      if(spec)
 {
   int rc;

   rc=keyserver_work(KS_GET,((void*)0),&desc,1,((void*)0),((void*)0),spec);
   if(rc)
     g10_log_info (gettext ("WARNING: unable to fetch URI %s: %s\n"),
       sl->d,g10_errstr(rc));

   free_keyserver_spec(spec);
 }
      else
 g10_log_info (gettext ("WARNING: unable to parse URI %s\n"),sl->d);
    }
...
keyserver_import_cert(const char *name,unsigned char **fpr,size_t *fpr_len)
{
  ...
  if(type==1)
    {
      ...
      rc=import_keys_stream (key, ((void*)0), fpr, fpr_len,
                             (opt.keyserver_options.import_options
      ...
      if(url)
 {
   ...
   spec=parse_keyserver_uri(url,1,((void*)0),0);
   if(spec)
     {
 ...
 g10_log_info(gettext ("no keyserver known (use option --keyserver)\n"));
...
keyserver_import_pka(const char *name,unsigned char **fpr,size_t *fpr_len)
{
  ...
  *fpr_len=20;
  ...
  if (uri && *uri)
    {
      struct keyserver_spec *spec;
      spec = parse_keyserver_uri (uri, 1, ((void*)0), 0);
      if (spec)
 {
        ...
        struct keyserver_spec *keyserver)
{
  ...
  rc=keyserver_work(KS_GETNAME,list,((void*)0),0,fpr,fpr_len,keyserver);
