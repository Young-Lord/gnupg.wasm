...
keyring_register_filename (const char *fname, int secret, void **ptr)
{
    ...
    if (active_handles)
        g10_log_bug0( "keyring.c" , 204, __FUNCTION__ );
...
keyring_new (void *token, int secret)
{
  ...
  ((void)((resource && !resource->secret == !secret) || (__assert_fail("resource && !resource->secret == !secret", "keyring.c", 257, __func__),0)));
...
keyring_release (KEYRING_HANDLE hd)
{
    if (!hd)
        return;
    ((void)((active_handles > 0) || (__assert_fail("active_handles > 0", "keyring.c", 271, __func__),0)));
    active_handles--;
    ...
    if (yes) {
        ...
        for (kr=kr_names; kr; kr = kr->next) {
            ...
            if (!kr->lockhd) {
                ...
                if (!kr->lockhd) {
                    g10_log_info ("can't allocate lock for `%s'\n", kr->fname );
                    rc = 1;
                }
            }
        }
...
keyring_get_keyblock (KEYRING_HANDLE hd, KBNODE *ret_kb)
{
    PACKET *pkt;
    int rc;
    KBNODE keyblock = ((void*)0), node, lastnode;
    IOBUF a;
    int in_cert = 0;
    int pk_no = 0;
    int uid_no = 0;
 ...
 return 13;
    ...
    while ((rc=dbg_parse_packet( (a), (pkt), "keyring.c", 390 )) != -1) {
        hd->found.n_packets++;
        if (rc == 2) {
     free_packet (pkt);
     do { (pkt)->pkttype = 0; (pkt)->pkt.generic = ((void*)0); } while(0);
 ...
 if (rc) {
            g10_log_error ("keyring_get_keyblock: read error: %s\n",
                       g10_errstr(rc) );
            rc = 32;
            break;
        }
        ...
        switch (pkt->pkttype){
          ...
          default:
     g10_log_error ("skipped packet of type %d in keyring\n",
                       (int)pkt->pkttype);
     free_packet(pkt);
     do { (pkt)->pkttype = 0; (pkt)->pkt.generic = ((void*)0); } while(0);
     continue;
...
keyring_update_keyblock (KEYRING_HANDLE hd, KBNODE kb)
{
    ...
    if (!hd->found.n_packets) {
        ...
        rc = keyring_get_keyblock (hd, ((void*)0));
        if (rc) {
            g10_log_error ("re-reading keyblock failed: %s\n", g10_errstr (rc));
            return rc;
        }
        if (!hd->found.n_packets)
            g10_log_bug0( "keyring.c" , 528, __FUNCTION__ );
    }
...
keyring_insert_keyblock (KEYRING_HANDLE hd, KBNODE kb)
{
    ...
    if (!hd)
        ...
        fname = hd->resource? hd->resource->fname:((void*)0);
...
keyring_delete_keyblock (KEYRING_HANDLE hd)
{
    ...
    if (!hd->found.n_packets) {
        ...
        if (!hd->found.n_packets)
            g10_log_bug0( "keyring.c" , 604, __FUNCTION__ );
    }
    ...
    rc = do_copy (2, hd->found.kr->fname, ((void*)0), hd->secret,
                  hd->found.offset, hd->found.n_packets );
    if (!rc) {
...
{
    ((void)((hd) || (__assert_fail("hd", "keyring.c", 635, __func__),0)));

    hd->current.kr = ((void*)0);
    iobuf_close (hd->current.iobuf);
    hd->current.iobuf = ((void*)0);
    hd->current.eof = 0;
    hd->current.error = 0;
...
prepare_search (KEYRING_HANDLE hd)
{
    ...
    if (!hd->current.kr) {
        ...
        ((void)((!hd->current.iobuf) || (__assert_fail("!hd->current.iobuf", "keyring.c", 670, __func__),0)));
    }
    else {
        iobuf_close (hd->current.iobuf);
        hd->current.iobuf = ((void*)0);
...
compare_name (int mode, const char *name, const char *uid, size_t uidlen)
{
 ...
 g10_log_bug0( "keyring.c" , 871, __FUNCTION__ );
  ...
  size_t ndesc, size_t *descindex)
{
  ...
  if (need_words)
    {
      ...
      g10_log_debug ("word search mode does not yet work\n");
      ...
      ((void)((name) || (__assert_fail("name", "keyring.c", 977, __func__),0)));
  ...
  pk_no = uid_no = 0;
  initial_skip = 1;
  while (!(rc=dbg_search_packet( (hd->current.iobuf), (&pkt), (&offset), (need_uid), "keyring.c", 996 )))
    {
      byte afp[20];
      size_t an;
      ...
      for (n=0; n < ndesc; n++)
        {
          ...
          case KEYDB_SEARCH_MODE_NONE:
            g10_log_bug0( "keyring.c" , 1058, __FUNCTION__ );
            break;
          case KEYDB_SEARCH_MODE_EXACT:
          case KEYDB_SEARCH_MODE_SUBSTR:
          case KEYDB_SEARCH_MODE_MAIL:
                 ...
                 char **r_bakfname, char **r_tmpfname, IOBUF *r_fp)
{
  ...
  *r_tmpfname = ((void*)0);
# 1203 "keyring.c"
    bakfname = xmalloc (strlen (template) + sizeof ("~") + 1);
    strcpy (stpcpy (bakfname, template), "~");

    tmpfname = xmalloc (strlen (template) + sizeof ("." "tmp") + 1 );
    strcpy (stpcpy(tmpfname, template), "." "tmp");
                 ...
                 const char *fname, int secret )
{
  ...
  if(secret && iobuf_ioctl(((void*)0),4,0,(char*)tmpfname)!=0)
    goto fail;


  if(iobuf_ioctl (((void*)0), 2, 0, (char*)tmpfname )!=0)
    goto fail;
  iobuf_ioctl (((void*)0), 2, 0, (char*)bakfname );
  ...
  if (!secret)
    {
      ...
      if (rename (fname, bakfname) )
        {
          g10_log_error ("renaming `%s' to `%s' failed: %s\n",
                     fname, bakfname, strerror((*__errno_location())) );
          return 37;
 }
    }
  ...
  if (rename (tmpfname, fname) )
    {
      g10_log_error (gettext ("renaming `%s' to `%s' failed: %s\n"),
                 tmpfname, fname, strerror((*__errno_location())) );
      register_secured_file (fname);
      rc = 37;
      goto fail;
    }
 ...
 (stat(bakfname,&statbuf)==0)) &&
      ...
      g10_log_error("WARNING: unable to restore permissions to `%s': %s",
 ...
 fail:
  if(secret)
    {
      g10_log_info(gettext ("WARNING: 2 files with confidential information exists.\n"));
      g10_log_info(gettext ("%s is the unchanged one\n"), fname );
      g10_log_info(gettext ("%s is the new one\n"), tmpfname );
      g10_log_info(gettext ("Please fix this possible security flaw\n"));
    }
      ...
      if (node->pkt->pkttype == PKT_SIGNATURE)
        {
          ...
          iobuf_writebyte(fp,0xb0);
          iobuf_writebyte(fp,2);
          iobuf_writebyte(fp,0);
          if (iobuf_writebyte(fp,cacheval)) {
            g10_log_error ("writing sigcache packet failed\n");
            return 22;
          }
        }
    }
...
keyring_rebuild_cache (void *token,int noisy)
{
  ...
  const char *lastresname = ((void*)0), *resname;
  IOBUF tmpfp = ((void*)0);
  char *tmpfilename = ((void*)0);
  char *bakfilename = ((void*)0);
  int rc;
  ...
  while ( !(rc = keyring_search (hd, &desc, 1, ((void*)0))) )
    {
      ...
      if (lastresname != resname )
        {
          if (tmpfp)
            {
              if (iobuf_close (tmpfp))
                {
                  g10_log_error ("error closing `%s': %s\n",
                             tmpfilename, strerror ((*__errno_location())));
                  rc = 36;
                  goto leave;
                }
          ...
          rc = lastresname? rename_tmp_file (bakfilename, tmpfilename,
                                             lastresname, 0) : 0;
          xfree (tmpfilename); tmpfilename = ((void*)0);
          xfree (bakfilename); bakfilename = ((void*)0);
          if (rc)
            goto leave;
          lastresname = resname;
          if (noisy && !opt.quiet)
            g10_log_info (gettext ("caching keyring `%s'\n"), resname);
      ...
      if (rc)
        {
          g10_log_error ("keyring_get_keyblock failed: %s\n", g10_errstr(rc));
          goto leave;
        }
      if ( keyblock->pkt->pkttype != PKT_PUBLIC_KEY)
        {
          ...
          g10_log_error ("unexpected keyblock found (pkttype=%d)%s\n",
                     keyblock->pkt->pkttype, noisy? " - deleted":"");
          if (noisy)
            continue;
          g10_log_info ("Hint: backup your keys and try running `%s'\n",
  ...
  check_key_signature (keyblock, node, ((void*)0));
      ...
      if ( !(++count % 50) && noisy && !opt.quiet)
        g10_log_info(gettext ("%lu keys cached so far (%lu signatures)\n"),
                 count, sigcount );
  ...
  if (rc)
    {
      g10_log_error ("keyring_search failed: %s\n", g10_errstr(rc));
      goto leave;
    }
  if(noisy || opt.verbose)
    g10_log_info(gettext ("%lu keys cached (%lu signatures)\n"), count, sigcount );
  if (tmpfp)
    {
      if (iobuf_close (tmpfp))
        {
         ...
         off_t start_offset, unsigned int n_packets )
{
    IOBUF fp, newfp;
    int rc=0;
    char *bakfname = ((void*)0);
    char *tmpfname = ((void*)0);
    ...
    fp = iobuf_open (fname);
    if (mode == 1 && !fp && (*__errno_location()) == (44)) {
 ...
 if( !opt.quiet )
     g10_log_info(gettext ("%s: keyring created\n"), fname );
 ...
 if( iobuf_close(newfp) ) {
     g10_log_error ("%s: close failed: %s\n", fname, strerror((*__errno_location())));
     return 36;
 }
 return 0;
    }
 ...
 rc = dbg_copy_all_packets((fp),(newfp), "keyring.c", 1585 );
 if( rc != -1 ) {
     g10_log_error("%s: copy to `%s' failed: %s\n",
 ...
 rc = dbg_copy_some_packets((fp),(newfp),(start_offset), "keyring.c", 1600 );
 if( rc ) {
     g10_log_error ("%s: copy to `%s' failed: %s\n",
                       fname, tmpfname, g10_errstr(rc) );
     iobuf_close(fp);
 ...
 ((void)((n_packets) || (__assert_fail("n_packets", "keyring.c", 1611, __func__),0)));
 rc = dbg_skip_some_packets((fp),(n_packets), "keyring.c", 1612 );
 if( rc ) {
     g10_log_error("%s: skipping %u packets failed: %s\n",
 ...
 rc = dbg_copy_all_packets((fp),(newfp), "keyring.c", 1637 );
 if( rc != -1 ) {
     g10_log_error("%s: copy to `%s' failed: %s\n",
        fname, tmpfname, g10_errstr(rc) );
     iobuf_close(fp);
