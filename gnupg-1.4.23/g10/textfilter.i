   ...
   byte *buf, size_t size, size_t *ret_len)
{
    ...
    ((void)((size > 10) || (__assert_fail("size > 10", "textfilter.c", 73, __func__),0)));
    size -= 2;
    while( !rc && len < size ) {
 int lf_seen;
      ...
      IOBUF a, byte *buf, size_t *ret_len)
{
 ...
 if( tfx->truncated )
     g10_log_error(gettext ("can't handle text lines longer than %d characters\n"),
   19995 );
 xfree( tfx->buffer );
 tfx->buffer = ((void*)0);
    }
      ...
      int escape_dash, int escape_from, int pgp2mode )
{
 ...
 if( escape_dash ) {
     if( pending_lf ) {
  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = ('\r') & 0xff; } while(0);
  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = ('\n') & 0xff; } while(0);
     }
     md_write( md, buffer,
...
# 234 "textfilter.c"
 ...
 if( !escape_dash )
    ...
    if( truncated )
 g10_log_info(gettext ("input line longer than %d characters\n"), 19995 );
