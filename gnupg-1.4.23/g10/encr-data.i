...
release_dfx_context (decode_filter_ctx_t dfx)
{
  ...
  ((void)((dfx->refcount) || (__assert_fail("dfx->refcount", "encr-data.c", 57, __func__),0)));
  if ( !--dfx->refcount )
    {
      cipher_close (dfx->cipher_hd);
      dfx->cipher_hd = ((void*)0);
...
decrypt_data( void *procctx, PKT_encrypted *ed, DEK *dek )
{
 ...
 if( s )
     g10_log_info(gettext ("%s encrypted data\n"), s );
 else
     g10_log_info(gettext ("encrypted with unknown algorithm %d\n"), dek->algo );
    ...
    if (opt.show_session_key)
      {
        ...
        g10_log_info ("session key: `%s'\n", buf);
        write_status_text (52, buf);
      }
 ...
 g10_log_fatal("unsupported blocksize %u\n", blocksize );
    nprefix = blocksize;
    if( ed->len && ed->len < (nprefix+2) )
 g10_log_bug0( "encr-data.c" , 120, __FUNCTION__ );
 ...
 if ( (opt.debug & 512) )
    ...
    if( rc == 43 )
      {
 g10_log_info(gettext ("WARNING: message was encrypted with" " a weak key in the symmetric cipher.\n"));

 rc=0;
      }
    else if( rc )
      {
 g10_log_error("key setup failed: %s\n", g10_errstr(rc) );
 goto leave;

      }
    if (!ed->buf) {
        g10_log_error(gettext ("problem handling encrypted packet\n"));
        goto leave;
    }

    cipher_setiv ( dfx->cipher_hd, ((void*)0), 0 );

    if( ed->len ) {
 for(i=0; i < (nprefix+2) && ed->len; i++, ed->len-- ) {
     if( (c=( ((ed->buf)->nofast || (ed->buf)->d.start >= (ed->buf)->d.len )? iobuf_readbyte((ed->buf)) : ( (ed->buf)->nbytes++, (ed->buf)->d.buf[(ed->buf)->d.start++] ) )) == -1 )
  break;
     else
  temp[i] = c;
 }
...
# 204 "encr-data.c"
 int datalen = md_digest_length( ed->mdc_method );
        ...
        ((void)((dfx->cipher_hd) || (__assert_fail("dfx->cipher_hd", "encr-data.c", 206, __func__),0)));
        ((void)((dfx->mdc_hash) || (__assert_fail("dfx->mdc_hash", "encr-data.c", 207, __func__),0)));
           ...
           byte *buf, size_t *ret_len)
{
    ...
    if( control == 3 && dfx->eof_seen ) {
 *ret_len = 0;
 rc = -1;
    }
    else if( control == 3 ) {
 ((void)((a) || (__assert_fail("a", "encr-data.c", 243, __func__),0)));
 ((void)((size > 44) || (__assert_fail("size > 44", "encr-data.c", 244, __func__),0)));
 ...
 if( n ) {
     ...
     ((void)((dfx->eof_seen) || (__assert_fail("dfx->eof_seen", "encr-data.c", 294, __func__),0)));
     rc = -1;
 }
 *ret_len = n;
    }
...
decode_filter( void *opaque, int control, IOBUF a, byte *buf, size_t *ret_len)
{
    ...
    if( control == 3 ) {
 ((void)((a) || (__assert_fail("a", "encr-data.c", 316, __func__),0)));
