...
do_show_revocation_reason( PKT_signature *sig )
{
 ...
 if( *p == 0 )
     text = gettext ("No reason specified");
 else if( *p == 0x01 )
     text = gettext ("Key is superseded");
 else if( *p == 0x02 )
     text = gettext ("Key has been compromised");
 else if( *p == 0x03 )
     text = gettext ("Key is no longer used");
 else if( *p == 0x20 )
     text = gettext ("User ID is no longer valid");
 else
     text = ((void*)0);

 g10_log_info( gettext ("reason for revocation: ") );
 if( text )
 ...
 do {
  ...
  pp = memchr( p, '\n', n );
  nn = pp? pp - p : n;
  g10_log_info( gettext ("revocation comment: ") );
...
# 115 "pkclist.c"
    ...
    if( rc ) {
 g10_log_debug( "failed to get the keyblock\n");
 return;
    }
   ...
   ( ( node->pkt->pkttype == PKT_PUBLIC_KEY
    ...
    if( !node ) {
 g10_log_debug("Oops, PK not in keyblock\n");
                    ...
                    unsigned *new_trust, int defer_help )
{
  ...
  switch(minimum)
    {
    default:
    case 2: min_num=1; break;
    case 3: min_num=2; break;
    case 4: min_num=3; break;
    case 5: min_num=4; break;
    }

  keyid_from_pk (pk, keyid);
  for(;;) {
# 207 "pkclist.c"
    const char *ans = gettext ("iImMqQsS");
    ...
    if( !did_help )
        ...
        if( !mode )
          {
            ...
            tty_printf(gettext ("No trust value assigned to:\n"));
     tty_printf("%4u%c/%s %s\n",nbits_from_pk( pk ),
     ...
     xfree(p);
            ...
            if (!keyblock)
                g10_log_bug0( "pkclist.c" , 225, __FUNCTION__ );
  ...
  if((opt.verify_options&(1<<0))
     && un->pkt->pkt.user_id->attrib_data)
    show_photos(un->pkt->pkt.user_id->attribs,
         un->pkt->pkt.user_id->numattribs,pk,((void*)0),
         un->pkt->pkt.user_id);

  p=utf8_to_native(un->pkt->pkt.user_id->name,
     un->pkt->pkt.user_id->len,0);

  tty_printf(gettext ("  aka \"%s\"\n"),p);
       }
 ...
 if(opt.trust_model==TM_DIRECT)
   {
     tty_printf(gettext ("How much do you trust that this key actually " "belongs to the named user?\n"));
 ...
 if(min_num<=1)
   tty_printf (gettext ("  %d = I don't know or won't say\n"), 1);
 if(min_num<=2)
   tty_printf (gettext ("  %d = I do NOT trust\n"), 2);
 if(min_num<=3)
   tty_printf (gettext ("  %d = I trust marginally\n"), 3);
 if(min_num<=4)
   tty_printf (gettext ("  %d = I trust fully\n"), 4);
        if (mode)
          tty_printf (gettext ("  %d = I trust ultimately\n"), 5);




        if( mode )
          tty_printf(gettext ("  m = back to the main menu\n"));
        else
   {
     tty_printf(gettext ("  s = skip this key\n"));
     tty_printf(gettext ("  q = quit\n"));
   }
        tty_printf("\n");
 if(minimum)
   tty_printf(gettext ("The minimum trust level for this key is: %s\n\n"),
       trust_value_to_string(minimum));
        did_help = 1;
      }
    if( strlen(ans) != 8 )
      g10_log_bug0( "pkclist.c" , 300, __FUNCTION__ );
    p = cpr_get("edit_ownertrust.value",gettext ("Your decision? "));
    trim_spaces(p);
    cpr_kill_prompt();
    if( !*p )
      did_help = 0;
    ...
    else if( !p[1] && ((*p >= '0'+min_num) && *p <= (mode?'5':'4')) )
      {
        ...
        switch( *p )
          {
          case '1': trust = 2; break;
          case '2': trust = 3 ; break;
          case '3': trust = 4 ; break;
          case '4': trust = 5 ; break;
          case '5': trust = 6 ; break;
          default: g10_log_bug0( "pkclist.c" , 318, __FUNCTION__ );
          }
        if (trust == 6
            && !cpr_get_answer_is_yes ("edit_ownertrust.set_ultimate.okay",
                                       gettext ("Do you really want to set this key" " to ultimate trust? (y/N) ")))
...
# 341 "pkclist.c"
    else if( mode && (*p == ans[2] || *p == ans[3] || *p == ('D' - 'A' + 1) ) )
      {
        break ;
      }
...
do_we_trust( PKT_public_key *pk, unsigned int trustlevel )
{
  ...
  if(trustlevel & 32
     || trustlevel & 64
     || (trustlevel & 15) == 1)
    g10_log_bug0( "pkclist.c" , 404, __FUNCTION__ );

  if( opt.trust_model==TM_ALWAYS )
    {
      if( opt.verbose )
 g10_log_info("No trust check due to `--trust-model always' option\n");
      return 1;
    }

  switch(trustlevel & 15)
    {
    default:
      g10_log_error ("invalid trustlevel %u returned from validation layer\n",
   trustlevel);

    case 0:
    case 2:
      g10_log_info(gettext ("%s: There is no assurance this key belongs" " to the named user\n"),keystr_from_pk(pk));

      return 0;

    case 4:
      g10_log_info(gettext ("%s: There is limited assurance this key belongs" " to the named user\n"),keystr_from_pk(pk));

      return 1;

    case 5:
      if( opt.verbose )
 g10_log_info(gettext ("This key probably belongs to the named user\n"));
      return 1;

    case 6:
      if( opt.verbose )
 g10_log_info(gettext ("This key belongs to us\n"));
      return 1;
    }
...
do_we_trust_pre( PKT_public_key *pk, unsigned int trustlevel )
{
  ...
  if( !opt.batch && !rc )
    {
      print_pubkey_info(((void*)0),pk);
      print_fingerprint (pk, ((void*)0), 2);
      tty_printf("\n");

      tty_printf(
        gettext ("It is NOT certain that the key belongs to the person named\n" "in the user ID.  If you *really* know what you are doing,\n" "you may answer the next question with yes.\n"));
    ...
    gettext ("Use this key anyway? (y/N) ")) )
 rc = 1;
...
check_signatures_trust( PKT_signature *sig )
{
  ...
  if (rc)
    {
      g10_log_error("Ooops; the key vanished  - can't check the trust\n");
      rc = 9;
      goto leave;
    }
  ...
  if ( opt.trust_model==TM_ALWAYS )
    {
      if( !opt.quiet )
        g10_log_info(gettext ("WARNING: Using untrusted key!\n"));
      if (opt.with_fingerprint)
        print_fingerprint (pk, ((void*)0), 1);
      goto leave;
    }

  if(pk->maybe_revoked && !pk->is_revoked)
    g10_log_info(gettext ("WARNING: this key might be revoked (revocation key" " not present)\n"));


  trustlevel = get_validity (pk, ((void*)0));

  if ( (trustlevel & 32) )
    {
      write_status( 10 );
      if(pk->is_revoked==2)
 g10_log_info(gettext ("WARNING: This key has been revoked by its" " designated revoker!\n"));

      else
 g10_log_info(gettext ("WARNING: This key has been revoked by its owner!\n"));
      g10_log_info(gettext ("         This could mean that the signature is forged.\n"));
      show_revocation_reason( pk, 0 );
    }
  else if ((trustlevel & 64) )
    {
      write_status( 10 );
      g10_log_info(gettext ("WARNING: This subkey has been revoked by its owner!\n"));
      show_revocation_reason( pk, 0 );
    }

  if ((trustlevel & 128))
    g10_log_info (gettext ("Note: This key has been disabled.\n"));
  ...
  if (sig->pka_info && sig->pka_info->valid)
    {
      ...
      if ( fprlen == 20 && !memcmp (sig->pka_info->fpr, fpr, 20) )
        {
          okay = 1;
          write_status_text (83, sig->pka_info->email);
          g10_log_info (gettext ("Note: Verified signer's address is `%s'\n"),
                    sig->pka_info->email);
        }
      else
        {
          okay = 0;
          write_status_text (82, sig->pka_info->email);
          g10_log_info (gettext ("Note: Signer's address `%s' " "does not match DNS entry\n"), sig->pka_info->email);
      ...
      switch ( (trustlevel & 15) )
        ...
        case 2:
        case 4:
          if (okay && opt.verify_options&(1<<8))
            {
              trustlevel = ((trustlevel & ~15) | 5);
              g10_log_info (gettext ("trustlevel adjusted to FULL" " due to valid PKA info\n"));
        ...
        case 5:
          if (!okay)
            {
              trustlevel = ((trustlevel & ~15) | 3);
              g10_log_info (gettext ("trustlevel adjusted to NEVER" " due to bad PKA info\n"));
  ...
  switch ( (trustlevel & 15) )
    {
    case 1:
      g10_log_info(gettext ("Note: This key has expired!\n"));
      print_fingerprint (pk, ((void*)0), 1);
      break;
    ...
    case 2:
      write_status( 11 );
      g10_log_info(gettext ("WARNING: This key is not certified with" " a trusted signature!\n"));

      g10_log_info(gettext ("         There is no indication that the " "signature belongs to the owner.\n"));
    ...
    case 3:
      ...
      write_status( 12 );
      g10_log_info(gettext ("WARNING: We do NOT trust this key!\n"));
      g10_log_info(gettext ("         The signature is probably a FORGERY.\n"));
      if (opt.with_fingerprint)
        print_fingerprint (pk, ((void*)0), 1);
      rc = 8;
      break;
    ...
    case 4:
      write_status( 13 );
      g10_log_info(gettext ("WARNING: This key is not certified with" " sufficiently trusted signatures!\n"));

      g10_log_info(gettext ("         It is not certain that the" " signature belongs to the owner.\n"));
...
default_recipient(void)
{
 ...
 return ((void*)0);
    ...
    i = get_seckey_byname( sk, ((void*)0), 0 );
    if( i ) {
 free_secret_key( sk );
 return ((void*)0);
    }
...
{
  STRLIST sl,output=((void*)0),rover;
...
build_pk_list( STRLIST rcpts, PK_LIST *ret_pk_list, unsigned int use )
{
  ...
  for ( rov = remusr; rov; rov = rov->next )
    {
      if ( !(rov->flags & 1) )
        {
          ...
          if ((rov->flags&2) && ((opt.compliance==CO_PGP2) || (opt.compliance==CO_PGP6) || (opt.compliance==CO_PGP7) || (opt.compliance==CO_PGP8)))
            {
              g10_log_info(gettext ("you may not use %s while in %s mode\n"),
                       "--hidden-recipient",
      ...
      else if ( (use & 2) && !opt.no_encrypt_to )
        {
          ...
          if ( (rc = get_pubkey_byname ( pk, rov->d, ((void*)0), ((void*)0), 1 )) )
            {
              free_public_key ( pk ); pk = ((void*)0);
              g10_log_error (gettext ("%s: skipped: %s\n"), rov->d, g10_errstr(rc) );
              write_status_text_and_buffer (61,
                                            (rc == 9
                                             || rc == 17)? "1 ":
                                            (rc == 35)? "14 ":
                                            "0 ",
              ...
              if (key_present_in_pk_list(pk_list, pk) == 0)
                {
                  free_public_key (pk); pk = ((void*)0);
                  if (!opt.quiet)
                    g10_log_info (gettext ("%s: skipped: public key already present\n"),
                              rov->d);
                }
              else
                {
                  ...
                  if ((r->flags&1) && ((opt.compliance==CO_PGP2) || (opt.compliance==CO_PGP6) || (opt.compliance==CO_PGP7) || (opt.compliance==CO_PGP8)))
                    {
                      g10_log_info(gettext ("you may not use %s while in %s mode\n"),
                               "--hidden-encrypt-to",
                               compliance_option_string());
          ...
          else
            {
              ...
              free_public_key( pk ); pk = ((void*)0);
              g10_log_error(gettext ("%s: skipped: %s\n"), rov->d, g10_errstr(rc) );
              write_status_text_and_buffer (61, "3 ",
                                            rov->d, strlen (rov->d), -1);
              goto fail;
            }
        }
  ...
  if ( !any_recipients && !opt.batch )
    {
      ...
      if ( !have_def_rec )
        tty_printf(gettext ("You did not specify a user ID. (you may use \"-r\")\n"));
      ...
      for (;;)
        {
          ...
          if ( have_def_rec )
          ...
          else
            {
              ...
              tty_printf("\n");
              tty_printf(gettext ("Current recipients:\n"));
          ...
          pk = xmalloc_clear( sizeof *pk );
          pk->req_usage = use;
          rc = get_pubkey_byname( pk, answer, ((void*)0), ((void*)0), 0 );
          if (rc)
            tty_printf(gettext ("No such user ID.\n"));
          else if ( !(rc=check_pubkey_algo2(pk->pubkey_algo, use)) )
            {
              if ( have_def_rec )
                {
                  ...
                  if (!key_present_in_pk_list(pk_list, pk))
                    {
                      free_public_key (pk); pk = ((void*)0);
                      g10_log_info (gettext ("skipped: public key " "already set as default recipient\n") );
              ...
              else
                {
                  ...
                  if ( (trustlevel & 128) )
                    {
                      tty_printf (gettext ("Public key is disabled.\n") );
                    }
                  else if ( do_we_trust_pre (pk, trustlevel) )
                    {
                      ...
                      if (!key_present_in_pk_list(pk_list, pk))
                        {
                          free_public_key(pk); pk = ((void*)0);
                          g10_log_info(gettext ("skipped: public key already set\n") );
                        }
                      else
                        {
                          PK_LIST r;
  ...
  else if ( !any_recipients && (def_rec = default_recipient()) )
    {
      ...
      rc = get_pubkey_byname (pk, def_rec, ((void*)0), ((void*)0), 1);
      if (rc)
        g10_log_error(gettext ("unknown default recipient \"%s\"\n"), def_rec );
      else if ( !(rc=check_pubkey_algo2(pk->pubkey_algo, use)) )
        {
      ...
      for (; remusr; remusr = remusr->next )
        {
          ...
          pk = xmalloc_clear( sizeof *pk );
          pk->req_usage = use;
          if ( (rc = get_pubkey_byname( pk, remusr->d, ((void*)0), ((void*)0), 0 )) )
            {
              ...
              if ( (trustlevel & 128) )
                {
                  ...
                  free_public_key(pk); pk = ((void*)0);
                  g10_log_info(gettext ("%s: skipped: public key is disabled\n"),
                           remusr->d);
                  write_status_text_and_buffer (61, "13 ",
                                                remusr->d,
                                                strlen (remusr->d),
                                                -1);
                  rc=53;
                ...
                {
                  free_public_key( pk ); pk = ((void*)0);
                  write_status_text_and_buffer (61, "10 ",
                                                remusr->d,
                                                strlen (remusr->d),
                                                -1);
                  rc=53;
  ...
  if ( !rc && !any_recipients )
    {
      g10_log_error(gettext ("no valid addressees\n"));
      write_status_text (62, "0");
      rc = 16;
    }
...
algo_available( preftype_t preftype, int algo, const union pref_hint *hint )
{
  if( preftype == PREFTYPE_SYM )
    {
      if((opt.compliance==CO_PGP6) && (algo != 1
    && algo != 2
    && algo != 3))
 return 0;

      if((opt.compliance==CO_PGP7) && (algo != 1
    && algo != 2
    && algo != 3
    && algo != 7
    && algo != 8
      ...
      if(hint && hint->digest_length)
 {
  ...
  return 0;
     ...
     return 0;
      ...
      if(((opt.compliance==CO_PGP6) || (opt.compliance==CO_PGP7)) && (algo != 1
       && algo != 2
       && algo != 3))
 return 0;


      if((opt.compliance==CO_PGP8) && (algo != 1
    && algo != 2
    && algo != 3
    && algo != 8))
 return 0;
    ...
    {
      if(((opt.compliance==CO_PGP6) || (opt.compliance==CO_PGP7)) && (algo != 0
       && algo != 1))
 return 0;
         ...
         int request, const union pref_hint *hint)
{
 ...
 case PREFTYPE_SYM:
   ...
   if((opt.compliance==CO_PGP2) && pkr->pk->version<4 && pkr->pk->selfsigversion<4)
     implicit=1;
   else
     implicit=2;
  ...
  if(result==-1)
    {
      ...
      if(preftype==PREFTYPE_HASH && scores[1])
 {
# 1431 "pkclist.c"
   for(i=1 +1;i<256;i++)
     if(scores[i])
