    ...
    PKT_signature *revsig,PKT_signature *revkey)
{
  ...
  if(rc)
    {
      g10_log_error(gettext ("build_packet failed: %s\n"), g10_errstr(rc) );
      return rc;
    }
      ...
      if(!node)
 {
   ...
   if(uid)
     ...
     {
       g10_log_error(gettext ("key %s has no user IDs\n"),keystr(keyid));
       return 1;
     }
 }
      ...
      while((signode=find_next_kbnode(signode,PKT_SIGNATURE)))
 {
   if(keyid[0]==signode->pkt->pkt.signature->keyid[0] &&
      ...
      (((signode->pkt->pkt.signature)->sig_class & ~3) == 0x10))
     {
...
gen_desig_revoke( const char *uname, STRLIST locusr )
{
 ...
 return 1;
    ...
    rc = desc.mode? keydb_search2((kdbhd),(&desc),(1),((void*)0)) : 35;
    if (rc) {
 ...
 goto leave;
    ...
    node = find_kbnode( keyblock, PKT_PUBLIC_KEY );
    if( !node )
      g10_log_bug0( "revoke.c" , 242, __FUNCTION__ );
 ...
 if(rc)
   goto leave;
    ...
    if(!pk->revkey && pk->numrevkeys)
      g10_log_bug0( "revoke.c" , 258, __FUNCTION__ );
 ...
 if(sk_list)
  ...
  if(memcmp(fpr,pk->revkey[i].fpr,20)==0)
    break;
     ...
     if(list)
       sk=copy_secret_key(((void*)0),list->sk);
     else
       continue;
   }
 else
   {
     ...
     rc=get_seckey_byfprint(sk,pk->revkey[i].fpr,20);
   }


 if(!rc)
   {
     PKT_signature *revkey = ((void*)0);
     ...
     tty_printf (gettext ("To be revoked by:\n"));
            print_seckey_info (sk);

     if(pk->revkey[i].class&0x40)
       tty_printf(gettext ("(This is a sensitive revocation key)\n"));
     tty_printf("\n");

     if( !cpr_get_answer_is_yes("gen_desig_revoke.okay",
         gettext ("Create a designated revocation certificate for this key? (y/N) ")))
       continue;
     ...
     if( !opt.armor )
       tty_printf(gettext ("ASCII armored output forced.\n"));

     if( (rc = open_outfile( ((void*)0), 0, &out )) )
       goto leave;
     ...
     rc = make_keysig_packet( &sig, pk, ((void*)0), ((void*)0), sk, 0x20, 0,
         0, 0, 0,
         revocation_reason_build_cb, reason );
     if( rc ) {
       g10_log_error(gettext ("make_keysig_packet failed: %s\n"), g10_errstr(rc));
       goto leave;
     }
# 364 "revoke.c"
     while(!revkey)
  ...
  if(keyid[0]==signode->pkt->pkt.signature->keyid[0] &&
     keyid[1]==signode->pkt->pkt.signature->keyid[1] &&
     ((signode->pkt->pkt.signature)->sig_class == 0x1f))
    {
      int j;
   ...
   if(pk->revkey[i].class==
     ...
     if(!revkey)
       g10_log_bug0( "revoke.c" , 398, __FUNCTION__ );
     ...
     tty_printf(gettext ("Revocation certificate created.\n"));
     break;
   }
      }

    if(!any)
      g10_log_error(gettext ("no revocation keys found for \"%s\"\n"),uname);
...
gen_revoke( const char *uname )
{
    ...
    u32 sk_keyid[2];
    IOBUF out = ((void*)0);
    KBNODE keyblock = ((void*)0), pub_keyblock = ((void*)0);
    KBNODE node;
    KEYDB_HANDLE kdbhd;
    struct revocation_reason_info *reason = ((void*)0);
    KEYDB_SEARCH_DESC desc;
 ...
 return 1;
    ...
    if (rc)
      {
 g10_log_error (gettext ("secret key \"%s\" not found: %s\n"),
                   uname, g10_errstr (rc));
 goto leave;
      }
 ...
 goto leave;
    ...
    if( !node )
 g10_log_bug0( "revoke.c" , 483, __FUNCTION__ );
    ...
    if(!pub_keyblock)
      {
 g10_log_error(gettext ("no corresponding public key: %s\n"), g10_errstr(rc) );
 goto leave;
      }

    node=find_kbnode(pub_keyblock,PKT_PUBLIC_KEY);
    if(!node)
      g10_log_bug0( "revoke.c" , 502, __FUNCTION__ );

    pk=node->pkt->pkt.public_key;

    if( cmp_public_secret_key( pk, sk ) ) {
 g10_log_error(gettext ("public key does not match secret key!\n") );
 rc = 1;
 goto leave;
    }
    ...
    gettext ("Create a revocation certificate for this key? (y/N) ")) )
      {
 rc = 0;
 goto leave;
      }
 ...
 goto leave;
    ...
    switch( is_secret_key_protected( sk ) ) {
      case -1:
 g10_log_error(gettext ("unknown protection algorithm\n"));
 rc = 4;
 break;
      case -3:
 tty_printf (gettext ("Secret parts of primary key are not available.\n"));
        ...
        break;
      case 0:
 tty_printf(gettext ("NOTE: This key is not protected!\n"));
 break;
      default:
        rc = check_secret_key( sk, 0 );
 break;
 ...
 goto leave;
    ...
    if((opt.compliance==CO_PGP2) || (opt.compliance==CO_PGP6) || (opt.compliance==CO_PGP7) || (opt.compliance==CO_PGP8))
      {


 rc=export_minimal_pk(out,pub_keyblock,sig,((void*)0));
 if(rc)
   goto leave;
      }
    else
...
{
    int code=-1;
    char *description = ((void*)0);
    struct revocation_reason_info *reason;
    const char *text_0 = gettext ("No reason specified");
    const char *text_1 = gettext ("Key has been compromised");
    const char *text_2 = gettext ("Key is superseded");
    const char *text_3 = gettext ("Key is no longer used");
    const char *text_4 = gettext ("User ID is no longer valid");
    const char *code_text = ((void*)0);
 ...
 tty_printf(gettext ("Please select the reason for the revocation:\n"));
 tty_printf( "  0 = %s\n", text_0 );
 if( key_rev )
     tty_printf("  1 = %s\n", text_1 );
 if( key_rev )
 ...
 tty_printf( "  Q = %s\n", gettext ("Cancel") );
 if( hint )
     tty_printf(gettext ("(Probably you want to select %d here)\n"), hint );

 while(code==-1) {
     int n;
     char *answer = cpr_get("ask_revocation_reason.code",
      gettext ("Your decision? "));
     trim_spaces( answer );
     cpr_kill_prompt();
 ...
 tty_printf(gettext ("Enter an optional description; " "end it with an empty line:\n") );
 ...
 tty_printf(gettext ("Reason for revocation: %s\n"), code_text );
 if( !description )
     tty_printf(gettext ("(No description given)\n") );
 else
     tty_printf("%s\n", description );

    } while( !cpr_get_answer_is_yes("ask_revocation_reason.okay",
         gettext ("Is this okay? (y/N) ")) );
