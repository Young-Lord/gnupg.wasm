...
# 94 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/bits/alltypes.h" 3
typedef struct {
    union {
        int __i[10];
...
# 299 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/bits/alltypes.h" 3
typedef int clock_t;
...
# 110 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/bits/alltypes.h" 3
...
# 335 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/bits/alltypes.h" 3
...
# 364 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/bits/alltypes.h" 3
...
enum {
  ...
  SW_TERM_STATE = 0x6285,
  SW_EEPROM_FAILURE = 0x6581,
  SW_WRONG_LENGTH = 0x6700,
  SW_SM_NOT_SUP = 0x6882,
  SW_CC_NOT_SUP = 0x6884,
  SW_CHV_WRONG = 0x6982,
  SW_CHV_BLOCKED = 0x6983,
  SW_USE_CONDITIONS = 0x6985,
  SW_BAD_PARAMETER = 0x6a80,
...
static struct reader_table_s reader_table[4];
...
new_reader_slot (void)
{
  ...
  if (reader == -1)
    {
      g10_log_error ("new_reader_slot: out of slots\n");
      return -1;
    }
# 349 "apdu.c"
  reader_table[reader].connect_card = ((void*)0);
  reader_table[reader].disconnect_card = ((void*)0);
  reader_table[reader].close_reader = ((void*)0);
  reader_table[reader].shutdown_reader = ((void*)0);
  reader_table[reader].reset_reader = ((void*)0);
  reader_table[reader].get_status_reader = ((void*)0);
  reader_table[reader].send_apdu_reader = ((void*)0);
  reader_table[reader].check_keypad = ((void*)0);
  reader_table[reader].dump_status_reader = ((void*)0);
  reader_table[reader].set_progress_cb = ((void*)0);
...
ct_activate_card (int slot)
{
  ...
  if (rc || buflen < 2 || buf[buflen-2] != 0x90)
    {
      g10_log_error ("ct_activate_card: can't get status of reader %d: %s\n",
  ...
  if (rc || buflen < 2 || buf[buflen-2] != 0x90)
    {
      g10_log_error ("ct_activate_card(%d): activation failed: %s\n",
                 slot, ct_error_string (rc));
      if (!rc)
  ...
  if (buflen - 2 > (sizeof(reader_table[0].atr)/sizeof((reader_table[0].atr)[0])))
    {
      g10_log_error ("ct_activate_card(%d): ATR too long\n", slot);
      return SW_HOST_CARD_IO_ERROR;
    }
...
open_ct_reader (int port)
{
  ...
  if (port < 0 || port > 0xffff)
    {
      g10_log_error ("open_ct_reader: invalid port %d requested\n", port);
      return -1;
    }
  ...
  if (rc)
    {
      g10_log_error ("apdu_open_ct_reader failed on port %d: %s\n",
...
pcsc_error_to_sw (long ec)
{
  ...
  switch ( ((ec) & 0xffffffff) )
    {
    case 0: rc = 0; break;

    case 0x80100002: rc = SW_HOST_ABORTED; break;
    case 0x80100006: rc = SW_HOST_OUT_OF_CORE; break;
    case 0x8010000A: rc = SW_HOST_CARD_IO_ERROR; break;
    case 0x8010000B: rc = SW_HOST_LOCKING_FAILED; break;
    case 0x8010000C: rc = SW_HOST_NO_CARD; break;
    case 0x80100069: rc = SW_HOST_NO_CARD; break;

    case 0x80100005:
    case 0x80100011:
    case 0x80100003:
    case 0x80100004:
    case 0x80100008: rc = SW_HOST_INV_VALUE; break;
...
dump_pcsc_reader_status (int slot)
{
  if (reader_table[slot].pcsc.card)
    {
      g10_log_info ("reader slot %d: active protocol:", slot);
      if ((reader_table[slot].pcsc.protocol & 1))
        log_printf (" T0");
      else if ((reader_table[slot].pcsc.protocol & 2))
        log_printf (" T1");
      else if ((reader_table[slot].pcsc.protocol & 4))
        log_printf (" raw");
      log_printf ("\n");
    }
  else
    g10_log_info ("reader slot %d: not connected\n", slot);
}
...
pcsc_get_status_direct (int slot, unsigned int *status)
{
  ...
  rdrstates[0].current_state = 0x0000;
  ...
  if (err)
    {
      g10_log_error ("pcsc_get_status_change failed: %s (0x%lx)\n",
                 pcsc_error_string (err), err);
      return pcsc_error_to_sw (err);
    }
# 852 "apdu.c"
  *status = 0;
  if ( (rdrstates[0].event_state & 0x0020) )
    *status |= (2);
  if ( !(rdrstates[0].event_state & 0x0200) )
    *status |= (4);
  ...
  if ( (*status & ((2)|(4)))
       == ((2)|(4))
       && !(rdrstates[0].event_state & 0x0100) )
    *status |= (1);
# 873 "apdu.c"
  return 0;
}
                       ...
                       struct pininfo_s *pininfo)
{
  ...
  if ((reader_table[slot].pcsc.protocol & 2))
      send_pci.protocol = 2;
  else
      send_pci.protocol = 1;
  ...
  err = pcsc_transmit (reader_table[slot].pcsc.card,
                       ...
                       ((void*)0), buffer, &recv_len);
  *buflen = recv_len;
  if (err)
    g10_log_error ("pcsc_transmit failed: %s (0x%lx)\n",
               pcsc_error_string (err), err);
...
close_pcsc_reader_direct (int slot)
{
  ...
  reader_table[slot].rdrname = ((void*)0);
  reader_table[slot].used = 0;
  return 0;
}
# 1247 "apdu.c"
...
connect_pcsc_card (int slot)
{
  ...
  ((void)((slot >= 0 && slot < 4) || (__assert_fail("slot >= 0 && slot < MAX_READER", "apdu.c", 1265, __func__),0)));
  ...
  if (err)
    {
      ...
      if (err != 0x8010000C)
        g10_log_error ("pcsc_connect failed: %s (0x%lx)\n",
                   pcsc_error_string (err), err);
    }
  else
    {
      ...
      atrlen = (sizeof(reader_table[0].atr)/sizeof((reader_table[0].atr)[0]));
      ...
      if (err)
        g10_log_error ("pcsc_status failed: %s (0x%lx) %lu\n",
                   pcsc_error_string (err),
                   (unsigned long)err, (unsigned long)readerlen);
      else
        {
          if (atrlen > (sizeof(reader_table[0].atr)/sizeof((reader_table[0].atr)[0])))
            g10_log_bug ("ATR returned by pcsc_status is too large\n");
          reader_table[slot].atrlen = atrlen;


          reader_table[slot].last_status = ( (1)
                                             | (2)
                                             | (4));
          reader_table[slot].is_t0 = !!(card_protocol & 1);
        }
    }
...
disconnect_pcsc_card (int slot)
{
  ...
  ((void)((slot >= 0 && slot < 4) || (__assert_fail("slot >= 0 && slot < MAX_READER", "apdu.c", 1331, __func__),0)));

  if (!reader_table[slot].pcsc.card)
    return 0;

  err = pcsc_disconnect (reader_table[slot].pcsc.card, 0);
  if (err)
    {
      g10_log_error ("pcsc_disconnect failed: %s (0x%lx)\n",
                 pcsc_error_string (err), err);
      return SW_HOST_CARD_IO_ERROR;
    }
  reader_table[slot].pcsc.card = 0;
...
open_pcsc_reader_direct (const char *portstr)
{
  ...
  err = pcsc_establish_context (2, ((void*)0), ((void*)0),
                                &reader_table[slot].pcsc.context);
  if (err)
    {
      g10_log_error ("pcsc_establish_context failed: %s (0x%lx)\n",
                 pcsc_error_string (err), err);
      reader_table[slot].used = 0;
      return -1;
    }
  ...
  err = pcsc_list_readers (reader_table[slot].pcsc.context,
                           ((void*)0), ((void*)0), &nreader);
  if (!err)
    {
      list = xmalloc((nreader+1));
      if (!list)
        {
          g10_log_error ("error allocating memory for reader list\n");
          pcsc_release_context (reader_table[slot].pcsc.context);
          reader_table[slot].used = 0;
          return -1 ;
        }
  ...
  if (err)
    {
      g10_log_error ("pcsc_list_readers failed: %s (0x%lx)\n",
                 pcsc_error_string (err), err);
      pcsc_release_context (reader_table[slot].pcsc.context);
      reader_table[slot].used = 0;
      xfree (list);
  ...
  while (nreader)
    {
      if (!*p && !p[1])
        break;
      if (*p)
        g10_log_info ("detected reader `%s'\n", p);
      if (nreader < (strlen (p)+1))
        {
          g10_log_error ("invalid response from pcsc_list_readers\n");
          break;
        }
      nreader -= strlen (p)+1;
      p += strlen (p) + 1;
  ...
  reader_table[slot].rdrname = xmalloc((strlen (portstr? portstr : list)+1));
  if (!reader_table[slot].rdrname)
    {
      g10_log_error ("error allocating memory for reader name\n");
      pcsc_release_context (reader_table[slot].pcsc.context);
      reader_table[slot].used = 0;
      return -1;
    }
      ...
      if (!ct_api_loaded)
        {
          ...
          handle = dlopen (opt.ctapi_driver, 1);
          if (!handle)
            {
              g10_log_error ("apdu_open_reader: failed to open driver: %s\n",
                         dlerror ());
              return -1;
            }
          ...
          if (!CT_init || !CT_data || !CT_close)
            {
              g10_log_error ("apdu_open_reader: invalid CT-API driver\n");
              dlclose (handle);
              return -1;
            }
          ct_api_loaded = 1;
  ...
  if (!pcsc_api_loaded)
    {
      ...
      handle = dlopen (opt.pcsc_driver, 1);
      if (!handle)
        {
          g10_log_error ("apdu_open_reader: failed to open driver `%s': %s\n",
      ...
      if (!pcsc_establish_context
                                    ...
                                    )
        {
          ...
          g10_log_error ("apdu_open_reader: invalid PC/SC driver "
...
apdu_close_reader (int slot)
{
  ...
  if (slot < 0 || slot >= 4 || !reader_table[slot].used )
...
apdu_connect (int slot)
{
  ...
  apdu_get_status_internal (slot, 1, 1, ((void*)0), ((void*)0));
...
apdu_get_atr (int slot, size_t *atrlen)
{
  ...
  if (!reader_table[slot].atrlen)
    return ((void*)0);
  buf = xmalloc((reader_table[slot].atrlen));
  if (!buf)
    return ((void*)0);
         ...
         struct pininfo_s *pininfo, int extended_mode)
{
  ...
  unsigned char short_result_buffer[258 +10];
  unsigned char *result_buffer = ((void*)0);
  size_t result_buffer_size;
  unsigned char *result;
  size_t resultlen;
  unsigned char short_apdu_buffer[5+256+1];
  unsigned char *apdu_buffer = ((void*)0);
  ...
  if ((opt.debug & 2048))
    g10_log_debug ("send apdu: c=%02X i=%02X p1=%02X p2=%02X lc=%d le=%d em=%d\n",
  ...
  if (use_extended_length)
    {
      ...
      apdu_buffer = xmalloc((apdu_buffer_size + 10));
      if (!apdu_buffer)
  ...
  if (use_extended_length && (le > 256 || le < 0))
    {
      ...
      result_buffer = xmalloc((result_buffer_size + 10));
      if (!result_buffer)
        {
          xfree (apdu_buffer);
          return SW_HOST_OUT_OF_CORE;
  ...
  do
      ...
      if (use_extended_length)
      ...
      else
        {
          ...
          if (use_chaining && lc > 255)
            {
              apdu[apdulen] |= 0x10;
              ((void)((use_chaining < 256) || (__assert_fail("use_chaining < 256", "apdu.c", 3047, __func__),0)));
    ...
    exact_length_hack:
      ...
      ((void)((apdulen <= apdu_buffer_size) || (__assert_fail("apdulen <= apdu_buffer_size", "apdu.c", 3077, __func__),0)));
      memset (apdu+apdulen, 0, apdu_buffer_size - apdulen);
      resultlen = result_buffer_size;
      rc = send_apdu (slot, apdu, apdulen, result, &resultlen, pininfo);
      if (rc || resultlen < 2)
        {
          g10_log_info ("apdu_send_simple(%d) failed: %s\n",
      ...
      if (!use_extended_length
          && !did_exact_length_hack && (((sw)&~0xff) == SW_EXACT_LENGTH))
        {
  ...
  if ((opt.debug & 2048))
    {
      g10_log_debug (" response: sw=%04X  datalen=%d\n",
  ...
  if (sw == SW_SUCCESS || sw == SW_EOF_REACHED)
    {
      if (retbuf)
        {
          *retbuf = xmalloc((resultlen? resultlen : 1));
          if (!*retbuf)
            {
              unlock_slot (slot);
              xfree (result_buffer);
      ...
      if (retbuf)
        {
          *retbuf = p = xmalloc((bufsize));
          ...
          ((void)((resultlen < bufsize) || (__assert_fail("resultlen < bufsize", "apdu.c", 3149, __func__),0)));
      ...
      do
        {
          ...
          if ((opt.debug & 2048))
            g10_log_debug ("apdu_send_simple(%d): %d more bytes available\n",
                       slot, len);
          apdu_buffer_size = sizeof short_apdu_buffer;
          apdu = short_apdu_buffer;
          apdulen = 0;
          ...
          apdu[apdulen++] = 0;
          apdu[apdulen++] = len;
          ((void)((apdulen <= apdu_buffer_size) || (__assert_fail("apdulen <= apdu_buffer_size", "apdu.c", 3169, __func__),0)));
          memset (apdu+apdulen, 0, apdu_buffer_size - apdulen);
          resultlen = result_buffer_size;
          rc = send_apdu (slot, apdu, apdulen, result, &resultlen, ((void*)0));
          if (rc || resultlen < 2)
            {
              g10_log_error ("apdu_send_simple(%d) for get response failed: %s\n",
                         slot, apdu_strerror (rc));
              unlock_slot (slot);
              xfree (result_buffer);
              return rc? rc : SW_HOST_INCOMPLETE_CARD_RESPONSE;
          ...
          if ((opt.debug & 2048))
            {
              g10_log_debug ("     more: sw=%04X  datalen=%d\n",
                         sw, (unsigned int)resultlen);
              if (!retbuf && (sw==SW_SUCCESS || (sw&0xff00)==SW_MORE_DATA))
                log_printhex ("     dump: ", result, resultlen);
            }
          ...
          if ((sw & 0xff00) == SW_MORE_DATA
            ...
            g10_log_info ("apdu_send_simple(%d) "
  ...
  if ((opt.debug & 2048) && retbuf && sw == SW_SUCCESS)
           ...
           int lc, const char *data, unsigned char **retbuf, size_t *retbuflen)
{
  return send_le (slot, class, ins, p0, p1, lc, data, 256,
                  retbuf, retbuflen, ((void*)0), extended_mode);
}
# 3288 "apdu.c"
int
apdu_send_simple (int slot, int extended_mode,
                  ...
                  int lc, const char *data)
{
  return send_le (slot, class, ins, p0, p1, lc, data, -1, ((void*)0), ((void*)0), ((void*)0),
                  extended_mode);
}
                  ...
                  unsigned char **retbuf, size_t *retbuflen)
{
  ...
  if (apdudatalen > sizeof short_apdu_buffer - 5)
    {
      apdu_buffer = xmalloc((apdudatalen + 5));
      if (!apdu_buffer)
        return SW_HOST_OUT_OF_CORE;
      apdu = apdu_buffer;
    }
  ...
  if (rc || resultlen < 2)
    {
      g10_log_error ("apdu_send_direct(%d) failed: %s\n",
                 slot, apdu_strerror (rc));
      unlock_slot (slot);
      xfree (result_buffer);
      return rc? rc : SW_HOST_INCOMPLETE_CARD_RESPONSE;
  ...
  if (handle_more && (sw & 0xff00) == SW_MORE_DATA)
    {
      ...
      if (retbuf)
        {
          *retbuf = p = xmalloc((bufsize + 2));
          ...
          ((void)((resultlen < bufsize) || (__assert_fail("resultlen < bufsize", "apdu.c", 3431, __func__),0)));
          memcpy (p, result, resultlen);
          p += resultlen;
        }
      ...
      do
        {
          ...
          if ((opt.debug & 2048))
            g10_log_debug ("apdu_send_direct(%d): %d more bytes available\n",
                       slot, len);
          apdu = short_apdu_buffer;
          apdulen = 0;
          apdu[apdulen++] = class;
          ...
          if (rc || resultlen < 2)
            {
              g10_log_error ("apdu_send_direct(%d) for get response failed: %s\n",
                         slot, apdu_strerror (rc));
              unlock_slot (slot);
              xfree (result_buffer);
              return rc ? rc : SW_HOST_INCOMPLETE_CARD_RESPONSE;
          ...
          if ((sw & 0xff00) == SW_MORE_DATA
            ...
            g10_log_info ("apdu_send_direct(%d) "
                      "got unexpected status %04X from get response\n",
                      slot, sw);
        }
      while ((sw & 0xff00) == SW_MORE_DATA);
      ...
      if (retbuf)
        {
          *retbuflen = p - *retbuf;
          tmp = xrealloc((*retbuf),(*retbuflen + 2));
          if (tmp)
            *retbuf = tmp;
        }
    }
  else
    {
      if (retbuf)
        {
          *retbuf = xmalloc(((resultlen? resultlen : 1)+2));
          if (!*retbuf)
            {
              unlock_slot (slot);
              xfree (result_buffer);
