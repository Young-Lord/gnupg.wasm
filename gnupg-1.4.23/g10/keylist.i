  ...
  for (node = keyblock; node; node = node->next )
    {
      if (node->pkt->pkttype == PKT_SECRET_KEY
          || (node->pkt->pkttype == PKT_SECRET_SUBKEY) )
        {
         ...
         pubkey_letter (sk->pubkey_algo),
         keystr_from_sk(sk));
          tty_fprintf (fp, gettext ("created: %s"), datestr_from_sk (sk));
          tty_fprintf (fp, "  ");
          tty_fprintf (fp, gettext ("expires: %s"), expirestr_from_sk (sk));
          if (sk->is_protected && sk->protect.s2k.mode == 1002)
            {
              tty_fprintf (fp, "\n                      ");
              tty_fprintf (fp, gettext ("card-no: "));
...
status_one_subpacket(sigsubpkttype_t type,size_t len,int flags,const byte *buf)
{
  ...
  write_status_text_and_buffer(77,status,buf,len,0);
}
...
show_policy_url(PKT_signature *sig,int indent,int mode)
{
      ...
      if(mode!=2)
 {
   ...
   if(crit)
     str=gettext ("Critical signature policy: ");
   else
     str=gettext ("Signature policy: ");
   if(mode)
     g10_log_info("%s",str);
   else
     printf("%s",str);
   ...
   fprintf(fp,"\n");
      ...
      if(mode)
 write_status_buffer ( 55, p, len, 0 );
    }
}
...
show_keyserver_url(PKT_signature *sig,int indent,int mode)
{
      ...
      if(mode!=2)
 {
   ...
   if(crit)
     str=gettext ("Critical preferred keyserver: ");
   else
     str=gettext ("Preferred keyserver: ");
   if(mode)
     g10_log_info("%s",str);
   else
     printf("%s",str);
...
show_notation(PKT_signature *sig,int indent,int mode,int which)
{
      ...
      if(mode!=2)
 {
  ...
  str=gettext ("Critical signature notation: ");
       else
  str=gettext ("Signature notation: ");
       if(mode)
  g10_log_info("%s",str);
       else
  printf("%s",str);
      ...
      if(mode)
 {
   write_status_buffer(53,
         nd->name,strlen(nd->name),0);
   write_status_buffer(54,
...
list_all( int secret )
{
    ...
    if( rc ) {
 if( rc != -1 )
     g10_log_error("keydb_search_first failed: %s\n", g10_errstr(rc) );
 goto leave;
    }
    ...
    do {
        ...
        if (rc) {
            g10_log_error ("keydb_get_keyblock failed: %s\n", g10_errstr(rc));
            goto leave;
        }
 if(!opt.with_colons)
   {
 ...
 list_keyblock( keyblock, secret, opt.fingerprint,
         opt.check_sigs?&stats:((void*)0));
 release_kbnode( keyblock );
        keyblock = ((void*)0);
    } while (!(rc = keydb_search_next (hd)));
    if( rc && rc != -1 )
 g10_log_error ("keydb_search_next failed: %s\n", g10_errstr(rc));
...
list_one( STRLIST names, int secret )
{
    ...
    GETKEY_CTX ctx;
    const char *resname;
    const char *keyring_str = gettext ("Keyring");
    int i;
    struct sig_stats stats;
...
# 486 "keylist.c"
    if( secret ) {
 rc = get_seckey_bynames( &ctx, ((void*)0), names, &keyblock );
 if( rc ) {
     g10_log_error("error reading key: %s\n", g10_errstr(rc) );
     get_seckey_end( ctx );
     return;
 }
 do {
     if ((opt.list_options&(1<<8)) && !opt.with_colons) {
  ...
  putchar('\n');
     ...
     list_keyblock( keyblock, 1, opt.fingerprint, ((void*)0) );
     release_kbnode( keyblock );
 } while( !get_seckey_next( ctx, ((void*)0), &keyblock ) );
 get_seckey_end( ctx );
    }
    else {
 rc = get_pubkey_bynames( &ctx, ((void*)0), names, &keyblock );
 if( rc ) {
     g10_log_error("error reading key: %s\n", g10_errstr(rc) );
     get_pubkey_end( ctx );
     return;
 ...
 do {
  ...
  putchar('\n');
     ...
     list_keyblock( keyblock, 0, opt.fingerprint,
      opt.check_sigs?&stats:((void*)0) );
     release_kbnode( keyblock );
 } while( !get_pubkey_next( ctx, ((void*)0), &keyblock ) );
 get_pubkey_end( ctx );
    }
...
print_capabilities (PKT_public_key *pk, PKT_secret_key *sk, KBNODE keyblock)
{
      ...
      if ( use & 1 )
 {
  ...
  if(pk->is_primary)
    disabled=(((pk)->is_disabled)?((pk)->is_disabled==2):(cache_disabled_value((pk))));

                if ( pk->is_valid && !pk->is_revoked && !pk->has_expired ) {
                    if ( pk->pubkey_usage & 2 )
                        enc = 1;
                    if ( pk->pubkey_usage & 1 )
        {
   sign = 1;
   if(pk->is_primary)
     cert = 1;
        }
                    if ( pk->pubkey_usage & 4 )
                      cert = 1;
                    if ( (pk->pubkey_usage & 8) )
                      auth = 1;
                }
            }
       ...
       && sk->protect.s2k.mode!=1001 ) {
                    if ( sk->pubkey_usage & 2 )
                        enc = 1;
                    if ( sk->pubkey_usage & 1 )
        {
   sign = 1;
   if(sk->is_primary)
     cert = 1;
        }
                    if ( (sk->pubkey_usage & 4) )
                        cert = 1;
                    if ( (sk->pubkey_usage & 8) )
                        auth = 1;
                }
            }
        }
...
print_subpackets_colon(PKT_signature *sig)
{
  ...
  ((void)((opt.show_subpackets) || (__assert_fail("opt.show_subpackets", "keylist.c", 665, __func__),0)));
...
dump_attribs(const PKT_user_id *uid,PKT_public_key *pk,PKT_secret_key *sk)
{
      ...
      if(is_status_enabled())
 {
   ...
   if(pk)
     ...
     g10_log_bug0( "keylist.c" , 706, __FUNCTION__ );
...
list_keyblock_print ( KBNODE keyblock, int secret, int fpr, void *opaque )
{
 ...
 if(sk->has_expired)
   {
     printf(" [");
     printf(gettext ("expired: %s"),expirestr_from_sk(sk));
     printf("]");
   }
 else if(sk->expiredate )
   {
     printf(" [");
     printf(gettext ("expires: %s"),expirestr_from_sk(sk));
     printf("]");
   }
 ...
 if(pk->is_revoked)
   {
     printf(" [");
     printf(gettext ("revoked: %s"),revokestr_from_pk(pk));
     printf("]");
   }
 else if(pk->has_expired)
   {
     printf(" [");
     printf(gettext ("expired: %s"),expirestr_from_pk(pk));
     printf("]");
   }
 else if(pk->expiredate)
   {
     printf(" [");
     printf(gettext ("expires: %s"),expirestr_from_pk(pk));
     printf("]");
   }
# 813 "keylist.c"
 printf("\n");
 ...
 if( node->pkt->pkttype == PKT_USER_ID && !opt.fast_list_mode ) {
  ...
  continue;
     ...
     if(attrib_fp && uid->attrib_data!=((void*)0))
       dump_attribs(uid,pk,sk);

     if((uid->is_revoked || uid->is_expired)
        || ((opt.list_options&(1<<5)) && pk))
       {
  const char *validity;
  int indent;

  validity=uid_trust_string_fixed(pk,uid);
  indent=(keystrlen()+9)-atoi(uid_trust_string_fixed(((void*)0),((void*)0)));
  ...
  printf("uid%*s%s ",indent,"",validity);
     ...
     if((opt.list_options&(1<<0)) && uid->attribs!=((void*)0))
  ...
  continue;
     ...
     if( pk2->is_revoked )
       {
  printf(" [");
  printf(gettext ("revoked: %s"),revokestr_from_pk(pk2));
  printf("]");
       }
     else if( pk2->has_expired )
       {
  printf(" [");
  printf(gettext ("expired: %s"),expirestr_from_pk(pk2));
  printf("]");
       }
     else if( pk2->expiredate )
       {
  printf(" [");
  printf(gettext ("expires: %s"),expirestr_from_pk(pk2));
  printf("]");
       }
            putchar('\n');
     if( fpr > 1 )
       print_fingerprint( pk2, ((void*)0), 0 );
     if( opt.with_key_data )
       print_key_data( pk2 );
   }
 else if( node->pkt->pkttype == PKT_SECRET_SUBKEY )
   {
     ...
     keystr_from_sk(sk2),datestr_from_sk( sk2 ) );
       ...
       {
  printf(" [");
  printf(gettext ("expires: %s"),expirestr_from_sk(sk2));
  printf("]");
       }
     putchar('\n');
     if( fpr > 1 )
              {
                print_fingerprint( ((void*)0), sk2, 0 );
  ...
  switch( rc ) {
   case 0: sigrc = '!'; break;
   case 8: stats->inv_sigs++; sigrc = '-'; break;
   case 9:
   case 53: stats->no_key++; continue;
...
print_revokers(PKT_public_key *pk)
{
  ...
  if( !pk->revkey && pk->numrevkeys )
    g10_log_bug0( "keylist.c" , 1016, __FUNCTION__ );
  else
    {
      int i,j;
...
list_keyblock_colon( KBNODE keyblock, int secret, int fpr )
{
 ...
 keyid_from_pk( pk, keyid );
        ...
        if ( !pk->is_valid )
            ...
            trustletter = get_validity_info ( pk, ((void*)0) );
            if( trustletter == 'u' )
                ulti_hack = 1;
            putchar(trustletter);
        }
 ...
 if( node->pkt->pkttype == PKT_USER_ID && !opt.fast_list_mode ) {
     PKT_user_id *uid=node->pkt->pkt.user_id;
     if(attrib_fp && node->pkt->pkt.user_id->attrib_data!=((void*)0))
  ...
  any = 1;
   ...
   colon_strtime (pk2->expiredate)
            ...
            print_capabilities (pk2, ((void*)0), ((void*)0));
            putchar('\n');
     if( fpr > 1 )
  print_fingerprint( pk2, ((void*)0), 0 );
     if( opt.with_key_data )
  ...
  any = 1;
   ...
   colon_strtime (sk2->expiredate)
            ...
            print_capabilities (((void*)0), sk2, ((void*)0));
            if (opt.fixed_list_mode) {
  ...
  continue;
     ...
     if( opt.check_sigs ) {
         PKT_public_key *signer_pk=((void*)0);

  fflush((stdout));
  if(opt.no_sig_cache)
    signer_pk=xmalloc_clear(sizeof(PKT_public_key));

  rc = check_key_signature2( keyblock, node, ((void*)0), signer_pk,
...
{
    KBNODE primary = ((void*)0), primary0 = ((void*)0), primary2 = ((void*)0);
    KBNODE last, node;
 ...
 if( node->pkt->pkttype == PKT_USER_ID &&
    ...
    for (last=((void*)0), node=keyblock; node; last = node, node = node->next) {
 if( node->pkt->pkttype == PKT_USER_ID )
            break;
    }
    ((void)((node) || (__assert_fail("node", "keylist.c", 1430, __func__),0)));
    ((void)((last) || (__assert_fail("last", "keylist.c", 1431, __func__),0)));
    ((void)((primary0) || (__assert_fail("primary0", "keylist.c", 1432, __func__),0)));
    if ( node == primary )
        return;
 ...
 if(pk->main_keyid[0]==pk->keyid[0] && pk->main_keyid[1]==pk->keyid[1])
   primary=1;
    ...
    if(mode&0x80 && !primary)
      {
 g10_log_error("primary key is not really primary!\n");
 return;
      }
 ...
 if(sk)
   {
     ...
     print_fingerprint(((void*)0),primary_sk,mode|0x80);
 ...
 else
   {
     ...
     print_fingerprint(primary_pk,((void*)0),mode|0x80);
    ...
    if (mode == 1) {
        fp = log_stream ();
 if(primary)
   text = gettext ("Primary key fingerprint:");
 else
   text = gettext ("     Subkey fingerprint:");
    }
    else if (mode == 2) {
        fp = ((void*)0);
 if(primary)
...
print_card_serialno (PKT_secret_key *sk)
{
  ...
  fputs (gettext ("      Card serial no. ="), (stdout));
  putchar (' ');
  if (sk->protect.ivlen == 16
      && !memcmp (sk->protect.iv, "\xD2\x76\x00\x01\x24\x01", 6) )
    {
...
void set_attrib_fd(int fd)
{
  ...
  if( !attrib_fp ) {
    g10_log_fatal("can't open fd %d for attribute output: %s\n",
