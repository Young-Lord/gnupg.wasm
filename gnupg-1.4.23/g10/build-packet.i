...
build_packet( IOBUF out, PACKET *pkt )
{
    ...
    if( (opt.debug & 1) )
 g10_log_debug("build_packet() type=%d\n", pkt->pkttype );
    ((void)((pkt->pkt.generic) || (__assert_fail("pkt->pkt.generic", "build-packet.c", 75, __func__),0)));
 ...
 break;
      case PKT_MDC:
      default:
 g10_log_bug("invalid packet type in build_packet()\n");
 break;
      }
...
calc_packet_length( PACKET *pkt )
{
    ...
    ((void)((pkt->pkt.generic) || (__assert_fail("pkt->pkt.generic", "build-packet.c", 162, __func__),0)));
    switch( pkt->pkttype ) {
      case PKT_PLAINTEXT:
 ...
 break;
      ...
      case PKT_COMPRESSED:
      default:
 g10_log_bug("invalid packet type in calc_packet_length()");
 break;
    }
...
do_public_key( IOBUF out, int ctb, PKT_public_key *pk )
{
 ...
 write_16(a, ndays );
    ...
    iobuf_writebyte(a,pk->pubkey_algo);
 ...
 mpi_write(a, pk->pkey[i] );
    ...
    write_header2(out, ctb, ( (a)->d.len ), pk->hdrbytes);
...
do_secret_key( IOBUF out, int ctb, PKT_secret_key *sk )
{
 ...
 write_16(a, ndays);
    ...
    iobuf_writebyte(a,sk->pubkey_algo);
 ...
 goto leave;
    ...
    ((void)((npkey < nskey) || (__assert_fail("npkey < nskey", "build-packet.c", 299, __func__),0)));
 ...
 mpi_write(a, sk->skey[i] );
    ...
    if( sk->is_protected ) {
 if( ((sk->pubkey_algo)==1 || (sk->pubkey_algo)==2 || (sk->pubkey_algo)==3 ) && sk->version < 4
        && !sk->protect.s2k.mode ) {

     iobuf_writebyte(a,sk->protect.algo);
     iobuf_write(a, sk->protect.iv, sk->protect.ivlen );
 }
 else {

     iobuf_writebyte(a,sk->protect.sha1chk? 0xfe : 0xff);
     iobuf_writebyte(a,sk->protect.algo);
  ...
  iobuf_writebyte(a,101);
  iobuf_writebyte(a,sk->protect.s2k.hash_algo);
  iobuf_write(a, "GNU", 3 );
  iobuf_writebyte(a,sk->protect.s2k.mode - 1000);
     }
     else {
  iobuf_writebyte(a,sk->protect.s2k.mode);
  iobuf_writebyte(a,sk->protect.s2k.hash_algo);
     }
  ...
  iobuf_writebyte(a,sk->protect.s2k.count);
 ...
 iobuf_writebyte(a,0);
    ...
    if( sk->protect.s2k.mode == 1001 )
      ...
      {
 iobuf_writebyte(a,sk->protect.ivlen);
 ...
 ((void)((((sk->skey[npkey]) && (mpi_get_flags (sk->skey[npkey])&4))) || (__assert_fail("mpi_is_opaque( sk->skey[npkey] )", "build-packet.c", 362, __func__),0)));
 ...
 for( ; i < nskey; i++ ) {
            ...
            ((void)((((sk->skey[i]) && (mpi_get_flags (sk->skey[i])&4))) || (__assert_fail("mpi_is_opaque (sk->skey[i])", "build-packet.c", 373, __func__),0)));
 ...
 write_16(a, sk->csum );
  ...
  leave:
    ...
    write_header2(out, ctb, ( (a)->d.len ), sk->hdrbytes);
...
do_symkey_enc( IOBUF out, int ctb, PKT_symkey_enc *enc )
{
    ...
    ((void)((enc->version == 4) || (__assert_fail("enc->version == 4", "build-packet.c", 405, __func__),0)));
    switch( enc->s2k.mode ) {
      case 0: case 1: case 3: break;
      default: g10_log_bug("do_symkey_enc: s2k=%d\n", enc->s2k.mode );
    }
    iobuf_writebyte(a,enc->version);
    iobuf_writebyte(a,enc->cipher_algo);
    iobuf_writebyte(a,enc->s2k.mode);
    iobuf_writebyte(a,enc->s2k.hash_algo);
    if( enc->s2k.mode == 1 || enc->s2k.mode == 3 ) {
 iobuf_write(a, enc->s2k.salt, 8 );
 if( enc->s2k.mode == 3 )
     iobuf_writebyte(a,enc->s2k.count);
    }
    if( enc->seskeylen )
 iobuf_write(a, enc->seskey, enc->seskeylen );

    write_header(out, ctb, ( (a)->d.len ) );
    if( iobuf_write_temp( out, a ) )
 rc = 22;
...
do_pubkey_enc( IOBUF out, int ctb, PKT_pubkey_enc *enc )
{
 ...
 write_32(a, enc->keyid[1] );
    ...
    iobuf_writebyte(a,enc->pubkey_algo);
...
do_plaintext( IOBUF out, int ctb, PKT_plaintext *pt )
{
    ...
    iobuf_writebyte(out,pt->mode);
    iobuf_writebyte(out,pt->namelen);
    for(i=0; i < pt->namelen; i++ )
 iobuf_writebyte(out,pt->name[i]);
 ...
 n += nbytes;
    ...
    do { volatile char *_vptr=(volatile char *)(buf); size_t _vlen=(1000); while(_vlen) { *_vptr=(0); _vptr++; _vlen--; } } while(0);
    if( (ctb&0x40) && !pt->len )
      iobuf_set_partial_block_mode(out, 0 );




    if( pt->len && n != pt->len )
      g10_log_error("do_plaintext(): wrote %lu bytes but expected %lu bytes\n",
...
do_encrypted_mdc( IOBUF out, int ctb, PKT_encrypted *ed )
{
    ...
    ((void)((ed->mdc_method) || (__assert_fail("ed->mdc_method", "build-packet.c", 537, __func__),0)));


    n = ed->len ? (ed->len + ed->extralen + 1 + 22) : 0;
    write_header(out, ctb, n );
    iobuf_writebyte(out,1);
...
do_compressed( IOBUF out, int ctb, PKT_compressed *cd )
{
    ...
    iobuf_writebyte(out,cd->algorithm);
...
delete_sig_subpkt (subpktarea_t *area, sigsubpkttype_t reqtype )
{
 ...
 if( type == reqtype ) {
  ...
  break;
    ...
    if (!okay)
        g10_log_error ("delete_subpkt: buffer shorter than subpacket\n");
    ((void)((unused <= area->len) || (__assert_fail("unused <= area->len", "build-packet.c", 631, __func__),0)));
    area->len -= unused;
    return !!unused;
}
# 646 "build-packet.c"
    ...
    const byte *buffer, size_t buflen )
{
    ...
    if(parse_one_sig_subpkt(buffer,buflen,type)<0)
      g10_log_bug0( "build-packet.c" , 660, __FUNCTION__ );
...
string_to_notation(const char *string,int is_utf8)
{
 ...
 break;
      ...
      if( !*s || !(0 ? isascii(*s) : (unsigned)(*s) < 128) || (!(0 ? isgraph(*s) : ((unsigned)(*s)-0x21) < 0x5e) && !__isspace(*s)) )
 {
   g10_log_error(gettext ("a notation name must have only printable characters" " or spaces, and end with an '='\n") );
  ...
  if(!saw_at && !opt.expert)
    {
      g10_log_error(gettext ("a user notation name must contain the '@' character\n"));
      goto fail;
    }

  if (saw_at > 1)
    {
      g10_log_error(gettext ("a notation name must not contain more than" " one '@' character\n"));
  ...
  if(*s)
    {
      ...
      for(s++; *s ; s++ )
 {
   if ( !(0 ? isascii(*s) : (unsigned)(*s) < 128) )
     highbit=1;
   else if (iscntrl(*s))
     {
       g10_log_error(gettext ("a notation value must not use any" " control characters\n"));
...
sig_to_notation(PKT_signature *sig)
{
  ...
  size_t len;
  int seq=0,crit;
  struct notation *list=((void*)0);

  while((p=enum_sig_subpkt(sig->hashed,SIGSUBPKT_NOTATION,&len,&seq,&crit)))
    {
      int n1,n2;
      struct notation *n=((void*)0);

      if(len<8)
 {
   g10_log_info(gettext ("WARNING: invalid notation data found\n"));
   continue;
 }
      ...
      else
 {
   ...
   n->value=xmalloc(2+strlen(gettext ("not human readable"))+2+1);
   strcpy(n->value,"[ ");
   strcat(n->value,gettext ("not human readable"));
   strcat(n->value," ]");
 }
...
do_signature( IOBUF out, int ctb, PKT_signature *sig )
{
 ...
 iobuf_writebyte(a,sig->version);
    if( sig->version < 4 )
 iobuf_writebyte(a,5);
    iobuf_writebyte(a,sig->sig_class);
    if( sig->version < 4 ) {
 write_32(a, sig->timestamp );
 write_32(a, sig->keyid[0] );
 write_32(a, sig->keyid[1] );
    }
    iobuf_writebyte(a,sig->pubkey_algo);
    iobuf_writebyte(a,sig->digest_algo);
    if( sig->version >= 4 ) {
 size_t nn;
 ...
 if( nn )
    ...
    iobuf_writebyte(a,sig->digest_start[0]);
    iobuf_writebyte(a,sig->digest_start[1]);
 ...
 mpi_write(a, sig->data[i] );
    ...
    if( ((sig->pubkey_algo)==1 || (sig->pubkey_algo)==2 || (sig->pubkey_algo)==3 ) && sig->version < 4 )
 write_sign_packet_header(out, ctb, ( (a)->d.len ) );
    else
 write_header(out, ctb, ( (a)->d.len ) );
    if( iobuf_write_temp( out, a ) )
 rc = 22;
...
do_onepass_sig( IOBUF out, int ctb, PKT_onepass_sig *ops )
{
    ...
    write_version( a, ctb );
    iobuf_writebyte(a,ops->sig_class);
    iobuf_writebyte(a,ops->digest_algo);
    iobuf_writebyte(a,ops->pubkey_algo);
    write_32(a, ops->keyid[0] );
    write_32(a, ops->keyid[1] );
    iobuf_writebyte(a,ops->last);
...
{
    iobuf_writebyte(out,a>>8);
    if( iobuf_writebyte(out,a) )
 return -1;
    return 0;
}
...
{
    iobuf_writebyte(out,a>> 24);
    iobuf_writebyte(out,a>> 16);
    iobuf_writebyte(out,a>> 8);
    if( iobuf_writebyte(out,a) )
 return -1;
    return 0;
...
write_sign_packet_header( IOBUF out, int ctb, u32 len )
{
    ...
    iobuf_writebyte(out,0x89);
    iobuf_writebyte(out,len >> 8);
    return iobuf_writebyte(out,len) == -1 ? -1:0;
}
# 1224 "build-packet.c"
static int
  ...
  if( len || hdrlen )
    {
      if( ctb & 2 )
 {
   if(iobuf_writebyte(out,len >> 24))
     return -1;
   if(iobuf_writebyte(out,len >> 16))
     return -1;
 }

      if( ctb & 3 )
 if(iobuf_writebyte(out,len >> 8))
   return -1;

      if( iobuf_writebyte(out,len) )
 return -1;
    }
...
{
    if( hdrlen )
 g10_log_bug("can't cope with hdrlen yet\n");
 ...
 if( len < 192 ) {
  ...
  return -1;
     ...
     len -= 192;
     if( iobuf_writebyte(out,(len / 256) + 192) )
  return -1;
     if( iobuf_writebyte(out,(len % 256)) )
  return -1;
 }
 else {
     if( iobuf_writebyte(out,0xff) )
  return -1;
     if( iobuf_writebyte(out,(len >> 24)&0xff) )
  return -1;
     if( iobuf_writebyte(out,(len >> 16)&0xff) )
  return -1;
     if( iobuf_writebyte(out,(len >> 8)&0xff) )
  return -1;
     if( iobuf_writebyte(out,len & 0xff) )
  return -1;
 }
    }
    return 0;
