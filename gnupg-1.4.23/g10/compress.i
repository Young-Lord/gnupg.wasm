...
# 62 "compress.c"
 ...
 g10_log_error("invalid compression level; using default level\n");
 level = (-1);
    }

    if( (rc = zfx->algo == 1? deflateInit2_((zs),(level),(8),(-13),(8), (0), "1.1.4", sizeof(z_stream))

       : deflateInit_((zs), (level), "1.1.4", sizeof(z_stream))
       ) != 0 ) {
 g10_log_fatal("zlib problem: %s\n", zs->msg? zs->msg :
          rc == (-4) ? "out of core" :
          rc == (-6) ? "invalid lib version" :
             "unknown error" );
    }
...
do_compress( compress_filter_context_t *zfx, z_stream *zs, int flush, IOBUF a )
{
 ...
 if( (opt.debug & 8) )
     g10_log_debug("enter deflate: avail_in=%u, avail_out=%u, flush=%d\n",
 ...
 if( zrc == 1 && flush == 4 )
     ...
     if( zs->msg )
  g10_log_fatal("zlib deflate problem: %s\n", zs->msg );
     else
  g10_log_fatal("zlib deflate problem: rc=%d\n", zrc );
 }
 n = zfx->outbufsize - zs->avail_out;
 if( (opt.debug & 8) )
     g10_log_debug("leave deflate: "
        "avail_in=%u, avail_out=%u, n=%u, zrc=%d\n",
  (unsigned)zs->avail_in, (unsigned)zs->avail_out,
            (unsigned)n, zrc );

 if( iobuf_write( a, zfx->outbuf, n ) ) {
     g10_log_debug("deflate: iobuf_write failed\n");
     return 22;
 }
    } while( zs->avail_in || (flush == 4 && zrc != 1) );
    return 0;
}
...
init_uncompress( compress_filter_context_t *zfx, z_stream *zs )
{
    int rc;
# 140 "compress.c"
    if( (rc = zfx->algo == 1? inflateInit2_((zs), (-15), "1.1.4", sizeof(z_stream))
       : inflateInit_((zs), "1.1.4", sizeof(z_stream))) != 0 ) {
 g10_log_fatal("zlib problem: %s\n", zs->msg? zs->msg :
          rc == (-4) ? "out of core" :
          rc == (-6) ? "invalid lib version" :
             "unknown error" );
        ...
        IOBUF a, size_t *ret_len )
{
    ...
    if( (opt.debug & 8) )
 g10_log_debug("begin inflate: avail_in=%u, avail_out=%u, inbuf=%u\n",
  (unsigned)zs->avail_in, (unsigned)zs->avail_out,
 ...
 refill = 1;
 if( (opt.debug & 8) )
     g10_log_debug("enter inflate: avail_in=%u, avail_out=%u\n",
      (unsigned)zs->avail_in, (unsigned)zs->avail_out);
 ...
 if( (opt.debug & 8) )
     g10_log_debug("leave inflate: avail_in=%u, avail_out=%u, zrc=%d\n",
     (unsigned)zs->avail_in, (unsigned)zs->avail_out, zrc);
 if( zrc == 1 )
     rc = -1;
 else if( zrc != 0 && zrc != (-5) ) {
     if( zs->msg )
  g10_log_fatal("zlib inflate problem: %s\n", zs->msg );
     else
  g10_log_fatal("zlib inflate problem: rc=%d\n", zrc );
 }
    } while (zs->avail_out && zrc != 1 && zrc != (-5)
             && !leave);
    *ret_len = zfx->outbufsize - zs->avail_out;
    if( (opt.debug & 8) )
 g10_log_debug("do_uncompress: returning %u bytes\n", (unsigned)*ret_len );
    return rc;
}
   ...
   IOBUF a, byte *buf, size_t *ret_len)
{
 ...
 if( !zfx->status ) {
     ...
     if(zfx->algo != 1
        && zfx->algo != 2)
       g10_log_bug0( "compress.c" , 252, __FUNCTION__ );
  ...
  g10_log_bug("build_packet(PKT_COMPRESSED) failed\n");
     zs = zfx->opaque = xmalloc_clear( sizeof *zs );
 ...
 if( zfx->status == 1 ) {
     ...
     xfree(zs);
     zfx->opaque = ((void*)0);
     xfree(zfx->outbuf); zfx->outbuf = ((void*)0);
 }
        ...
        int algo,int rel)
{
  ...
  switch(zfx->algo)
    ...
    default:
      g10_log_bug0( "compress.c" , 368, __FUNCTION__ );
    }
}
