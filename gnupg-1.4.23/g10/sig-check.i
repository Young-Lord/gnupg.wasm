...
{
    return signature_check2( sig, digest, ((void*)0), ((void*)0), ((void*)0), ((void*)0) );
}
    ...
    int *r_expired, int *r_revoked, PKT_public_key *ret_pk )
{
    ...
    if( (rc=check_digest_algo(sig->digest_algo)) )
    ...
    else if(!md_algo_present(digest,sig->digest_algo))
      {
        ...
        g10_log_info(gettext ("WARNING: signature digest conflict in message\n"));
 rc=1;
      }
 ...
 if(rc==0 && !pk->is_primary && pk->backsig<2)
   {
     if(pk->backsig==0)
       {
  g10_log_info(gettext ("WARNING: signing subkey %s is not" " cross-certified\n"),keystr_from_pk(pk));

  g10_log_info(gettext ("please see %s for more information\n"),
  ...
  if(opt.flags.require_cross_cert)
    rc=1;
       ...
       {
  g10_log_info(gettext ("WARNING: signing subkey %s has an invalid" " cross-certification\n"),keystr_from_pk(pk));
...
# 140 "sig-check.c"
 ...
 for(i=0; i < nsig; i++ ) {
     ...
     p = mpi_get_buffer( sig->data[i], &n, ((void*)0) );
     md_write( md, p, n );
     xfree(p);
 }
 md_final( md );
     ...
     int *r_expired, int *r_revoked )
{
 ...
 g10_log_info(d==1
   ?gettext ("public key %s is %lu second newer than the signature\n")
   :gettext ("public key %s is %lu seconds newer than the signature\n"),
   keystr_from_pk(pk),d );
 ...
 g10_log_info( d==1
    ? gettext ("key %s was created %lu second" " in the future (time warp or clock problem)\n")

    : gettext ("key %s was created %lu seconds" " in the future (time warp or clock problem)\n"),
 ...
 if( !opt.ignore_time_conflict )
   return 40;
    ...
    if (pk->has_expired || (pk->expiredate && pk->expiredate < cur_time)) {
        char buf[11];
        if (opt.verbose)
   g10_log_info(gettext ("NOTE: signature key %s expired %s\n"),
     keystr_from_pk(pk), asctimestamp( pk->expiredate ) );

 sprintf(buf,"%lu",(ulong)pk->expiredate);
 write_status_text(9,buf);
 write_status(64);
 if(r_expired)
   *r_expired = 1;
    }
   ...
   int *r_expired, int *r_revoked, PKT_public_key *ret_pk )
{
        ...
        if (sig->digest_algo == weak->algo)
          {
            if (!weak->rejection_shown)
              {
                g10_log_info
                  (gettext ("Note: signatures using the %s algorithm are rejected\n"),
    ...
    if( sig->version >= 4 )
 do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = (sig->version) & 0xff; } while(0);
    do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = (sig->sig_class) & 0xff; } while(0);
    if( sig->version < 4 ) {
 u32 a = sig->timestamp;
 do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = ((a >> 24) & 0xff) & 0xff; } while(0);
 do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = ((a >> 16) & 0xff) & 0xff; } while(0);
 do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = ((a >> 8) & 0xff) & 0xff; } while(0);
 do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = (a & 0xff) & 0xff; } while(0);
    }
    else {
 byte buf[6];
 size_t n;
 do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = (sig->pubkey_algo) & 0xff; } while(0);
 do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = (sig->digest_algo) & 0xff; } while(0);
 if( sig->hashed ) {
     n = sig->hashed->len;
            do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = ((n >> 8)) & 0xff; } while(0);
            do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = (n) & 0xff; } while(0);
     ...
     n += 6;
          ...
          do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = (0) & 0xff; } while(0);
   do { if( (digest)->bufcount == (digest)->bufsize ) md_write( (digest), ((void*)0), 0 ); (digest)->buffer[(digest)->bufcount++] = (0) & 0xff; } while(0);
   n = 6;
 }
 ...
 md_write( digest, buf, 6 );
    ...
    result = encode_md_value( pk, ((void*)0), digest, sig->digest_algo );
    if (!result)
        return 1;
    rc = pubkey_verify( pk->pubkey_algo, result, sig->data, pk->pkey );
    mpi_free( result );

    if(rc==8 && ((pk->pubkey_algo)==1 || (pk->pubkey_algo)==2 || (pk->pubkey_algo)==3 )
       && sig->digest_algo==11)
      {
# 326 "sig-check.c"
 ...
 result=pkcs1_encode_md(digest,11,28,
          mpi_get_nbits(pk->pkey[0]),asn,(sizeof(asn)/sizeof((asn)[0])));
 ...
 mpi_free(result);
    ...
    if (sig->digest_algo == 1)
      md5_digest_warn (0);

    if( !rc && sig->flags.unknown_critical )
      {
 g10_log_info(gettext ("assuming bad signature from key %s" " due to an unknown critical bit\n"),keystr_from_pk(pk));
...
hash_uid_node( KBNODE unode, MD_HANDLE md, PKT_signature *sig )
{
    ...
    ((void)((unode->pkt->pkttype == PKT_USER_ID) || (__assert_fail("unode->pkt->pkttype == PKT_USER_ID", "sig-check.c", 362, __func__),0)));
    if( uid->attrib_data ) {
 if( sig->version >=4 ) {
     byte buf[5];
     buf[0] = 0xd1;
...
check_revocation_keys(PKT_public_key *pk,PKT_signature *sig)
{
  ...
  ((void)((((sig)->sig_class == 0x20)) || (__assert_fail("IS_KEY_REV(sig)", "sig-check.c", 428, __func__),0)));
  ((void)(((sig->keyid[0]!=pk->keyid[0]) || (sig->keyid[0]!=pk->keyid[1])) || (__assert_fail("(sig->keyid[0]!=pk->keyid[0]) || (sig->keyid[0]!=pk->keyid[1])", "sig-check.c", 429, __func__),0)));
  ...
  if( !pk->revkey && pk->numrevkeys )
     g10_log_bug0( "sig-check.c" , 448, __FUNCTION__ );
  else
      for(i=0;i<pk->numrevkeys;i++)
 {
          u32 keyid[2];

          keyid_from_fingerprint(pk->revkey[i].fpr,20,keyid);
       ...
       PKT_signature *backsig)
{
 ...
 return rc;
      ...
      return backsig->flags.valid? 0 : 8;
    }

  md=md_open(backsig->digest_algo,0);
  hash_public_key(md,main_pk);
  hash_public_key(md,sub_pk);
  rc=do_check(sub_pk,backsig,md,((void*)0),((void*)0),((void*)0));
...
{
  return check_key_signature2(root, node, ((void*)0), ((void*)0), is_selfsig, ((void*)0), ((void*)0) );
}
 ...
 *is_selfsig = 0;
    ...
    if( r_expired )
        *r_expired = 0;
    ((void)((node->pkt->pkttype == PKT_SIGNATURE) || (__assert_fail("node->pkt->pkttype == PKT_SIGNATURE", "sig-check.c", 540, __func__),0)));
    ((void)((root->pkt->pkttype == PKT_PUBLIC_KEY) || (__assert_fail("root->pkt->pkttype == PKT_PUBLIC_KEY", "sig-check.c", 541, __func__),0)));
  ...
  if( keyid[0] == sig->keyid[0] && keyid[1] == sig->keyid[1] )
     ...
     if((rc=do_check_messages(pk,sig,r_expired,((void*)0))))
       return rc;
            return sig->flags.valid? 0 : 8;
        }
    }
 ...
 keyid_from_pk( pk, keyid );
 ...
 else
   {
     md = md_open( algo, 0 );
     hash_public_key( md, pk );
     rc = do_check( pk, sig, md, r_expired, ((void*)0), ret_pk );
     cache_sig_result ( sig, rc );
     md_close(md);
   }
    }
 ...
 if( snode ) {
   ...
   {
            if (opt.verbose)
       g10_log_info (gettext ("key %s: no subkey for subkey" " revocation signature\n"),keystr_from_pk(pk));
 ...
 if( snode ) {
  ...
  if( keyid[0] == sig->keyid[0] && keyid[1] == sig->keyid[1] )
   ...
   {
            if (opt.verbose)
       g10_log_info(gettext ("key %s: no subkey for subkey" " binding signature\n"),keystr_from_pk(pk));
 ...
 if( unode ) {
  ...
  rc = do_check( pk, sig, md, r_expired, ((void*)0), ret_pk );
       ...
       rc=do_check(check_pk,sig,md,r_expired,((void*)0),ret_pk);
     else
       rc=signature_check2(sig,md,r_expiredate,r_expired,((void*)0),ret_pk);
   ...
   {
            if (!opt.quiet)
       g10_log_info ("key %s: no user ID for key signature packet"
