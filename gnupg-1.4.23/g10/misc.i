...
disable_core_dumps()
{
    ...
    limit.rlim_cur = 0;
    limit.rlim_max = 0;
    if( !setrlimit( 4, &limit ) )
 return 0;
    if( (*__errno_location()) != (28) && (*__errno_location()) != (52) )
 g10_log_fatal(gettext ("can't disable core dumps: %s\n"), strerror((*__errno_location())) );
...
checksum_mpi( MPI a )
{
    ...
    buffer = mpi_get_buffer( a, &nbytes, ((void*)0) );
...
print_pubkey_algo_note( int algo )
{
 ...
 {
   warn=1;
   g10_log_info(gettext ("WARNING: using experimental public key algorithm %s\n"),
     pubkey_algo_to_string(algo));
 }
    }
  else if (algo == 20)
    {
      g10_log_info (gettext ("WARNING: Elgamal sign+encrypt keys are deprecated\n"));
    }
}
...
print_cipher_algo_note( int algo )
{
 ...
 {
   warn=1;
   g10_log_info(gettext ("WARNING: using experimental cipher algorithm %s\n"),
...
print_digest_algo_note( int algo )
{
 ...
 {
   warn=1;
   g10_log_info(gettext ("WARNING: using experimental digest algorithm %s\n"),
     digest_algo_to_string(algo));
      ...
      for (weak = opt.weak_digests; weak; weak = weak->next)
        if (weak->algo == algo)
          g10_log_info (gettext ("WARNING: digest algorithm %s is deprecated\n"),
                    digest_algo_to_string(algo));
}
...
openpgp_pk_test_algo( int algo, unsigned int usage_flags )
{
    ...
    if (!(opt.compliance==CO_RFC2440) && algo == 20)
        return 4;
    if( algo < 0 || algo > 110 )
 return 4;
...
not_in_gpg1_notice (void)
{
  ...
  if (!warned)
    {
      g10_log_info (gettext ("NOTE: This feature is not available in %s\n"), "GnuPG 1.x");
      g10_log_info (gettext ("please see %s for more information\n"),
     ...
     const char *option,const char *repl1,const char *repl2)
{
 ...
 repl1+=2;
      ...
      g10_log_info(gettext ("%s:%d: deprecated option \"%s\"\n"),
        configname,configlineno,option);
    }
  else
    g10_log_info(gettext ("WARNING: \"%s\" is a deprecated option\n"),option);

  g10_log_info(gettext ("please use \"%s%s\" instead\n"),repl1,repl2);
}
...
{
  g10_log_info(gettext ("WARNING: \"%s\" is a deprecated command - do not use it\n"),
           name);
}
...
string_to_compress_algo(const char *string)
{
  ...
  if(match_multistr(gettext ("uncompressed|none"),string))
    return 0;
...
compliance_failure(void)
{
  ...
  g10_log_info(gettext ("this message may not be usable by %s\n"),ver);
       ...
       struct parse_options *opts,int noisy)
{
 ...
 if(opts[i].help && maxlen<strlen(opts[i].name))
   maxlen=strlen(opts[i].name);
      ...
      for(i=0;opts[i].name;i++)
        if(opts[i].help)
   printf("%s%*s%s\n",opts[i].name,
   maxlen+2-(int)strlen(opts[i].name),"",gettext (opts[i].help));
      ...
      for(i=0;opts[i].name;i++)
 {
       ...
       if(toklen!=strlen(opts[i].name))
  {
   ...
   {
     if(noisy)
       g10_log_info(gettext ("ambiguous option `%s'\n"),otok);
     return 0;
   }
      }
  }
 ...
 {
   if(noisy)
     g10_log_info(gettext ("unknown option `%s'\n"),otok);
   return 0;
 }
    }
  ...
  while (*s)
    {
      if (*s == '%' && s[1] && s[2])
        {
          s++;
          *d = (((*(s) <= '9'? (*(s)- '0'): *(s) <= 'F'? (*(s)-'A'+10):(*(s)-'a'+10)) * 16) + (*((s)+1) <= '9'? (*((s)+1)- '0'): *((s)+1) <= 'F'? (*((s)+1)-'A'+10):(*((s)+1)-'a'+10)));
          if (!*d)
            *d = '\xff';
          d++;
          s += 2;
...
{
# 1266 "misc.c"
  return "/home/yang/wasm/gnupg-1.4.23/prefix/libexec/gnupg";
}
...
{
  struct weakhash *weak = ((void*)0);
  const int algo = string_to_digest_algo(digestname);

  if (algo == 0)
    {
      g10_log_error(gettext ("Unknown weak digest '%s'\n"), digestname);
      return;
    }


  for (weak = opt.weak_digests; weak != ((void*)0); weak = weak->next)
    if (algo == weak->algo)
      return;
