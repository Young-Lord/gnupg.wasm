...
get_session_key( PKT_pubkey_enc *k, DEK *dek )
{
    ...
    rc = check_pubkey_algo2 (k->pubkey_algo, 2);
    if( rc )
 goto leave;

    if( (k->keyid[0] || k->keyid[1]) && !opt.try_all_secrets ) {
 sk = xmalloc_clear( sizeof *sk );
 sk->pubkey_algo = k->pubkey_algo;
 if( !(rc = get_seckey( sk, k->keyid )) )
     rc = get_it( k, dek, sk, k->keyid );
    }
    else {
 void *enum_context = ((void*)0);
 u32 keyid[2];
 char *p;
 ...
 for(;;) {
  ...
  continue;
     ...
     g10_log_info(gettext ("anonymous recipient; trying secret key %s ...\n"),
                     keystr(keyid));
  ...
  xfree(p);
     ...
     rc = check_secret_key( sk, opt.try_all_secrets?1:-1 );
...
# 127 "pubkey-enc.c"
  if(rc)
    next_to_last_passphrase();
     ...
     if( !rc )
       {
  g10_log_info(gettext ("okay, we are the anonymous recipient.\n") );
  break;
       }
 }
 enum_secret_keys( &enum_context, ((void*)0), 0, 0 );
    }
  ...
  if (sk->is_protected && sk->protect.s2k.mode == 1002)
    {
      ...
      size_t rbuflen;
      char *snbuf;
      unsigned char *indata = ((void*)0);
      unsigned int indatalen;

      snbuf = serialno_and_fpr_from_sk (sk->protect.iv, sk->protect.ivlen, sk);

      indata = mpi_get_buffer (enc->data[0], &indatalen, ((void*)0));
      if (!indata)
        g10_log_bug0( "pubkey-enc.c" , 171, __FUNCTION__ );
  ...
  else
    {
      ...
      if( rc )
 goto leave;
      frame = mpi_get_buffer( plain_dek, &nframe, ((void*)0) );
      mpi_free( plain_dek ); plain_dek = ((void*)0);
    }
# 213 "pubkey-enc.c"
    if( g10c_debug_mode )
 g10_log_hexdump("DEK frame:", frame, nframe );
    n=0;
    if (!card)
      {
        if( n + 7 > nframe )
        ...
        if( frame[n] == 1 && frame[nframe-1] == 2 ) {
          g10_log_info(gettext ("old encoding of the DEK is not supported\n"));
          rc = 12;
          goto leave;
        }
        if( frame[n] != 2 )
 ...
 { rc = 18; goto leave; }
    ...
    if( rc ) {
 if( !opt.quiet && rc == 12 ) {
     g10_log_info(gettext ("cipher algorithm %d%s is unknown or disabled\n"),
                     dek->algo, dek->algo == 1? " (IDEA)":"");
 }
 dek->algo = 0;
 goto leave;
    }
 ...
 goto leave;
    ...
    if( g10c_debug_mode )
 g10_log_hexdump("DEK is:", dek->key, dek->keylen );
 ...
 if( !pkb ) {
            rc = -1;
     g10_log_error("oops: public key not found for preference check\n");
        }
 else if(pkb->pkt->pkt.public_key->selfsigversion > 3
  && dek->algo != 2
  && !opt.quiet
  && !is_algo_in_prefs( pkb, PREFTYPE_SYM, dek->algo ))
   g10_log_info(gettext ("WARNING: cipher algorithm %s not found in recipient" " preferences\n"),cipher_algo_to_string(dek->algo));
        ...
        if (!rc) {
            ...
            if (!pk)
                g10_log_bug0( "pubkey-enc.c" , 294, __FUNCTION__ );
            if ( pk->expiredate && pk->expiredate <= make_timestamp() ) {
                g10_log_info(gettext ("NOTE: secret key %s expired at %s\n"),
                         keystr(keyid), asctimestamp( pk->expiredate) );
            }
        }

        if ( pk && pk->is_revoked ) {
            g10_log_info( gettext ("NOTE: key has been revoked") );
            putc( '\n', log_stream() );
...
get_override_session_key( DEK *dek, const char *string )
{
 ...
 return 20;
    s++;
    for(i=0; i < (sizeof(dek->key)/sizeof((dek->key)[0])) && *s; i++, s +=2 ) {
 int c = hextobyte ( s );
 if (c == -1)
     return 20;
 dek->key[i] = c;
