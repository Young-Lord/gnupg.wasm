...
send_status_info (ctrl_t ctrl, const char *keyword, ...)
{
  ...
  __builtin_va_start(arg_ptr, keyword);

  p = buf;
  n = 0;
  valuelen = strlen (keyword);
  for ( ; valuelen && n < (sizeof(buf)/sizeof((buf)[0]))-2; n++, valuelen--, keyword++)
    *p++ = *keyword;

  while ( (value = __builtin_va_arg(arg_ptr, const unsigned char *)) )
    {
      valuelen = __builtin_va_arg(arg_ptr, size_t);
      if (!valuelen)
        continue;
      if (n)
        {
      ...
      for ( ; valuelen && n < (sizeof(buf)/sizeof((buf)[0]))-2; n++, valuelen--, value++)
        {
          if (*value < ' ' || *value == '+')
            {
              sprintf (p, "%%%02X", *value);
...
send_status_direct (ctrl_t ctrl, const char *keyword, const char *args)
{
  ...
  if (strchr (args, '\n'))
    g10_log_error ("error: LF detected in status line - not sending\n");
  else
    {
                     ...
                     const void *buffer, size_t length)
{
  ...
  if (!h)
    g10_log_bug0( "cardglue.c" , 234, __FUNCTION__ );
...
app_set_default_reader_port (const char *portstr)
{
  ...
  default_reader_port = portstr? xstrdup (portstr): ((void*)0);
}
...
app_get_serial_and_stamp (app_t app, char **serial, time_t *stamp)
{
  ...
  *stamp = 0;
  ...
  buf = xmalloc((app->serialnolen * 2 + 1));
  if (!buf)
    return (1);
...
agent_release_card_info (struct agent_card_info_s *info)
{
  ...
  xfree (info->serialno); info->serialno = ((void*)0);
  xfree (info->apptype); info->apptype = ((void*)0);
  xfree (info->disp_name); info->disp_name = ((void*)0);
  xfree (info->disp_lang); info->disp_lang = ((void*)0);
  xfree (info->pubkey_url); info->pubkey_url = ((void*)0);
  xfree (info->login_data); info->login_data = ((void*)0);
  info->fpr1valid = info->fpr2valid = info->fpr3valid = 0;
  info->cafpr1valid = info->cafpr2valid = info->cafpr3valid = 0;
  for (i=0; i < 4; i++)
    {
      xfree (info->private_do[i]);
      info->private_do[i] = ((void*)0);
    }
}
...
test_transact (int rc, const char *command)
{
  if (!rc)
    return 0;
  g10_log_error ("sending command `%s' to agent failed: %s\n",
  ...
  if (rc)
    {
      if ((rc & 0xffff) == 60 || (rc & 0xffff) == 119)
        ...
        {
          write_status_text (71, "4");
          g10_log_info ("selecting openpgp failed: %s\n", assuan_strerror (rc));
          *scd_available = 1;
        }
 ...
 retry:
  if (did_shutdown)
  ...
  else
    {
      ...
      if (slot == -1)
        {
          write_status_text (71, "5");
          g10_log_error (gettext ("card reader not available\n"));
          return ((void*)0);
        }
    }
  ...
  if (rc)
    {
      write_status_text (71, "4");
      g10_log_info (gettext ("selecting openpgp failed: %s\n"), g10_errstr ((rc)));
      apdu_close_reader (slot);
      xfree (app);
      return ((void*)0);
    }
...
format_cacheid (const char *sn)
{
  ...
  if (sn)
    {
      for (s=sn,snlen=0; ((*(s) >= '0' && *(s) <= '9') || (*(s) >= 'A' && *(s) <= 'F') || (*(s) >= 'a' && *(s) <= 'f')); s++, snlen++)
        ;
      if (snlen == 32)
        {

          cacheid = xmalloc((7+snlen+1));
          if (cacheid)
            {
...
{
  if (rc == 56)
    write_status_text (79, "1");
  else if (rc == 11)
    write_status_text (79, "2");
  else if (rc)
    write_status (79);
}
# 584 "cardglue.c"
...
check_card_serialno (app_t app, const char *serialno)
{
  ...
  for (s = serialno, n=0; *s != '/' && ((*(s) >= '0' && *(s) <= '9') || (*(s) >= 'A' && *(s) <= 'F') || (*(s) >= 'a' && *(s) <= 'f')); s++, n++)
    ;
  if (n != 32)
    {
      g10_log_error ("invalid serial number in keyring detected\n");
      return (1);
    }
  if (app->serialnolen != 16)
    ask = 1;
  for (s = serialno, n=0; !ask && n < 16; s += 2, n++)
    if (app->serialno[n] != (((*(s) <= '9'? (*(s)- '0'): *(s) <= 'F'? (*(s)-'A'+10):(*(s)-'a'+10)) * 16) + (*((s)+1) <= '9'? (*((s)+1)- '0'): *((s)+1) <= 'F'? (*((s)+1)-'A'+10):(*((s)+1)-'a'+10))))
      ask = 1;
  if (ask)
    {
      char buf[5+32+1];
      ...
      if (!opt.batch)
        tty_printf (gettext ("Please remove the current card and " "insert the one with serial number:\n" "   %.*s\n"), 32, serialno);
      ...
      if ( !opt.batch
           && cpr_get_answer_okay_cancel ("cardctrl.change_card.okay",
                                          gettext ("Hit return when ready " "or enter 'c' to cancel: "),
...
unhexify_fpr (const char *hexstr, unsigned char *fpr)
{
  ...
  for (s=hexstr, n=0; ((*(s) >= '0' && *(s) <= '9') || (*(s) >= 'A' && *(s) <= 'F') || (*(s) >= 'a' && *(s) <= 'f')); s++, n++)
    ;
  if (*s || (n != 40))
    return 0;
  n /= 2;
  for (s=hexstr, n=0; *s; s += 2, n++)
    fpr[n] = (((*(s) <= '9'? (*(s)- '0'): *(s) <= 'F'? (*(s)-'A'+10):(*(s)-'a'+10)) * 16) + (*((s)+1) <= '9'? (*((s)+1)- '0'): *((s)+1) <= 'F'? (*((s)+1)-'A'+10):(*((s)+1)-'a'+10)));
  return 1;
}
...
store_serialno (const char *line)
{
  ...
  for (s=line; ((*(s) >= '0' && *(s) <= '9') || (*(s) >= 'A' && *(s) <= 'F') || (*(s) >= 'a' && *(s) <= 'f')); s++)
    ;
  p = xmalloc (s + 1 - line);
...
learn_status_cb (void *opaque, const char *line)
{
  ...
  for (keywordlen=0; *line && !(*(line) == ' ' || *(line) == '\t'); line++, keywordlen++)
    ;
  while ((*(line) == ' ' || *(line) == '\t'))
    line++;
  ...
  if (keywordlen == 8 && !memcmp (keyword, "SERIALNO", keywordlen))
    {
      ...
      parm->is_v2 = (strlen (parm->serialno) >= 16
                     && (((*(parm->serialno+12) <= '9'? (*(parm->serialno+12)- '0'): *(parm->serialno+12) <= 'F'? (*(parm->serialno+12)-'A'+10):(*(parm->serialno+12)-'a'+10)) * 16) + (*((parm->serialno+12)+1) <= '9'? (*((parm->serialno+12)+1)- '0'): *((parm->serialno+12)+1) <= 'F'? (*((parm->serialno+12)+1)-'A'+10):(*((parm->serialno+12)+1)-'a'+10))) >= 2 );
    }
    ...
    {
      parm->sig_counter = strtoul (line, ((void*)0), 0);
    }
  ...
  else if (keywordlen == 6 && !memcmp (keyword, "EXTCAP", keywordlen))
    {
      ...
      if (buf)
        {
          for (p = strtok (buf, " "); p; p = strtok (((void*)0), " "))
            {
              p2 = strchr (p, '=');
              if (p2)
                {
  ...
  else if (keywordlen == 8 && !memcmp (keyword, "KEY-TIME", keywordlen))
    {
      ...
      while ((*(line) == ' ' || *(line) == '\t'))
        line++;
      if (no == 1)
        parm->fpr1time = strtoul (line, ((void*)0), 10);
      else if (no == 2)
        parm->fpr2time = strtoul (line, ((void*)0), 10);
      else if (no == 3)
        parm->fpr3time = strtoul (line, ((void*)0), 10);
    }
    ...
    {
      int no = keyword[11] - '1';
      ((void)((no >= 0 && no <= 3) || (__assert_fail("no >= 0 && no <= 3", "cardglue.c", 843, __func__),0)));
  ...
  else if (keywordlen == 8 && !memcmp (keyword, "KEY-ATTR", keywordlen))
    {
      ...
      keyno--;
      if (keyno >= 0 && keyno < (sizeof(parm->key_attr)/sizeof((parm->key_attr)[0])))
        {
  ...
  if (app->assuan_ctx)
    {
      ...
      if (12 + strlen (name) > (sizeof(line)/sizeof((line)[0]))-1)
        return (1);
...
pin_cb (void *opaque, const char *info, char **retstr)
{
  ...
  *retstr = ((void*)0);
  ...
  if (info && *info =='|' && (ends=strchr (info+1, '|')))
    ...
    g10_log_debug ("pin_cb called without proper PIN info hack\n");
 ...
 again:
      ...
      if (parm && parm->sn && *parm->sn)
        {
          ...
          write_status_text (76, buf);
          xfree (buf);
        }
      else
        write_status_text (76,
  ...
  value = ask_passphrase (info, again_text,
                          ...
                          isadmin? "passphrase.adminpin.ask" :
                                   "passphrase.pin.ask",
                          newpin && isadmin? gettext ("Enter New Admin PIN: ") :
                          newpin? gettext ("Enter New PIN: ") :
                          isadmin? gettext ("Enter Admin PIN: ")
                                 : gettext ("Enter PIN: "),
                          cacheid,
                          &canceled);
  xfree (cacheid);
  ...
  if (newpin)
    {
      ...
      value2 = ask_passphrase (info, ((void*)0),
                               "passphrase.pin.repeat",
                               gettext ("Repeat this PIN: "),
                               ((void*)0),
                               &canceled);
      if (!value2 && canceled)
        {
      ...
      if (strcmp (value, value2))
        {
          again_text = ("PIN not correctly repeated; try again");
          xfree (value2);
          xfree (value);
          value = ((void*)0);
          goto again;
  ...
  if (app->assuan_ctx)
    {
      ...
      *p++ = ' ';
      for (; valuelen; value++, valuelen--)
        {
          if (p >= line + (sizeof(line)/sizeof((line)[0]))-5 )
            return (1);
  ...
  if (app->assuan_ctx)
    {
      ...
      snprintf (line, (sizeof(line)/sizeof((line)[0]))-1, "SCD WRITEKEY --force OPENPGP.%d", keyno);
    ...
    {
      snprintf (line, (sizeof(line)/sizeof((line)[0]))-1, "OPENPGP.%d", keyno);
      line[(sizeof(line)/sizeof((line)[0]))-1] = 0;
      rc = app->fnc.writekey (app, ((void*)0), line, 0x0001,
                              pin_cb, &parm,
                              keydata, keydatalen);
    }
  ...
  if (keywordlen == 8 && !memcmp (keyword, "KEY-DATA", keywordlen))
    {
      ...
      if( mpi_fromstr (a, buf) )
        g10_log_error ("error parsing received key data\n");
      else if (*name == 'n' && (*(name+1) == ' ' || *(name+1) == '\t'))
        parm->n = a;
      else if (*name == 'e' && (*(name+1) == ' ' || *(name+1) == '\t'))
        parm->e = a;
      else
        {
          g10_log_info ("unknown parameter name in received key data\n");
          mpi_free (a);
        }
      xfree (buf);
    }
    ...
    {
      parm->created_at = (u32)strtoul (line, ((void*)0), 10);
    }
                  ...
                  const char *serialno, u32 *createtime)
{
  ...
  if (app->assuan_ctx)
    {
      snprintf (line, (sizeof(line)/sizeof((line)[0]))-1, "SCD GENKEY %s%d",
                force? "--force ":"", keyno);
      line[(sizeof(line)/sizeof((line)[0]))-1] = 0;
      rc = test_transact (assuan_transact (app->assuan_ctx, line,
                                           ((void*)0), ((void*)0), ((void*)0), ((void*)0),
    ...
    {
      snprintf (line, (sizeof(line)/sizeof((line)[0]))-1, "%d", keyno);
  ...
  if (app->assuan_ctx)
    {
      ...
      if (indatalen*2 + 50 > (sizeof(line)/sizeof((line)[0])))
        return (1);
      ...
      if (!rc)
        {
          ...
          snprintf (line, (sizeof(line)/sizeof((line)[0]))-1, "SCD PKSIGN %s%s",
                    hashalgo == 2? "--hash=sha1 ":
                    hashalgo == 11? "--hash=sha224 ":
                    hashalgo == 8? "--hash=sha256 ":
                    hashalgo == 9? "--hash=sha384 ":
                    hashalgo == 10? "--hash=sha512 ":
                    hashalgo == 3? "--hash=rmd160 ":
                    hashalgo == 1? "--hash=md5 " : "",
                    serialno);
          line[(sizeof(line)/sizeof((line)[0]))-1] = 0;
          rc = test_transact (assuan_transact (app->assuan_ctx, line,
  ...
  if (app->assuan_ctx)
    {
      ...
      if (!rc)
        {
          init_membuf (&data, 1024);
          snprintf (line, (sizeof(line)/sizeof((line)[0]))-1, "SCD PKDECRYPT %s", serialno);
          line[(sizeof(line)/sizeof((line)[0]))-1] = 0;
          rc = test_transact (assuan_transact (app->assuan_ctx, line,
                                               membuf_data_cb, &data,
                                               ((void*)0), ((void*)0), ((void*)0), ((void*)0)),
  ...
  if (app->assuan_ctx)
    {
      ...
      snprintf (line, (sizeof(line)/sizeof((line)[0]))-1, "SCD PASSWD%s %d",
                reset? " --reset":"", chvno);
      line[(sizeof(line)/sizeof((line)[0]))-1] = 0;
      rc = test_transact (assuan_transact (app->assuan_ctx, line,
                                           ((void*)0), ((void*)0), ((void*)0), ((void*)0), ((void*)0), ((void*)0)),
  ...
  else
    {
      ...
      rc = app->fnc.change_pin (app, ((void*)0), chvnostr, reset,
                                pin_cb, &parm);
    }
  ...
  if (app->assuan_ctx)
    {
      ...
      if (15 + strlen (serialnobuf) > (sizeof(line)/sizeof((line)[0]))-1)
        return (1);
...
agent_clear_pin_cache (const char *sn)
{
  ...
  if (cacheid)
    {
      passphrase_clear_cache (((void*)0), cacheid, 0);
      xfree (cacheid);
    }
}
