...
make_session_key( DEK *dek )
{
 ...
 g10_log_info(gettext ("weak key created - retrying\n") );

 randomize_buffer( dek->key, dek->keylen, 1 );
    }
    g10_log_fatal(gettext ("cannot avoid weak key for symmetric cipher; tried %d times!\n"),
...
encode_session_key( DEK *dek, unsigned nbits )
{
 ...
 g10_log_bug("can't encode a %d bit key in a %d bits frame\n",
      dek->keylen*8, nbits );
# 97 "seskey.c"
    csum = 0;
 ...
 csum += *p++;
    ...
    ((void)((i > 0) || (__assert_fail("i > 0", "seskey.c", 106, __func__),0)));
 ...
 xfree(pp);
    ...
    frame[n++] = csum >>8;
    frame[n++] = csum;
    ((void)((n == nframe) || (__assert_fail("n == nframe", "seskey.c", 137, __func__),0)));
   ...
   const byte *asn, size_t asnlen )
{
 ...
 g10_log_bug("can't encode a %d bit MD into a %d bits frame\n",
      (int)(len*8), (int)nbits);
    ...
    frame = ((md)->secure)? xmalloc_secure( nframe ) : xmalloc( nframe );
    n = 0;
    frame[n++] = 0;
    frame[n++] = 1;
    i = nframe - len - asnlen -3 ;
    ((void)((i > 1) || (__assert_fail("i > 1", "seskey.c", 168, __func__),0)));
    memset( frame+n, 0xff, i ); n += i;
    frame[n++] = 0;
    memcpy( frame+n, asn, asnlen ); n += asnlen;
    memcpy( frame+n, md_read(md, algo), len ); n += len;
    ((void)((n == nframe) || (__assert_fail("n == nframe", "seskey.c", 173, __func__),0)));
    a = (((md)->secure)
   ...
   MD_HANDLE md, int hash_algo )
{
  ...
  ((void)((hash_algo) || (__assert_fail("hash_algo", "seskey.c", 205, __func__),0)));
  ((void)((pk || sk) || (__assert_fail("pk || sk", "seskey.c", 206, __func__),0)));

  if((pk?pk->pubkey_algo:sk->pubkey_algo) == 17)
    {
      ...
      if(qbytes%8)
 {
   g10_log_error(gettext ("DSA requires the hash length to be a" " multiple of 8 bits\n"));
...
# 230 "seskey.c"
      if(qbytes<160)
 {
   g10_log_error(gettext ("DSA key %s uses an unsafe (%u bit) hash\n"),
 ...
 {
   g10_log_error(gettext ("DSA key %s requires a %u bit or larger hash\n"),
