    ...
    int nooutput, int clearsig )
{
 ...
 write_status_text_and_buffer(73,
         status,pt->name,pt->namelen,0);

 if(!pt->is_partial)
   {
     sprintf(status,"%lu",(ulong)pt->len);
     write_status_text(74,status);
   }
      }
 ...
 g10_log_info(gettext ("data not saved; use option \"--output\" to save it\n"));
 nooutput = 1;
    }
 ...
 while( !overwrite_filep (fname) ) {
            char *tmp = ask_outfile_name (((void*)0), 0);
            if ( !tmp || !*tmp ) {
                xfree (tmp);
                rc = 25;
                goto leave;
 ...
 ;
      ...
      {
        (*__errno_location()) = (63);
 g10_log_error(gettext ("error creating `%s': %s\n"), fname, strerror((*__errno_location())) );
 rc = 25;
 goto leave;
      }
...
# 189 "plaintext.c"
    if( !pt->is_partial ) {
        ...
        if (clearsig) {
            g10_log_error ("clearsig encountered while not expected\n");
            rc = 39;
            goto leave;
        }
 ...
 if( convert ) {
     for( ; pt->len; pt->len-- ) {
  if( (c = ( ((pt->buf)->nofast || (pt->buf)->d.start >= (pt->buf)->d.len )? iobuf_readbyte((pt->buf)) : ( (pt->buf)->nbytes++, (pt->buf)->d.buf[(pt->buf)->d.start++] ) )) == -1 ) {
      g10_log_error("Problem reading source (%u bytes remaining)\n",
         (unsigned)pt->len);
      rc = 21;
      goto leave;
  }
  if( mfx->md )
      do { if( (mfx->md)->bufcount == (mfx->md)->bufsize ) md_write( (mfx->md), ((void*)0), 0 ); (mfx->md)->buffer[(mfx->md)->bufcount++] = (c) & 0xff; } while(0);
 ...
 if( convert ) {
     while( (c = ( ((pt->buf)->nofast || (pt->buf)->d.start >= (pt->buf)->d.len )? iobuf_readbyte((pt->buf)) : ( (pt->buf)->nbytes++, (pt->buf)->d.buf[(pt->buf)->d.start++] ) )) != -1 ) {
  if( mfx->md )
      do { if( (mfx->md)->bufcount == (mfx->md)->bufsize ) md_write( (mfx->md), ((void*)0), 0 ); (mfx->md)->buffer[(mfx->md)->bufcount++] = (c) & 0xff; } while(0);
 ...
 while( (c = ( ((pt->buf)->nofast || (pt->buf)->d.start >= (pt->buf)->d.len )? iobuf_readbyte((pt->buf)) : ( (pt->buf)->nbytes++, (pt->buf)->d.buf[(pt->buf)->d.start++] ) )) != -1 ) {
  ...
  continue;
     if( state == 2 ) {
  do { if( (mfx->md)->bufcount == (mfx->md)->bufsize ) md_write( (mfx->md), ((void*)0), 0 ); (mfx->md)->buffer[(mfx->md)->bufcount++] = ('\r') & 0xff; } while(0);
  do { if( (mfx->md)->bufcount == (mfx->md)->bufsize ) md_write( (mfx->md), ((void*)0), 0 ); (mfx->md)->buffer[(mfx->md)->bufcount++] = ('\n') & 0xff; } while(0);
  state = 0;
     }
     if( !state ) {
  if( c == '\r' )
 ...
 g10_log_error("Error flushing plaintext: %s\n",strerror((*__errno_location())));
 rc=22;
      }
...
do_hash( MD_HANDLE md, MD_HANDLE md2, IOBUF fp, int textmode )
{
 ...
 int lc = -1;
 while( (c = ( ((fp)->nofast || (fp)->d.start >= (fp)->d.len )? iobuf_readbyte((fp)) : ( (fp)->nbytes++, (fp)->d.buf[(fp)->d.start++] ) )) != -1 ) {
     if( c == '\n' && lc == '\r' )
  do { if( (md2)->bufcount == (md2)->bufsize ) md_write( (md2), ((void*)0), 0 ); (md2)->buffer[(md2)->bufcount++] = (c) & 0xff; } while(0);
     else if( c == '\n' ) {
  do { if( (md2)->bufcount == (md2)->bufsize ) md_write( (md2), ((void*)0), 0 ); (md2)->buffer[(md2)->bufcount++] = ('\r') & 0xff; } while(0);
  do { if( (md2)->bufcount == (md2)->bufsize ) md_write( (md2), ((void*)0), 0 ); (md2)->buffer[(md2)->bufcount++] = (c) & 0xff; } while(0);
     }
     else if( c != '\n' && lc == '\r' ) {
  do { if( (md2)->bufcount == (md2)->bufsize ) md_write( (md2), ((void*)0), 0 ); (md2)->buffer[(md2)->bufcount++] = ('\n') & 0xff; } while(0);
  do { if( (md2)->bufcount == (md2)->bufsize ) md_write( (md2), ((void*)0), 0 ); (md2)->buffer[(md2)->bufcount++] = (c) & 0xff; } while(0);
     }
     else
  do { if( (md2)->bufcount == (md2)->bufsize ) md_write( (md2), ((void*)0), 0 ); (md2)->buffer[(md2)->bufcount++] = (c) & 0xff; } while(0);
     ...
     if( md )
  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (c) & 0xff; } while(0);
     lc = c;
 }
    }
    else {
      ...
      const char *inname, int textmode )
{
    ...
    fp = open_sigfile( inname, &pfx );

    if( !fp && !opt.batch ) {
 int any=0;
 tty_printf(gettext ("Detached signature.\n"));
 do {
     char *name;
     xfree(answer);
                                ;
     name = cpr_get("detached_signature.filename",
      gettext ("Please enter name of data file: "));
                             ;
     cpr_kill_prompt();
     answer=make_filename(name,(void *)((void*)0));
     xfree(name);
  ...
  goto leave;
     ...
     if( !fp && (*__errno_location()) == (44) ) {
  tty_printf("No such file, try again or hit enter to quit.\n");
  any++;
     }
     else if( !fp )
       {
  g10_log_error(gettext ("can't open `%s': %s\n"), answer, strerror((*__errno_location())));
  rc = 21;
  goto leave;
       }
 } while( !fp );
    ...
    if( !fp ) {
 if( opt.verbose )
     g10_log_info(gettext ("reading stdin ...\n"));
 fp = iobuf_open( ((void*)0) );
 ((void)((fp) || (__assert_fail("fp", "plaintext.c", 516, __func__),0)));
    }
  ...
  const char *sigfilename, int textmode )
{
    ...
    if( !files ) {
        ...
        g10_log_error (gettext ("no signed data\n"));
        return 24;
    }
 ...
 if( !fp ) {
     g10_log_error(gettext ("can't open signed data `%s'\n"),
