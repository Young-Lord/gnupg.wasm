...
overwrite_filep( const char *fname )
{
 ...
 return 0;
    ...
    tty_printf(gettext ("File `%s' exists. "), fname);
    if( cpr_enabled () )
        tty_printf ("\n");
    if( cpr_get_answer_is_yes("openfile.overwrite.okay",
          gettext ("Overwrite? (y/N) ")) )
 return 1;
    return 0;
}
# 106 "openfile.c"
...
make_outfile_name( const char *iname )
{
 ...
 return buf;
    ...
    else if( n > 5 && !strcmp( (iname+n-5), ("." "sign") ) ) {
 char *buf = xstrdup( iname );
 buf[n-5] = 0;
 return buf;
    }

    g10_log_info(gettext ("%s: unknown suffix\n"), iname );
    return ((void*)0);
}
# 140 "openfile.c"
char *
ask_outfile_name( const char *name, size_t namelen )
{
 ...
 return ((void*)0);
    ...
    s = gettext ("Enter new filename");

    defname = name && namelen? make_printable_string( name, namelen, 0): ((void*)0);
 ...
 sprintf(prompt, "%s: ", s );
    ...
    xfree(prompt);
    if( !*fname ) {
 xfree( fname ); fname = ((void*)0);
 fname = defname; defname = ((void*)0);
    }
    xfree(defname);
    if (fname)
        trim_spaces (fname);
...
open_outfile( const char *iname, int mode, IOBUF *a )
{
  ...
  *a = ((void*)0);
  if( iobuf_is_pipe_filename (iname) && !opt.outfile ) {
    *a = iobuf_create(((void*)0));
    if( !*a ) {
      g10_log_error(gettext ("can't open `%s': %s\n"), "[stdout]", strerror((*__errno_location())) );
      rc = 25;
    }
    else if( opt.verbose )
      g10_log_info(gettext ("writing to stdout\n"));
  }
  else {
    char *buf = ((void*)0);
    const char *name;
...
get_matching_datafile (const char *sigfilename)
{
  ...
  if (len > 4
      && (!strcmp (sigfilename + len - 4, "." "sig")
          || (len > 5 && !strcmp(sigfilename + len - 5, "." "sign"))
          || !strcmp(sigfilename + len - 4, "." "asc")))
    {
  ...
  if (buf)
    {
      ...
      if (a)
        g10_log_info (gettext ("assuming signed data in `%s'\n"), buf);
      if (a && pfx)
...
{
    const char *datadir = "/home/yang/wasm/gnupg-1.4.23/prefix/share/gnupg";
    char *fname;
    FILE *src, *dst;
    int linefeeds=0;
    int c;
 ...
 return;
    ...
    strcpy(stpcpy(fname, destdir), "/" "gpg" "." "conf" );
    oldmask=umask(077);
 ...
 if( linefeeds < 3 ) {
  ...
  linefeeds++;
    ...
    fclose( dst );
    fclose( src );
    g10_log_info(gettext ("new configuration file `%s' created\n"), fname );
    if (any_option)
        g10_log_info (gettext ("WARNING: options in `%s'" " are not yet active during this run\n"),
...
try_make_homedir( const char *fname )
{
 ...
 if( mkdir( fname, 0400|0200|0100 ) )
     g10_log_fatal( gettext ("can't create directory `%s': %s\n"),
     fname, strerror((*__errno_location())) );
 else if( !opt.quiet )
     g10_log_info( gettext ("directory `%s' created\n"), fname );
# 481 "openfile.c"
