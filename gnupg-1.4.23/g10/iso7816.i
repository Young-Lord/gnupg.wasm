...
map_sw (int sw)
{
  ...
  switch (sw)
    {
    case SW_EEPROM_FAILURE: ec = 1; break;
    case SW_TERM_STATE: ec = 1; break;
    case SW_WRONG_LENGTH: ec = 45; break;
    case SW_SM_NOT_SUP: ec = 19; break;
    case SW_CC_NOT_SUP: ec = 19; break;
    case SW_CHV_WRONG: ec = 11; break;
    case SW_CHV_BLOCKED: ec = 26; break;
    case SW_USE_CONDITIONS: ec = 1; break;
    case SW_NOT_SUPPORTED: ec = 19; break;
    case SW_BAD_PARAMETER: ec = 45; break;
    case SW_FILE_NOT_FOUND: ec = 24; break;
    case SW_RECORD_NOT_FOUND:ec= 1; break;
    case SW_REF_NOT_FOUND: ec = 1; break;
    case SW_BAD_P0_P1: ec = 45; break;
    case SW_EXACT_LENGTH: ec = 45; break;
    case SW_INS_NOT_SUP: ec = 1; break;
    case SW_CLA_NOT_SUP: ec = 1; break;
    case SW_SUCCESS: ec = 0; break;

    case SW_HOST_OUT_OF_CORE: ec = 31; break;
    case SW_HOST_INV_VALUE: ec = 45; break;
    case SW_HOST_INCOMPLETE_CARD_RESPONSE: ec = 1; break;
    case SW_HOST_NOT_SUPPORTED: ec = 19; break;
    case SW_HOST_LOCKING_FAILED: ec = 1; break;
    case SW_HOST_BUSY: ec = 1; break;
    case SW_HOST_NO_CARD: ec = 57; break;
    case SW_HOST_CARD_INACTIVE: ec = 1; break;
    case SW_HOST_CARD_IO_ERROR: ec = 1; break;
    case SW_HOST_GENERAL_ERROR: ec = 1; break;
    case SW_HOST_NO_READER: ec = 1; break;
    case SW_HOST_ABORTED: ec = 56; break;
    case SW_HOST_NO_KEYPAD: ec = 19; break;
...
{
  int sw;
  sw = apdu_send_simple (slot, 0, 0x00, 0xA4, 4,
                     ...
                     unsigned char **result, size_t *resultlen)
{
  ...
  if (result || resultlen)
  ...
  else
    {
      ...
      p1 = 0x0c;
      sw = apdu_send_simple (slot, 0, 0x00, 0xA4,
                             p0, p1, 2, (char*)tagbuf );
      return map_sw (sw);
    }
                        ...
                        unsigned char **result, size_t *resultlen)
{
  ...
  *resultlen = 0;
  ...
  sw = apdu_send (slot, 0, 0x80, 0xAA, list_dirs? 1:2, 0, -1, ((void*)0),
                  result, resultlen);
  if (sw != SW_SUCCESS)
    {
                   ...
                   iso7816_pininfo_t *pininfo)
{
  ...
  if (pininfo && pininfo->mode)
    sw = apdu_send_simple_kp (slot, 0x00, 0x20, 0, chvno, chvlen, chv,
                              pininfo->mode,
                              pininfo->minlen,
                              pininfo->maxlen,
                              pininfo->padlen);
  else
    sw = apdu_send_simple (slot, 0, 0x00, 0x20, 0, chvno, chvlen, chv);
  return map_sw (sw);
}
...
{
  return iso7816_verify_kp (slot, chvno, chv, chvlen, ((void*)0));
}
                                  ...
                                  iso7816_pininfo_t *pininfo)
{
  ...
  buf = xmalloc((oldchvlen + newchvlen));
  if (!buf)
    return ((1));
  if (oldchvlen)
  ...
  if (pininfo && pininfo->mode)
    sw = apdu_send_simple_kp (slot, 0x00, 0x24,
    ...
    sw = apdu_send_simple (slot, 0, 0x00, 0x24,
                           oldchvlen? 0 : 1, chvno, oldchvlen+newchvlen, buf);
  xfree (buf);
  return map_sw (sw);
                                ...
                                iso7816_pininfo_t *pininfo)
{
  ...
  if (pininfo && pininfo->mode)
    sw = apdu_send_simple_kp (slot, 0x00, 0x2C,
    ...
    sw = apdu_send_simple (slot, 0, 0x00, 0x2C,
                           2, chvno, newchvlen, newchv);
  return map_sw (sw);
}
...
iso7816_reset_retry_counter (int slot, int chvno,
                             const char *newchv, size_t newchvlen)
{
  return iso7816_reset_retry_counter_kp (slot, chvno, newchv, newchvlen, ((void*)0));
}
                  ...
                  unsigned char **result, size_t *resultlen)
{
  ...
  *resultlen = 0;
  ...
  sw = apdu_send_le (slot, extended_mode, 0x00, 0xCA,
                     ((tag >> 8) & 0xff), (tag & 0xff), -1, ((void*)0), le,
                     result, resultlen);
  if (sw != SW_SUCCESS)
    {
                  ...
                  const unsigned char *data, size_t datalen)
{
  ...
  sw = apdu_send_simple (slot, extended_mode, 0x00, 0xDA,
                         ((tag >> 8) & 0xff), (tag & 0xff),
                      ...
                      const unsigned char *data, size_t datalen)
{
  ...
  sw = apdu_send_simple (slot, extended_mode, 0x00, 0xDA +1,
                         ((tag >> 8) & 0xff), (tag & 0xff),
                         datalen, (const char*)data);
  return map_sw (sw);
}
                             ...
                             const unsigned char *data, size_t datalen)
{
  ...
  sw = apdu_send_simple (slot, 0, 0x00, 0x22, p1, p2,
                     ...
                     unsigned char **result, size_t *resultlen)
{
  ...
  *resultlen = 0;
  ...
  sw = apdu_send_le (slot, extended_mode,
                     0x00, 0x47, read_only? 0x81:0x80, 0,
                     datalen, (const char*)data,
                     le >= 0 && le < 256? 256:le,
                     result, resultlen);
  if (sw != SW_SUCCESS)
  ...
  do
    {
      ...
      bufferlen = 0;
      n = read_all? 0 : nmax;
      sw = apdu_send_le (slot, 0, 0x00, 0xB0,
                         ((offset>>8) & 0xff), (offset & 0xff) , -1, ((void*)0),
                         n, &buffer, &bufferlen);
      if ( (((sw)&~0xff) == SW_EXACT_LENGTH) )
        {
          n = (sw & 0x00ff);
          sw = apdu_send_le (slot, 0, 0x00, 0xB0,
                             ((offset>>8) & 0xff), (offset & 0xff) , -1, ((void*)0),
      ...
      if (*result)
        {
          unsigned char *p = xrealloc((*result),(*resultlen + bufferlen));
          if (!p)
            {
              gpg_error_t err = (1);
              xfree (buffer);
                     ...
                     unsigned char **result, size_t *resultlen)
{
  ...
  *resultlen = 0;
  ...
  buffer = ((void*)0);
  bufferlen = 0;
  sw = apdu_send_le (slot, 0, 0x00, 0xB2,
                     recno,
