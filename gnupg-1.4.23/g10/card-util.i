...
change_pin (int unblock_v2, int allow_admin)
{
  ...
  if (rc)
    {
      g10_log_error (gettext ("OpenPGP card not available: %s\n"),
                  g10_errstr ((rc)));
      return;
    }

  g10_log_info (gettext ("OpenPGP card no. %s detected\n"),
  ...
  if (unblock_v2)
    {
      if (!info.is_v2)
        g10_log_error (gettext ("This command is only available for version 2 cards\n"));
 ...
 answer = cpr_get("cardutil.change_pin.menu",gettext ("Your selection? "));
 cpr_kill_prompt();
 if (strlen (answer) != 1)
   continue;
...
print_name (FILE *fp, const char *text, const char *name)
{
  ...
  if (name && *name)
    ...
    tty_fprintf (fp, gettext ("[not set]"));
  tty_fprintf (fp, "\n");
}
...
card_status (FILE *fp, char *serialno, size_t serialnobuflen)
{
  ...
  if (!info.serialno || strncmp (info.serialno, "D27600012401", 12)
      || strlen (info.serialno) != 32 )
    {
      if (info.apptype && !strcmp (info.apptype, "NKS"))
        {
          if (opt.with_colons)
            fputs ("netkey-card:\n", fp);
          g10_log_info ("this is a NetKey card\n");
        }
      else if (info.apptype && !strcmp (info.apptype, "DINSIG"))
        {
          if (opt.with_colons)
            fputs ("dinsig-card:\n", fp);
          g10_log_info ("this is a DINSIG compliant card\n");
        }
      else if (info.apptype && !strcmp (info.apptype, "P15"))
        {
          if (opt.with_colons)
            fputs ("pkcs15-card:\n", fp);
          g10_log_info ("this is a PKCS#15 compliant card\n");
        }
      else if (info.apptype && !strcmp (info.apptype, "GELDKARTE"))
        {
          if (opt.with_colons)
            fputs ("geldkarte-card:\n", fp);
          g10_log_info ("this is a Geldkarte compliant card\n");
        }
      else
        {
          if (opt.with_colons)
      ...
      g10_log_info ("not an OpenPGP card\n");
      agent_release_card_info (&info);
      xfree (pk);
      return;
    }
  ...
  if (!serialno)
    ...
    g10_log_error ("serial number longer than expected\n");
  else
  ...
  if (opt.with_colons)
    {
      ...
      uval = (((*(info.serialno+16) <= '9'? (*(info.serialno+16)- '0'): *(info.serialno+16) <= 'F'? (*(info.serialno+16)-'A'+10):(*(info.serialno+16)-'a'+10)) * 16) + (*((info.serialno+16)+1) <= '9'? (*((info.serialno+16)+1)- '0'): *((info.serialno+16)+1) <= 'F'? (*((info.serialno+16)+1)-'A'+10):(*((info.serialno+16)+1)-'a'+10)))*256 + (((*(info.serialno+18) <= '9'? (*(info.serialno+18)- '0'): *(info.serialno+18) <= 'F'? (*(info.serialno+18)-'A'+10):(*(info.serialno+18)-'a'+10)) * 16) + (*((info.serialno+18)+1) <= '9'? (*((info.serialno+18)+1)- '0'): *((info.serialno+18)+1) <= 'F'? (*((info.serialno+18)+1)-'A'+10):(*((info.serialno+18)+1)-'a'+10)));
      ...
      for (i=0; i < (sizeof(info.key_attr)/sizeof((info.key_attr)[0])); i++)
      ...
      fputs ("cafpr:", fp);
      print_sha1_fpr_colon (fp, info.cafpr1valid? info.cafpr1:((void*)0));
      print_sha1_fpr_colon (fp, info.cafpr2valid? info.cafpr2:((void*)0));
      print_sha1_fpr_colon (fp, info.cafpr3valid? info.cafpr3:((void*)0));
      putc ('\n', fp);
      fputs ("fpr:", fp);
      print_sha1_fpr_colon (fp, info.fpr1valid? info.fpr1:((void*)0));
      print_sha1_fpr_colon (fp, info.fpr2valid? info.fpr2:((void*)0));
      print_sha1_fpr_colon (fp, info.fpr3valid? info.fpr3:((void*)0));
      putc ('\n', fp);
  ...
  else
    {
      ...
      tty_fprintf (fp, "Manufacturer .....: %s\n",
                   get_manufacturer ((((*(info.serialno+16) <= '9'? (*(info.serialno+16)- '0'): *(info.serialno+16) <= 'F'? (*(info.serialno+16)-'A'+10):(*(info.serialno+16)-'a'+10)) * 16) + (*((info.serialno+16)+1) <= '9'? (*((info.serialno+16)+1)- '0'): *((info.serialno+16)+1) <= 'F'? (*((info.serialno+16)+1)-'A'+10):(*((info.serialno+16)+1)-'a'+10)))*256
                                     + (((*(info.serialno+18) <= '9'? (*(info.serialno+18)- '0'): *(info.serialno+18) <= 'F'? (*(info.serialno+18)-'A'+10):(*(info.serialno+18)-'a'+10)) * 16) + (*((info.serialno+18)+1) <= '9'? (*((info.serialno+18)+1)- '0'): *((info.serialno+18)+1) <= 'F'? (*((info.serialno+18)+1)-'A'+10):(*((info.serialno+18)+1)-'a'+10)))));
      tty_fprintf (fp, "Serial number ....: %.8s\n", info.serialno+20);

      print_isoname (fp, "Name of cardholder: ", "name", info.disp_name);
      print_name (fp, "Language prefs ...: ", info.disp_lang);
      tty_fprintf (fp, "Sex ..............: %s\n",
                   info.disp_sex == 1? gettext ("male"):
                   info.disp_sex == 2? gettext ("female") : gettext ("unspecified"));
      ...
      tty_fprintf (fp, "Signature PIN ....: %s\n",
                   info.chv1_cached? gettext ("not forced"): gettext ("forced"));
      if (info.key_attr[0].algo)
        {
      ...
      print_sha1_fpr (fp, info.fpr1valid? info.fpr1:((void*)0));
      if (info.fpr1valid && info.fpr1time)
        tty_fprintf (fp, "      created ....: %s\n",
                     isotimestamp (info.fpr1time));
      tty_fprintf (fp, "Encryption key....:");
      print_sha1_fpr (fp, info.fpr2valid? info.fpr2:((void*)0));
      if (info.fpr2valid && info.fpr2time)
        tty_fprintf (fp, "      created ....: %s\n",
                     isotimestamp (info.fpr2time));
      tty_fprintf (fp, "Authentication key:");
      print_sha1_fpr (fp, info.fpr3valid? info.fpr3:((void*)0));
      ...
      thefpr = (info.fpr1valid? info.fpr1 : info.fpr2valid? info.fpr2 :
                info.fpr3valid? info.fpr3 : ((void*)0));
          ...
          if ( !get_seckeyblock_byfprint (&keyblock, thefpr, 20) )
          ...
          else if ( !get_keyblock_byfprint (&keyblock, thefpr, 20) )
            {
              ...
              if (!auto_create_card_key_stub (info.serialno,
                                              info.fpr1valid? info.fpr1:((void*)0),
                                              info.fpr2valid? info.fpr2:((void*)0),
                                              info.fpr3valid? info.fpr3:((void*)0)))
                {
                  if ( !get_seckeyblock_byfprint (&keyblock, thefpr, 20) )
                    print_card_key_info (fp, keyblock);
  ...
  for (;;)
    {
      ...
      if (name[i])
        tty_printf (gettext ("Error: Only plain ASCII is currently allowed.\n"));
      else if (strchr (name, '<'))
        tty_printf (gettext ("Error: The \"<\" character may not be used.\n"));
      else if (strstr (name, "  "))
        tty_printf (gettext ("Error: Double spaces are not allowed.\n"));
      else
        return name;
      xfree (name);
    }
...
change_name (void)
{
  char *surname = ((void*)0), *givenname = ((void*)0);
  char *isoname, *p;
  int rc;

  surname = get_one_name ("keygen.smartcard.surname",
                                    gettext ("Cardholder's surname: "));
  givenname = get_one_name ("keygen.smartcard.givenname",
                                       gettext ("Cardholder's given name: "));
  ...
  if (strlen (isoname) > 39 )
    {
      tty_printf (gettext ("Error: Combined name too long " "(limit is %d characters).\n"), 39);

      xfree (isoname);
      return -1;
    }

  rc = agent_scd_setattr ("DISP-NAME", isoname, strlen (isoname), ((void*)0) );
  if (rc)
    g10_log_error ("error setting Name: %s\n", g10_errstr ((rc)));
...
change_url (void)
{
  ...
  url = cpr_get ("cardedit.change_url", gettext ("URL to retrieve public key: "));
  if (!url)
    return -1;
  trim_spaces (url);
  cpr_kill_prompt ();
  ...
  if (strlen (url) > 254 )
    {
      tty_printf (gettext ("Error: URL too long " "(limit is %d characters).\n"), 254);

      xfree (url);
      return -1;
    }

  rc = agent_scd_setattr ("PUBKEY-URL", url, strlen (url), ((void*)0) );
  if (rc)
    g10_log_error ("error setting URL: %s\n", g10_errstr ((rc)));
  xfree (url);
  write_sc_op_status (rc);
  return rc;
}
...
fetch_url(void)
{
  ...
  if(rc)
    g10_log_error("error retrieving URL from card: %s\n",g10_errstr ((rc)));
  else
    {
      struct keyserver_spec *spec=((void*)0);

      rc=agent_scd_getattr("KEY-FPR",&info);
      if(rc)
 g10_log_error("error retrieving key fingerprint from card: %s\n",
    g10_errstr ((rc)));
      else if (info.pubkey_url && *info.pubkey_url)
 {
   spec=parse_keyserver_uri(info.pubkey_url,1,((void*)0),0);
...
get_data_from_file (const char *fname, size_t maxlen, char **r_buffer)
{
  ...
  *r_buffer = ((void*)0);
  ...
  data = xmalloc((maxlen? maxlen:1));
  if (!data)
    {
      tty_printf (gettext ("error allocating enough memory: %s\n"), strerror ((*__errno_location())));
      fclose (fp);
      return -1;
    }
  ...
  if (n < 0)
    {
      tty_printf (gettext ("error reading `%s': %s\n"), fname, strerror ((*__errno_location())));
      xfree (data);
      return -1;
    }
  *r_buffer = data;
...
put_data_to_file (const char *fname, const void *buffer, size_t length)
{
  ...
  if (!fp)
    {
      tty_printf (gettext ("can't create `%s': %s\n"), fname, strerror ((*__errno_location())));
      return -1;
    }

  if (length && fwrite (buffer, length, 1, fp) != 1)
    {
      tty_printf (gettext ("error writing `%s': %s\n"), fname, strerror ((*__errno_location())));
      fclose (fp);
      return -1;
    }
  fclose (fp);
...
change_login (const char *args)
{
  ...
  if (args && *args == '<')
  ...
  else
    {
      data = cpr_get ("cardedit.change_login",
                      gettext ("Login data (account name): "));
      if (!data)
        return -1;
      trim_spaces (data);
      cpr_kill_prompt ();
  ...
  if (n > 254 )
    {
      tty_printf (gettext ("Error: Login data too long " "(limit is %d characters).\n"), 254);

      xfree (data);
      return -1;
    }

  rc = agent_scd_setattr ("LOGIN-DATA", data, n, ((void*)0) );
  if (rc)
    g10_log_error ("error setting login data: %s\n", g10_errstr ((rc)));
  xfree (data);
  write_sc_op_status (rc);
  return rc;
}
...
change_private_do (const char *args, int nr)
{
  ...
  ((void)((nr >= 1 && nr <= 4) || (__assert_fail("nr >= 1 && nr <= 4", "card-util.c", 898, __func__),0)));
  do_name[11] = '0' + nr;

  if (args && (args = strchr (args, '<')))
    {
      for (args++; (*(args) == ' ' || *(args) == '\t'); args++)
        ;
      n = get_data_from_file (args, 254, &data);
  ...
  if (n > 254 )
    {
      tty_printf (gettext ("Error: Private DO too long " "(limit is %d characters).\n"), 254);

      xfree (data);
      return -1;
    }

  rc = agent_scd_setattr (do_name, data, n, ((void*)0) );
  if (rc)
    g10_log_error ("error setting private DO: %s\n", g10_errstr ((rc)));
  xfree (data);
  write_sc_op_status (rc);
  return rc;
}
...
change_cert (const char *args)
{
  ...
  if (rc)
    g10_log_error ("error writing certificate to card: %s\n", g10_errstr ((rc)));
  xfree (data);
  write_sc_op_status (rc);
  return rc;
}
...
read_cert (const char *args)
{
  ...
  if (rc)
    g10_log_error ("error reading certificate from card: %s\n", g10_errstr ((rc)));
  else
...
change_lang (void)
{
  ...
  if (strlen (data) > 8 || (strlen (data) & 1))
    {
      tty_printf (gettext ("Error: invalid length of preference string.\n"));
      xfree (data);
      return -1;
    }
  ...
  if (*p)
    {
      tty_printf (gettext ("Error: invalid characters in preference string.\n"));
      xfree (data);
      return -1;
    }

  rc = agent_scd_setattr ("DISP-LANG", data, strlen (data), ((void*)0) );
  if (rc)
    g10_log_error ("error setting lang: %s\n", g10_errstr ((rc)));
  xfree (data);
  write_sc_op_status (rc);
  return rc;
}
...
change_sex (void)
{
  ...
  data = cpr_get ("cardedit.change_sex",
                  gettext ("Sex ((M)ale, (F)emale or space): "));
  if (!data)
    return -1;
  trim_spaces (data);
  cpr_kill_prompt ();
  ...
  if (!*data)
    ...
    {
      tty_printf (gettext ("Error: invalid response.\n"));
      xfree (data);
      return -1;
    }

  rc = agent_scd_setattr ("DISP-SEX", str, 1, ((void*)0) );
  if (rc)
    g10_log_error ("error setting sex: %s\n", g10_errstr ((rc)));
  xfree (data);
  write_sc_op_status (rc);
  return rc;
}
...
change_cafpr (int fprno)
{
  ...
  data = cpr_get ("cardedit.change_cafpr", gettext ("CA fingerprint: "));
  if (!data)
    return -1;
  trim_spaces (data);
  cpr_kill_prompt ();
  ...
  if (i != 20 || *s)
    {
      tty_printf (gettext ("Error: invalid formatted fingerprint.\n"));
      return -1;
    }

  rc = agent_scd_setattr (fprno==1?"CA-FPR-1":
                          fprno==2?"CA-FPR-2":
                          fprno==3?"CA-FPR-3":"x", fpr, 20, ((void*)0) );
  if (rc)
    g10_log_error ("error setting cafpr: %s\n", g10_errstr ((rc)));
  write_sc_op_status (rc);
  return rc;
}
...
toggle_forcesig (void)
{
  ...
  if (rc)
    {
      g10_log_error ("error getting current status: %s\n", g10_errstr ((rc)));
      return;
    }
  newstate = !info.chv1_cached;
  agent_release_card_info (&info);

  rc = agent_scd_setattr ("CHV-STATUS-1", newstate? "\x01":"", 1, ((void*)0));
  if (rc)
    g10_log_error ("error toggling signature PIN flag: %s\n", g10_errstr ((rc)));
  write_sc_op_status (rc);
}
...
get_info_for_key_operation (struct agent_card_info_s *info)
{
  ...
  if (rc || !info->serialno || strncmp (info->serialno, "D27600012401", 12)
      || strlen (info->serialno) != 32 )
    {
      g10_log_error (gettext ("key operation not possible: %s\n"),
                 rc ? g10_errstr ((rc)) : gettext ("not an OpenPGP card"));
      return rc? rc: -1;
    }
  rc = agent_scd_getattr ("KEY-FPR", info);
  if (!rc)
  ...
  if (rc)
    g10_log_error (gettext ("error getting current key info: %s\n"), g10_errstr ((rc)));
  return rc;
}
  ...
  if (*forced_chv1)
    {
      ...
      if (rc)
        {
          g10_log_error ("error clearing forced signature PIN flag: %s\n",
                     g10_errstr ((rc)));
          *forced_chv1 = 0;
        }
    }
  ...
  if (!rc)
    {
      ...
      if (rc)
        {
          g10_log_error ("error checking the PIN: %s\n", g10_errstr ((rc)));
          write_sc_op_status (rc);
        }
  }
  return rc;
...
restore_forced_chv1 (int *forced_chv1)
{
  ...
  if (*forced_chv1)
    {
      rc = agent_scd_setattr ("CHV-STATUS-1", "", 1, ((void*)0));
      if (rc)
        {
          g10_log_error ("error setting forced signature PIN flag: %s\n",
                     g10_errstr ((rc)));
        }
    }
}
...
{
  tty_fprintf (((void*)0), "Signature key ....:");
  print_sha1_fpr (((void*)0), info->fpr1valid? info->fpr1:((void*)0));
  tty_fprintf (((void*)0), "Encryption key....:");
  print_sha1_fpr (((void*)0), info->fpr2valid? info->fpr2:((void*)0));
  tty_fprintf (((void*)0), "Authentication key:");
  print_sha1_fpr (((void*)0), info->fpr3valid? info->fpr3:((void*)0));
  tty_printf ("\n");
}
...
{
  ((void)((keyno >= 0 && keyno <= 3) || (__assert_fail("keyno >= 0 && keyno <= 3", "card-util.c", 1244, __func__),0)));
  ...
  if ((keyno == 1 && info->fpr1valid)
    ...
    {
      tty_printf ("\n");
      g10_log_info ("WARNING: such a key has already been stored on the card!\n");
      tty_printf ("\n");
      if ( !cpr_get_answer_is_yes( "cardedit.genkeys.replace_key",
                                  gettext ("Replace existing key? (y/N) ")))
        return -1;
    }
  return 0;
}
...
show_keysize_warning (void)
{
  ...
  shown = 1;
  tty_printf
    (gettext ("NOTE: There is no guarantee that the card " "supports the requested size.\n" "      If the key generation does not succeed, " "please check the\n" "      documentation of your card to see what " "sizes are allowed.\n"));
  ...
  for (;;)
    {
      prompt = xasprintf
        (keyno == 0?
         gettext ("What keysize do you want for the Signature key? (%u) "):
         keyno == 1?
         gettext ("What keysize do you want for the Encryption key? (%u) "):
         gettext ("What keysize do you want for the Authentication key? (%u) "),
         nbits);
      ...
      if (req_nbits < min_nbits || req_nbits > max_nbits)
        ...
        {
          tty_printf (gettext ("The card will now be re-configured " "to generate a key of %u bits\n"), req_nbits);
...
do_change_keysize (int keyno, unsigned int nbits)
{
  ...
  err = agent_scd_setattr ("KEY-ATTR", args, strlen (args), ((void*)0));
  if (err)
    g10_log_error (gettext ("error changing size of key %d to %u bits: %s\n"),
               keyno+1, nbits, g10_errstr ((err)));
  return err;
}
...
generate_card_keys (void)
{
  ...
  if (info.extcap.ki)
    {
      ...
      answer = cpr_get ("cardedit.genkeys.backup_enc",
                        gettext ("Make off-card backup of encryption key? (Y/n) "));
  ...
  if ( (info.fpr1valid && !fpr_is_zero (info.fpr1))
    ...
    {
      tty_printf ("\n");
      g10_log_info (gettext ("NOTE: keys are already stored on the card!\n"));
      tty_printf ("\n");
      if ( !cpr_get_answer_is_yes ("cardedit.genkeys.replace_keys",
                                   gettext ("Replace existing keys? (y/N) ")))
        {
          agent_release_card_info (&info);
          return;
        }
  ...
  if (!info.disp_name || !*info.disp_name)
    {
      tty_printf ("\n");
      tty_printf (gettext ("Please note that the factory settings of the PINs are\n" "   PIN = `%s'     Admin PIN = `%s'\n" "You should change them using the command --change-pin\n"),
  ...
  if (info.is_v2 && info.extcap.aac)
    {
      ...
      for (keyno = 0; keyno < (sizeof(info.key_attr)/sizeof((info.key_attr)[0])); keyno++)
        {
  ...
  generate_keypair (((void*)0), info.serialno, want_backup? opt.homedir:((void*)0));
...
card_generate_subkey (KBNODE pub_keyblock, KBNODE sec_keyblock)
{
  ...
  tty_printf (gettext ("Please select the type of key to generate:\n"));

  tty_printf (gettext ("   (1) Signature key\n"));
  tty_printf (gettext ("   (2) Encryption key\n"));
  tty_printf (gettext ("   (3) Authentication key\n"));

  for (;;)
    {
      char *answer = cpr_get ("cardedit.genkeys.subkeytype",
                              gettext ("Your selection? "));
      cpr_kill_prompt();
      if (*answer == ('D' - 'A' + 1))
        {
          xfree (answer);
...
card_store_subkey (KBNODE node, int use)
{
  ...
  int rc;
  int keyno, i;
  PKT_secret_key *copied_sk = ((void*)0);
  PKT_secret_key *sk;
  size_t n;
  const char *s;
  int allow_keyno[3];
  ...
  ((void)((node->pkt->pkttype == PKT_SECRET_KEY || node->pkt->pkttype == PKT_SECRET_SUBKEY) || (__assert_fail("node->pkt->pkttype == PKT_SECRET_KEY || node->pkt->pkttype == PKT_SECRET_SUBKEY", "card-util.c", 1528, __func__),0)));
  ...
  if (!((sk->pubkey_algo)==1 || (sk->pubkey_algo)==2 || (sk->pubkey_algo)==3 ) || (!info.is_v2 && nbits != 1024) )
    {
  ...
  allow_keyno[0] = (!use || (use & (1)));
  allow_keyno[1] = (!use || (use & (2)));
  allow_keyno[2] = (!use || (use & (1|8)));

  tty_printf (gettext ("Please select where to store the key:\n"));
  ...
  switch (is_secret_key_protected (sk) )
    ...
    case 0:
      break;
    case -1:
      g10_log_error (gettext ("unknown key protection algorithm\n"));
      goto leave;
    default:
      if (sk->protect.s2k.mode == 1001)
        {
          g10_log_error (gettext ("secret parts of key are not available\n"));
          goto leave;
 }
      if (sk->protect.s2k.mode == 1002)
        {
          g10_log_error (gettext ("secret key already stored on a card\n"));
          goto leave;
 }

      copied_sk = sk = copy_secret_key (((void*)0), sk);
  ...
  if (rc)
    {
      g10_log_error (gettext ("error writing key to card: %s\n"), g10_errstr ((rc)));
      goto leave;
    }
  ...
  sk->skey[i] = gcry_mpi_set_opaque (((void*)0), xstrdup ("dummydata"), 10*8);
  sk->is_protected = 1;
...
static struct
{
...
} cmds[] =
  {
    ...
    { "q" , cmdQUIT , 0, ((void*)0) },
    { "admin" , cmdADMIN , 0, ("show admin commands")},
    { "help" , cmdHELP , 0, ("show this help")},
    { "?" , cmdHELP , 0, ((void*)0) },
    { "list" , cmdLIST , 0, ("list all available data")},
    { "l" , cmdLIST , 0, ((void*)0) },
    { "debug" , cmdDEBUG , 0, ((void*)0) },
    { "name" , cmdNAME , 1, ("change card holder's name")},
    { "url" , cmdURL , 1, ("change URL to retrieve key")},
    { "fetch" , cmdFETCH , 0, ("fetch the key specified in the card URL")},
    { "login" , cmdLOGIN , 1, ("change the login name")},
    { "lang" , cmdLANG , 1, ("change the language preferences")},
    { "sex" , cmdSEX , 1, ("change card holder's sex")},
    { "cafpr" , cmdCAFPR , 1, ("change a CA fingerprint")},
    { "forcesig", cmdFORCESIG, 1, ("toggle the signature force PIN flag")},
    { "generate", cmdGENERATE, 1, ("generate new keys")},
    { "passwd" , cmdPASSWD, 0, ("menu to change or unblock the PIN")},
    { "verify" , cmdVERIFY, 0, ("verify the PIN and list all data")},
    { "unblock" , cmdUNBLOCK,0, ("unblock the PIN using a Reset Code") },

    { "privatedo", cmdPRIVATEDO, 0, ((void*)0) },
    { "readcert", cmdREADCERT, 0, ((void*)0) },
    { "writecert", cmdWRITECERT, 1, ((void*)0) },
    { ((void*)0), cmdINVCMD, 0, ((void*)0) }
  };
# 1758 "card-util.c"
void
card_edit (strlist_t commands)
{
  ...
  for (;;)
    {
      ...
      if (redisplay )
        {
          if (opt.with_colons)
            {
              card_status ((stdout), serialnobuf, (sizeof(serialnobuf)/sizeof((serialnobuf)[0])));
              fflush ((stdout));
            }
          else
            {
              card_status (((void*)0), serialnobuf, (sizeof(serialnobuf)/sizeof((serialnobuf)[0])));
              tty_printf("\n");
            }
          redisplay = 0;
 }
     ...
     if (!have_commands)
              {
                                              ;
  answer = cpr_get_no_help("cardedit.prompt", gettext ("gpg/card> "));
  cpr_kill_prompt();
                           ;
              }
     trim_spaces(answer);
          ...
          if ((p=strchr (answer,' ')))
            {
              *p++ = 0;
              ...
              arg_string = p;
              arg_rest = p;
              while ((*(arg_rest) >= '0' && *(arg_rest) <= '9'))
                arg_rest++;
              while ((*(arg_rest) == ' ' || *(arg_rest) == '\t'))
                arg_rest++;
            }
 ...
 {
          tty_printf ("\n");
          tty_printf (gettext ("Admin-only command\n"));
          continue;
        }
 ...
 case cmdADMIN:
   ...
   if(allow_admin)
     tty_printf(gettext ("Admin commands are allowed\n"));
   else
     tty_printf(gettext ("Admin commands are not allowed\n"));
   break;
