          ...
          int *canceled )
{
 ...
 DEK *dek = ((void*)0);
 u32 keyid[4];
 CIPHER_HANDLE cipher_hd=((void*)0);
 PKT_secret_key *save_sk;

 if( sk->protect.s2k.mode == 1001 ) {
     g10_log_info(gettext ("secret key parts are not available\n"));
     return 1;
 }
 if( sk->protect.algo == 0 )
     g10_log_bug0( "seckey-cert.c" , 57, __FUNCTION__ );
 if( check_cipher_algo( sk->protect.algo ) ) {
     g10_log_info(gettext ("protection algorithm %d%s is not supported\n"),
   sk->protect.algo,sk->protect.algo==1?" (IDEA)":"" );
     return 12;
 }
 if(check_digest_algo(sk->protect.s2k.hash_algo))
   {
     g10_log_info(gettext ("protection digest %d is not supported\n"),
 ...
 xfree(dek);
 save_sk = copy_secret_key( ((void*)0), sk );
 ...
 if( sk->version >= 4 ) {
     ...
     if (!((sk->skey[i]) && (mpi_get_flags (sk->skey[i])&4)))
              p = ((void*)0);
            else
              p = mpi_get_opaque (sk->skey[i], &ndata);
            if (!p)
              g10_log_bug0( "seckey-cert.c" , 99, __FUNCTION__ );
            if ( ndata > 1 )
                csumc = p[ndata-2] << 8 | p[ndata-1];
     data = xmalloc_secure( ndata );
     cipher_decrypt( cipher_hd, data, p, ndata );
     mpi_free( sk->skey[i] ); sk->skey[i] = ((void*)0) ;
     p = data;
            if (sk->protect.sha1chk) {
                ...
                csum = 1;
                if( ndata < 20 )
                    g10_log_error("not enough bytes for SHA-1 checksum\n");
                else {
                    MD_HANDLE h = md_open (2, 1);
                    if (!h)
                        g10_log_bug0( "seckey-cert.c" , 117, __FUNCTION__ );
                    md_write (h, data, ndata - 20);
                    md_final (h);
                    if (!memcmp (md_read (h, 2),
                                 data + ndata - 20, 20) ) {
                ...
                if( ndata < 2 ) {
                    g10_log_error("not enough bytes for checksum\n");
                    sk->csum = 0;
                    csum = 1;
                }
                else {
     ...
     for(i=pubkey_get_npkey(sk->pubkey_algo);
      i < pubkey_get_nskey(sk->pubkey_algo); i++ ) {
                ...
                if (!((sk->skey[i]) && (mpi_get_flags (sk->skey[i])&4)))
                  ...
                  p = mpi_get_opaque (sk->skey[i], &ndata);
                if (!p || !(ndata >= 2))
                  g10_log_bug0( "seckey-cert.c" , 181, __FUNCTION__ );
                ((void)((ndata == ((p[0] << 8 | p[1]) + 7)/8 + 2) || (__assert_fail("ndata == ((p[0] << 8 | p[1]) + 7)/8 + 2", "seckey-cert.c", 182, __func__),0)));
 ...
 if( csum != sk->csum ) {
     copy_secret_key( sk, save_sk );
            passphrase_clear_cache ( keyid, ((void*)0), sk->pubkey_algo );
...
check_secret_key( PKT_secret_key *sk, int n )
{
 ...
 mode=1;
    ...
    if( n < 1 )
 n = (opt.batch && !opt.use_agent)? 1 : 3;

    for(i=0; i < n && rc == 11; i++ ) {
        int canceled = 0;
        const char *tryagain = ((void*)0);
 if (i) {
            tryagain = ("Invalid passphrase; please try again");
            g10_log_info (gettext ("%s ...\n"), gettext (tryagain));
        }
 rc = do_check( sk, tryagain, mode, &canceled );
 if( rc == 11 && is_status_enabled() ) {
     u32 kid[2];
     char buf[50];

     keyid_from_sk( sk, kid );
     sprintf(buf, "%08lX%08lX", (ulong)kid[0], (ulong)kid[1]);
     write_status_text( 25, buf );
 }
...
protect_secret_key( PKT_secret_key *sk, DEK *dek )
{
 ...
 if( check_cipher_algo( sk->protect.algo ) )
  ...
  g10_log_info(gettext ("WARNING: Weak key detected" " - please change passphrase again.\n"));

     sk->protect.ivlen = cipher_get_blocksize( sk->protect.algo );
     ((void)((sk->protect.ivlen <= (sizeof(sk->protect.iv)/sizeof((sk->protect.iv)[0]))) || (__assert_fail("sk->protect.ivlen <= DIM(sk->protect.iv)", "seckey-cert.c", 331, __func__),0)));
     if( sk->protect.ivlen != 8 && sk->protect.ivlen != 16 )
  g10_log_bug0( "seckey-cert.c" , 333, __FUNCTION__ );
  ...
  for(j=0, i = pubkey_get_npkey(sk->pubkey_algo);
   i < pubkey_get_nskey(sk->pubkey_algo); i++, j++ ) {
      ((void)((!((sk->skey[i]) && (mpi_get_flags (sk->skey[i])&4))) || (__assert_fail("!mpi_is_opaque( sk->skey[i] )", "seckey-cert.c", 345, __func__),0)));
      bufarr[j] = mpi_get_buffer( sk->skey[i], &narr[j], ((void*)0) );
      nbits[j] = mpi_get_nbits( sk->skey[i] );
      ndata += narr[j] + 2;
  }
  for( ; j < 6; j++ )
      bufarr[j] = ((void*)0);
  ndata += opt.simple_sk_checksum? 2 : 20;

  data = xmalloc_secure( ndata );
  p = data;
  for(j=0; j < 6 && bufarr[j]; j++ ) {
      p[0] = nbits[j] >> 8 ;
      p[1] = nbits[j];
      p += 2;
      ...
      xfree(bufarr[j]);
                ...
                if (opt.simple_sk_checksum) {
                    g10_log_info (gettext ("generating the deprecated 16-bit checksum" " for secret key protection\n"));
                    ...
                    *p++ = csum;
                    ...
                    MD_HANDLE h = md_open (2, 1);
                    if (!h)
                        g10_log_bug0( "seckey-cert.c" , 377, __FUNCTION__ );
                    md_write (h, data, ndata - 20);
                    md_final (h);
                    memcpy (p, md_read (h, 2), 20);
                    p += 20;
                    md_close (h);
                    sk->csum = csum = 0;
                    sk->protect.sha1chk = 1;
                }
                ((void)((p == data+ndata) || (__assert_fail("p == data+ndata", "seckey-cert.c", 386, __func__),0)));
  ...
  i = pubkey_get_npkey(sk->pubkey_algo);
  sk->skey[i] = mpi_set_opaque(((void*)0), data, ndata );
     }
     else {
  csum = 0;
  for(i=pubkey_get_npkey(sk->pubkey_algo);
   i < pubkey_get_nskey(sk->pubkey_algo); i++ ) {
      ...
      csum += checksum_mpi (sk->skey[i]);
      buffer = mpi_get_buffer( sk->skey[i], &nbytes, ((void*)0) );
      cipher_sync (cipher_hd);
      ((void)((!((sk->skey[i]) && (mpi_get_flags (sk->skey[i])&4))) || (__assert_fail("!mpi_is_opaque (sk->skey[i])", "seckey-cert.c", 407, __func__),0)));
                    data = xmalloc (nbytes+2);
                    nbits = mpi_get_nbits (sk->skey[i]);
                    ((void)((nbytes == (nbits + 7)/8) || (__assert_fail("nbytes == (nbits + 7)/8", "seckey-cert.c", 410, __func__),0)));
                    data[0] = nbits >> 8;
                    data[1] = nbits;
      ...
      xfree( buffer );
                    ...
                    mpi_free (sk->skey[i]);
                    sk->skey[i] = mpi_set_opaque (((void*)0), data, nbytes+2);
  }
  sk->csum = csum;
     }
     sk->is_protected = 1;
