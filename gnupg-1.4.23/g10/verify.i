...
# 90 "verify.c"
 ...
 add_to_strlist( &sl, files[i] );
    rc = proc_signature_packets( ((void*)0), fp, sl, sigfile );
    free_strlist(sl);
    iobuf_close(fp);
    if( (afx.no_openpgp_data && rc == -1) || rc == 58 ) {
 g10_log_error(gettext ("the signature could not be verified.\n" "Please remember that the signature file (.sig or .asc)\n" "should be the first file given on the command line.\n") );
...
verify_one_file( const char *name )
{
 ...
 if( use_armor_filter( fp ) ) {
    ...
    rc = proc_signature_packets( ((void*)0), fp, ((void*)0), name );
    iobuf_close(fp);
    write_status( 39 );
    reset_literals_seen();
    return rc;
...
verify_files( int nfiles, char **files )
{
 ...
 while( fgets(line, (sizeof(line)/sizeof((line)[0])), (stdin)) ) {
  ...
  g10_log_error(gettext ("input line %u too long or missing LF\n"), lno );
  return 1;
     }
