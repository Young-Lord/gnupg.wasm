...
static struct resource_item all_resources[40];
static int used_resources;
static void *primary_keyring=((void*)0);
...
struct keydb_handle {
  ...
  int current;
  int used;
  struct resource_item active[40];
};
...
maybe_create_keyring (char *filename, int force)
{
  ...
  *last_slash_in_filename = '/';
  ...
  if (!iobuf)
    {
      g10_log_error ( gettext ("error creating keyring `%s': %s\n"),
  ...
  if (!opt.quiet)
    g10_log_info (gettext ("keyring `%s' created\n"), filename);

  iobuf_close (iobuf);

  iobuf_ioctl (((void*)0), 2, 0, filename);
  rc = 0;
...
keydb_add_resource (const char *url, int flags, int secret)
{
 ...
 if (!strncmp( resname, "gnupg-ring:", 11) ) {
     ...
     g10_log_error ("invalid key resource URL `%s'\n", url );
     rc = 1;
     goto leave;
 }
    ...
    if (*resname != '/' ) {
 if (strchr(resname, '/') )
     filename = make_filename (resname, ((void*)0));
 else
     filename = make_filename (opt.homedir, resname, ((void*)0));
    }
    else
 ...
 g10_log_error ("unknown type of key resource `%s'\n", url );
 rc = 1;
 goto leave;
  ...
  all_resources[used_resources].u.kr = ((void*)0);
 ...
 break;
      ...
      default:
 g10_log_error ("resource type of `%s' not supported\n", url);
 rc = 1;
 goto leave;
    }
    ...
    if (rc)
      {
        ...
        if ((flags&4) && secret)
          g10_log_info (gettext ("keyblock resource `%s': %s\n"),
                    filename, g10_errstr(rc));
        else
          g10_log_error (gettext ("keyblock resource `%s': %s\n"),
                     filename, g10_errstr(rc));
      }
    else if (secret)
 any_secret = 1;
...
keydb_new (int secret)
{
  ...
  ((void)((used_resources <= 40) || (__assert_fail("used_resources <= MAX_KEYDB_RESOURCES", "keydb.c", 345, __func__),0)));
...
keydb_release (KEYDB_HANDLE hd)
{
    ...
    if (!hd)
        return;
    ((void)((active_handles > 0) || (__assert_fail("active_handles > 0", "keydb.c", 380, __func__),0)));
    active_handles--;
  ...
  for ( ; hd->current >= 0 && hd->current < hd->used; hd->current++)
    {
      switch (hd->active[hd->current].type)
        {
        case KEYDB_RESOURCE_TYPE_NONE:
          g10_log_bug0( "keydb.c" , 672, __FUNCTION__ );
          break;
        case KEYDB_RESOURCE_TYPE_KEYRING:
          if (keyring_is_writable (hd->active[hd->current].token))
            return 0;
  ...
  for (i=0; i < used_resources; i++)
    {
      ...
      switch (all_resources[i].type)
        ...
        case KEYDB_RESOURCE_TYPE_KEYRING:
          ...
          if (rc)
            g10_log_error (gettext ("failed to rebuild keyring cache: %s\n"),
                       g10_errstr (rc));
          break;
        }
    }
        ...
        size_t ndesc, size_t *descindex)
{
    ...
    while (rc == -1 && hd->current >= 0 && hd->current < hd->used) {
        switch (hd->active[hd->current].type) {
          case KEYDB_RESOURCE_TYPE_NONE:
            g10_log_bug0( "keydb.c" , 755, __FUNCTION__ );
            break;
          case KEYDB_RESOURCE_TYPE_KEYRING:
...
keydb_search_first (KEYDB_HANDLE hd)
{
    ...
    return keydb_search2((hd),(&desc),(1),((void*)0));
}
