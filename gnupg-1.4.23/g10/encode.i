...
encode_seskey( DEK *dek, DEK **seskey, byte *enckey )
{
    ...
    ((void)((dek->keylen <= 32) || (__assert_fail("dek->keylen <= 32", "encode.c", 70, __func__),0)));
    if(!*seskey)
      {
 *seskey=xmalloc_clear(sizeof(DEK));
 ...
 make_session_key(*seskey);
    ...
    hd = cipher_open( dek->algo, 2, 1 );
    cipher_setkey( hd, dek->key, dek->keylen );
    cipher_setiv( hd, ((void*)0), 0 );
...
use_mdc(PK_LIST pk_list,int algo)
{
  ...
  if((opt.compliance==CO_RFC1991 || opt.compliance==CO_PGP2) || (opt.compliance==CO_RFC2440))
    return 0;
...
encode_simple( const char *filename, int mode, int use_seskey )
{
    ...
    int do_compress = !(opt.compliance==CO_RFC1991 || opt.compliance==CO_PGP2) && default_compress_algo();
 ...
 iobuf_push_filter( inp, text_filter, &tfx );
    ...
    if ( (opt.compliance==CO_RFC1991 || opt.compliance==CO_PGP2) && use_seskey )
        use_seskey = 0;
 ...
 cfx.dek = passphrase_to_dek( ((void*)0), 0,
 ...
 if( !cfx.dek || !cfx.dek->keylen ) {
     ...
     return rc;
        ...
        if (use_seskey && s2k->mode != 1 && s2k->mode != 3) {
            use_seskey = 0;
            g10_log_info (gettext ("can't use a symmetric ESK packet " "due to the S2K mode\n"));
 ...
 if(opt.verbose)
   g10_log_info(gettext ("using cipher %s\n"),
     cipher_algo_to_string(cfx.dek->algo));

 cfx.dek->use_mdc=use_mdc(((void*)0),cfx.dek->algo);
    }
 ...
 && is_file_compressed(filename, &rc))
      {
        if (opt.verbose)
          g10_log_info(gettext ("`%s' already compressed\n"), filename);
        do_compress = 0;
      }
 ...
 iobuf_push_filter( out, armor_filter, &afx );
    ...
    if( s2k && !(opt.compliance==CO_RFC1991 || opt.compliance==CO_PGP2) ) {
 PKT_symkey_enc *enc = xmalloc_clear( sizeof *enc + seskeylen + 1 );
 enc->version = 4;
 enc->cipher_algo = cfx.dek->algo;
 enc->s2k = *s2k;
        if ( use_seskey && seskeylen ) {
            enc->seskeylen = seskeylen + 1;
...
# 283 "encode.c"
 ...
 push_compress_filter(out,&zfx,default_compress_algo());
    ...
    if (!opt.no_literal) {
 if( (rc = build_packet( out, &pkt )) )
     g10_log_error("build_packet failed: %s\n", g10_errstr(rc) );
    }
    else {
...
setup_symkey(STRING2KEY **symkey_s2k,DEK **symkey_dek)
{
  *symkey_s2k=xmalloc_clear(sizeof(STRING2KEY));
  ...
  (*symkey_s2k)->hash_algo = (opt.s2k_digest_algo?opt.s2k_digest_algo:2);

  *symkey_dek=passphrase_to_dek(((void*)0),0,opt.s2k_cipher_algo,
    *symkey_s2k,2,((void*)0),((void*)0));
...
write_symkey_enc(STRING2KEY *symkey_s2k,DEK *symkey_dek,DEK *dek,IOBUF out)
{
  ...
  if((rc=build_packet(out,&pkt)))
    g10_log_error("build symkey_enc packet failed: %s\n",g10_errstr(rc));
...
encode_crypt( const char *filename, STRLIST remusr, int use_symkey )
{
    ...
    PACKET pkt;
    PKT_plaintext *pt = ((void*)0);
    DEK *symkey_dek = ((void*)0);
    STRING2KEY *symkey_s2k = ((void*)0);
    int rc = 0, rc2 = 0;
    u32 filesize;
    cipher_filter_context_t cfx;
    armor_filter_context_t afx;
    ...
    int do_compress = opt.compress_algo && !(opt.compliance==CO_RFC1991 || opt.compliance==CO_PGP2);
 ...
 if(!(((work_list->pk->pubkey_algo)==1 || (work_list->pk->pubkey_algo)==2 || (work_list->pk->pubkey_algo)==3 ) &&
      nbits_from_pk(work_list->pk)<=2048))
   {
     g10_log_info(gettext ("you can only encrypt to RSA keys of 2048 bits or " "less in --pgp2 mode\n"));
 ...
 g10_log_info(gettext ("reading from `%s'\n"), filename? filename: "[stdin]");
 ...
 cfx.dek->algo = select_algo_from_prefs(pk_list,PREFTYPE_SYM,-1,((void*)0));
 ...
 if( cfx.dek->algo == -1 ) {
     ...
     if( (opt.compliance==CO_PGP2) ) {
       g10_log_info(gettext ("unable to use the IDEA cipher for all of the keys " "you are encrypting to.\n"));
  ...
  select_algo_from_prefs(pk_list,PREFTYPE_SYM,
    opt.def_cipher_algo,((void*)0))!=opt.def_cipher_algo)
 g10_log_info(gettext ("WARNING: forcing symmetric cipher %s (%d)" " violates recipient preferences\n"),
   ...
   opt.def_cipher_algo);
    ...
    if( g10c_debug_mode )
 g10_log_hexdump("DEK is: ", cfx.dek->key, cfx.dek->keylen );
        ...
        IOBUF a, byte *buf, size_t *ret_len)
{
    ...
    if( control == 3 ) {
 g10_log_bug0( "encode.c" , 679, __FUNCTION__ );
    }
    else if( control == 4 ) {
 if( !efx->header_okay ) {
     ...
     if( !opt.def_cipher_algo ) {
  efx->cfx.dek->algo =
    select_algo_from_prefs(efx->pk_list,PREFTYPE_SYM,-1,((void*)0));
  if( efx->cfx.dek->algo == -1 ) {
   ...
   select_algo_from_prefs(efx->pk_list,PREFTYPE_SYM,
     opt.def_cipher_algo,
     ((void*)0))!=opt.def_cipher_algo)
  g10_log_info(gettext ("forcing symmetric cipher %s (%d) " "violates recipient preferences\n"),
...
write_pubkey_enc_from_list( PK_LIST pk_list, DEK *dek, IOBUF out )
{
 ...
 if(opt.throw_keyid && ((opt.compliance==CO_PGP2) || (opt.compliance==CO_PGP6) || (opt.compliance==CO_PGP7) || (opt.compliance==CO_PGP8)))
   {
...
# 787 "encode.c"
 ...
 mpi_free( frame );
 if( rc )
     g10_log_error("pubkey_encrypt failed: %s\n", g10_errstr(rc) );
 else {
     if( opt.verbose ) {
  char *ustr = get_user_id_string_native (enc->keyid);
  g10_log_info(gettext ("%s/%s encrypted for: \"%s\"\n"),
  ...
  xfree(ustr);
     ...
     if( rc )
        g10_log_error("build_packet(pubkey_enc) failed: %s\n", g10_errstr(rc));
 }
 free_pubkey_enc(enc);
 if( rc )
     return rc;
...
encode_crypt_files(int nfiles, char **files, STRLIST remusr)
{
  ...
  if (opt.outfile)
    {
      g10_log_error(gettext ("--output doesn't work for this command\n"));
      return;
    }
      ...
      while ( fgets(line, (sizeof(line)/sizeof((line)[0])), (stdin)) )
        {
          ...
          if (!*line || line[strlen(line)-1] != '\n')
            {
              g10_log_error("input line %u too long or missing LF\n", lno);
              return;
            }
          line[strlen(line)-1] = '\0';
          print_file_status(38, line, 2);
          if ( (rc = encode_crypt(line, remusr, 0)) )
            g10_log_error("encryption of `%s' failed: %s\n",
                      print_fname_stdin(line), g10_errstr(rc) );
          write_status( 39 );
          iobuf_ioctl( ((void*)0), 2, 0, ((void*)0));
        }
    }
  else
    {
