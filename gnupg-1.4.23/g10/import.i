...
parse_import_options(char *str,unsigned int *options,int noisy)
{
  struct parse_options import_opts[]=
    {
      {"import-local-sigs",(1<<0),((void*)0),
       ("import signatures that are marked as local-only")},

      {"repair-pks-subkey-bug",(1<<1),((void*)0),
       ("repair damage from the pks keyserver during import")},

      {"keep-ownertrust", (1<<8), ((void*)0),
       ("do not clear the ownertrust values during import")},

      {"fast-import",(1<<2),((void*)0),
       ("do not update the trustdb after import")},

      {"convert-sk-to-pk",(1<<3),((void*)0),
       ("create a public key when importing a secret key")},

      {"merge-only",(1<<4),((void*)0),
       ("only accept updates to existing keys")},

      {"import-clean",(1<<6),((void*)0),
       ("remove unusable parts from key after import")},

      {"import-minimal",(1<<5)|(1<<6),((void*)0),
       ("remove as much as possible from key after import")},


      {"allow-local-sigs",(1<<0),((void*)0),((void*)0)},
      {"repair-hkp-subkey-bug",(1<<1),((void*)0),((void*)0)},

      {"import-unusable-sigs",0,((void*)0),((void*)0)},
      {"import-clean-sigs",0,((void*)0),((void*)0)},
      {"import-clean-uids",0,((void*)0),((void*)0)},
                      ...
                      int *r_gpgkeys_err)
{
 ...
 for(i=0; once || i < nnames; once=0, i++ ) {
     const char *fname = fnames? fnames[i] : ((void*)0);
     ...
     if( !inp2 )
     ...
     else
       {
         rc = import (inp2, fname, stats, fpr, fpr_len, options,
                             ((void*)0), ((void*)0), r_gpgkeys_err);
         iobuf_close(inp2);

                iobuf_ioctl (((void*)0), 2, 0, (char*)fname);
         if( rc )
    g10_log_error("import from `%s' failed: %s\n", fname,
       g10_errstr(rc) );
       }
 }
    }
...
{
  import_keys_internal (((void*)0), fnames, nnames, stats_handle, ((void*)0), ((void*)0),
                        options, ((void*)0), ((void*)0), ((void*)0));
}
...
import_keys_stream( IOBUF inp, void *stats_handle,
      unsigned char **fpr, size_t *fpr_len,unsigned int options,
             import_filter_t filter, void *filter_arg,
                    int *r_gpgkeys_err)
{
  return import_keys_internal (inp, ((void*)0), 0, stats_handle, fpr, fpr_len,
...
{
    PACKET *pending_pkt = ((void*)0);
    KBNODE keyblock = ((void*)0);
    int rc = 0;
    armor_filter_context_t *afx = ((void*)0);
 ...
 if( keyblock->pkt->pkttype == PKT_PUBLIC_KEY )
   ...
   && keyblock->pkt->pkt.signature->sig_class == 0x20 )
     ...
     g10_log_info( gettext ("skipping block of type %d\n"),
 ...
 if( !(++stats->count % 100) && !opt.quiet )
     g10_log_info(gettext ("%lu keys processed so far\n"), stats->count );
    }
    if( rc == -1 )
 rc = 0;
    else if( rc && rc != 32 )
 g10_log_error( gettext ("error reading `%s': %s\n"), fname, g10_errstr(rc));
...
import_print_stats (void *hd)
{
    ...
    if( !opt.quiet ) {
 g10_log_info(gettext ("Total number processed: %lu\n"), stats->count );
 if( stats->skipped_new_keys )
     g10_log_info(gettext ("      skipped new keys: %lu\n"),
      stats->skipped_new_keys );
 if( stats->no_user_id )
     g10_log_info(gettext ("          w/o user IDs: %lu\n"), stats->no_user_id );
 if( stats->imported || stats->imported_rsa ) {
     g10_log_info(gettext ("              imported: %lu"), stats->imported );
     if( stats->imported_rsa )
  fprintf((stderr), "  (RSA: %lu)", stats->imported_rsa );
     putc('\n', (stderr));
 }
 if( stats->unchanged )
     g10_log_info(gettext ("             unchanged: %lu\n"), stats->unchanged );
 if( stats->n_uids )
     g10_log_info(gettext ("          new user IDs: %lu\n"), stats->n_uids );
 if( stats->n_subk )
     g10_log_info(gettext ("           new subkeys: %lu\n"), stats->n_subk );
 if( stats->n_sigs )
     g10_log_info(gettext ("        new signatures: %lu\n"), stats->n_sigs );
 if( stats->n_revoc )
     g10_log_info(gettext ("   new key revocations: %lu\n"), stats->n_revoc );
 if( stats->secret_read )
     g10_log_info(gettext ("      secret keys read: %lu\n"), stats->secret_read );
 if( stats->secret_imported )
     g10_log_info(gettext ("  secret keys imported: %lu\n"), stats->secret_imported );
 if( stats->secret_dups )
     g10_log_info(gettext (" secret keys unchanged: %lu\n"), stats->secret_dups );
 if( stats->not_imported )
     g10_log_info(gettext ("          not imported: %lu\n"), stats->not_imported );
 if( stats->n_sigs_cleaned)
     g10_log_info(gettext ("    signatures cleaned: %lu\n"),stats->n_sigs_cleaned);
 if( stats->n_uids_cleaned)
     g10_log_info(gettext ("      user IDs cleaned: %lu\n"),stats->n_uids_cleaned);
    }
 ...
 *pending_pkt = ((void*)0);
 ...
 in_cert = 0;
    ...
    while( (rc=dbg_parse_packet( (a), (pkt), "import.c", 426 )) != -1 ) {
 if( rc ) {
     if( rc != 2 ) {
  g10_log_error("read_block: read error: %s\n", g10_errstr(rc) );
  rc = 32;
  goto ready;
     }
     free_packet( pkt );
...
fix_pks_corruption(KBNODE keyblock)
{
  ...
  KBNODE node,last=((void*)0),sknode=((void*)0);
       ...
       keycount>=2 && node->next==((void*)0))
 {
   ...
   if(check_key_signature(keyblock,node,((void*)0)))
     {
  ...
  for (node = keyblock->next; node; node=node->next)
    {
      ...
      if (node->pkt->pkttype == PKT_SIGNATURE
        ...
        {
          rc = check_key_signature (keyblock, node, ((void*)0));
          if (rc && rc != 4 )
            {



              g10_log_info ("key %s: invalid direct key signature removed\n",
                        keystr (keyid));
              delete_kbnode (node);
              count++;
            }
...
{
  g10_log_info(gettext ("WARNING: key %s contains preferences for unavailable\n"),
            keystr_from_pk(pk));


  g10_log_info(gettext ("         algorithms on these user IDs:\n"));
}
...
check_prefs(KBNODE keyblock)
{
  ...
  && node->pkt->pkt.user_id->prefs)
 {
       ...
       if(prefs->type==PREFTYPE_SYM)
  {
   ...
   check_prefs_warning(pk);
        g10_log_info(gettext ("         \"%s\": preference for cipher" " algorithm %s\n"),user,algo?algo:num);
    ...
    if(check_digest_algo(prefs->value))
      {
        ...
        if(!problem)
   check_prefs_warning(pk);
        g10_log_info(gettext ("         \"%s\": preference for digest" " algorithm %s\n"),user,algo?algo:num);
    ...
    if(check_compress_algo(prefs->value))
      {
        ...
        if(!problem)
   check_prefs_warning(pk);
        g10_log_info(gettext ("         \"%s\": preference for compression" " algorithm %s\n"),user,algo?algo:num);
  ...
  if(problem)
    {
      g10_log_info(gettext ("it is strongly suggested that you update" " your preferences and\n"));

      g10_log_info(gettext ("re-distribute this key to avoid potential algorithm" " mismatch problems\n"));


      if(!opt.batch)
 {
   STRLIST sl=((void*)0),locusr=((void*)0);
   size_t fprlen=0;
   byte fpr[20],*p;
   char username[(20*2)+1];
   unsigned int i;
 ...
 g10_log_info(gettext ("you can update your preferences with:" " gpg --edit-key %s updpref save\n"),keystr_from_pk(pk));
     ...
     int from_sk, import_filter_t filter, void *filter_arg)
{
    ...
    if( !node )
 g10_log_bug0( "import.c" , 792, __FUNCTION__ );
 ...
 putc('\n', (stderr));
    ...
    if( !uidnode )
      {
 g10_log_error( gettext ("key %s: no user ID\n"), keystr_from_pk(pk));
 return 0;
      }

    if (filter && filter (keyblock, filter_arg))
      {
        g10_log_error (gettext ("key %s: %s\n"), keystr_from_pk(pk),
                   gettext ("rejected by import filter"));
        return 0;
      }
 ...
 merge_keys_and_selfsig (keyblock);
    ...
    if(options&(1<<6))
      clean_key(keyblock,opt.verbose,options&(1<<5),((void*)0),((void*)0));

    clear_kbnode_flags( keyblock );

    if((options&(1<<1)) && fix_pks_corruption(keyblock)
       && opt.verbose)
      g10_log_info(gettext ("key %s: PKS subkey corruption repaired\n"),
        keystr_from_pk(pk));
 ...
 if( node->pkt->pkttype == PKT_USER_ID && !(node->flag & 1) )
   {
     ...
     node->flag |= 1;
     g10_log_info( gettext ("key %s: accepted non self-signed user ID \"%s\"\n"),
        keystr_from_pk(pk),user);
     xfree(user);
   }

    if( !delete_inv_parts( fname, keyblock, keyid, options ) ) {
        g10_log_error( gettext ("key %s: no valid user IDs\n"), keystr_from_pk(pk));
 if( !opt.quiet )
   g10_log_info(gettext ("this may be caused by a missing self-signature\n"));
 stats->no_user_id++;
 return 0;
    }
    ...
    if( rc && rc != 9 && rc != 53 )
      {
 g10_log_error( gettext ("key %s: public key not found: %s\n"),
     keystr(keyid), g10_errstr(rc));
      }
    else if ( rc && (opt.import_options&(1<<4)) )
      {
 if( opt.verbose )
   g10_log_info( gettext ("key %s: new key - skipped\n"), keystr(keyid));
 rc = 0;
 stats->skipped_new_keys++;
      }
    else if( rc ) {
        ...
        rc = keydb_locate_writable (hd, ((void*)0));
 if (rc) {
     g10_log_error (gettext ("no writable keyring found: %s\n"), g10_errstr (rc));
            keydb_release (hd);
     return 1;
 }
 if( opt.verbose > 1 )
     g10_log_info (gettext ("writing to `%s'\n"), keydb_get_resource_name (hd) );

 rc = keydb_insert_keyblock (hd, keyblock );
        if (rc)
    g10_log_error (gettext ("error writing keyring `%s': %s\n"),
         keydb_get_resource_name (hd), g10_errstr(rc));
 else if (!(opt.import_options & (1<<8)))
   {
 ...
 if( !opt.quiet )
   {
     ...
     g10_log_info( gettext ("key %s: public key \"%s\" imported\n"),
        keystr(keyid),p);
     xfree(p);
   }
 if( is_status_enabled() )
   {
     ...
     write_status_text( 36, us );
     xfree(us);
            print_import_ok (pk,((void*)0), 1);
   }
 stats->imported++;
 if( ((pk->pubkey_algo)==1 || (pk->pubkey_algo)==2 || (pk->pubkey_algo)==3 ) )
     stats->imported_rsa++;
 new_key = 1;
    }
    else {
 ...
 if( cmp_public_keys( pk_orig, pk ) )
   {
     g10_log_error( gettext ("key %s: doesn't match our copy\n"),keystr(keyid));
     goto leave;
   }
 ...
 if( rc )
   {
     g10_log_error (gettext ("key %s: can't locate original keyblock: %s\n"),
         keystr(keyid), g10_errstr(rc));
            keydb_release (hd);
     goto leave;
   }
 ...
 if (rc)
   {
     g10_log_error (gettext ("key %s: can't read original keyblock: %s\n"),
         keystr(keyid), g10_errstr(rc));
            keydb_release (hd);
     goto leave;
   }
 ...
 if(options&(1<<6))
   clean_key(keyblock_orig,opt.verbose,options&(1<<5),
     ...
     if( !opt.quiet )
       {
         char *p=get_user_id_native(keyid);
  if( n_uids == 1 )
    g10_log_info( gettext ("key %s: \"%s\" 1 new user ID\n"),
      keystr(keyid),p);
  else if( n_uids )
    g10_log_info( gettext ("key %s: \"%s\" %d new user IDs\n"),
       keystr(keyid),p,n_uids);
  if( n_sigs == 1 )
    g10_log_info( gettext ("key %s: \"%s\" 1 new signature\n"),
       keystr(keyid), p);
  else if( n_sigs )
    g10_log_info( gettext ("key %s: \"%s\" %d new signatures\n"),
       keystr(keyid), p, n_sigs );
  if( n_subk == 1 )
    g10_log_info( gettext ("key %s: \"%s\" 1 new subkey\n"),
       keystr(keyid), p);
  else if( n_subk )
    g10_log_info( gettext ("key %s: \"%s\" %d new subkeys\n"),
       keystr(keyid), p, n_subk );
  if(n_sigs_cleaned==1)
    g10_log_info(gettext ("key %s: \"%s\" %d signature cleaned\n"),
      keystr(keyid),p,n_sigs_cleaned);
  else if(n_sigs_cleaned)
    g10_log_info(gettext ("key %s: \"%s\" %d signatures cleaned\n"),
      keystr(keyid),p,n_sigs_cleaned);
  if(n_uids_cleaned==1)
    g10_log_info(gettext ("key %s: \"%s\" %d user ID cleaned\n"),
      keystr(keyid),p,n_uids_cleaned);
  else if(n_uids_cleaned)
    g10_log_info(gettext ("key %s: \"%s\" %d user IDs cleaned\n"),
      keystr(keyid),p,n_uids_cleaned);
  xfree(p);
       }
     ...
     stats->n_uids_cleaned +=n_uids_cleaned;
            ...
            if (is_status_enabled ())
                 print_import_ok (pk, ((void*)0),
                                  ((n_uids?2:0)|(n_sigs?4:0)|(n_subk?8:0)));
 }
 else
   {
     if (is_status_enabled ())
       print_import_ok (pk, ((void*)0), 0);

     if( !opt.quiet )
       {
  char *p=get_user_id_native(keyid);
  g10_log_info( gettext ("key %s: \"%s\" not changed\n"),keystr(keyid),p);
  xfree(p);
       }
 ...
 if(fpr)
   {
     xfree(*fpr);
     if(stats->imported==1)
       *fpr=fingerprint_from_pk(pk,((void*)0),fpr_len);
     else
       *fpr=((void*)0);
   }
...
{
  KBNODE secnode,pub_keyblock=((void*)0),ctx=((void*)0);
                   ...
                   import_filter_t filter, void *filter_arg)
{
    ...
    if( !node )
 g10_log_bug0( "import.c" , 1204, __FUNCTION__ );
    ...
    if (filter && filter (keyblock, filter_arg)) {
        g10_log_error (gettext ("secret key %s: %s\n"), keystr_from_sk(sk),
                   gettext ("rejected by import filter"));
        return 0;
    }
 ...
 putc('\n', (stderr));
    ...
    if ((options & (1<<7)))
      {
        g10_log_error (gettext ("importing secret keys not allowed\n"));
        return 0;
      }
 ...
 return 0;
    ...
    if(sk->protect.algo>110)
      {
 g10_log_error(gettext ("key %s: secret key with invalid cipher %d" " - skipped\n"),keystr_from_sk(sk),sk->protect.algo);
...
# 1259 "import.c"
    ...
    if( rc == 17 && !(opt.import_options&(1<<4)) )
      {
        ...
        rc = keydb_locate_writable (hd, ((void*)0));
 if (rc) {
   g10_log_error (gettext ("no default secret keyring: %s\n"), g10_errstr (rc));
   keydb_release (hd);
   return 1;
 }
 rc = keydb_insert_keyblock (hd, keyblock );
 ...
 if( !opt.quiet )
   g10_log_info( gettext ("key %s: secret key imported\n"), keystr_from_sk(sk));
 stats->secret_imported++;
        if (is_status_enabled ())
   print_import_ok (((void*)0), sk, 1|16);
 ...
 if(options&(1<<3))
  ...
  import_one (fname, pub_keyblock, stats,
                            ((void*)0), ((void*)0), opt.import_options, 1,
                            ((void*)0), ((void*)0));
  release_kbnode(pub_keyblock);
       }
   }
 ...
 if(node)
   {
     ...
     release_kbnode(node);
      ...
      {
 g10_log_error( gettext ("key %s: already in secret keyring\n"),
     keystr_from_sk(sk));
 stats->secret_dups++;
        if (is_status_enabled ())
   print_import_ok (((void*)0), sk, 16);



      }
    else
      g10_log_error( gettext ("key %s: secret key not found: %s\n"),
...
import_revoke_cert( const char *fname, KBNODE node, struct stats_s *stats )
{
    ...
    KBNODE onode, keyblock = ((void*)0);
    KEYDB_HANDLE hd = ((void*)0);
    u32 keyid[2];
    int rc = 0;

    ((void)((!node->next) || (__assert_fail("!node->next", "import.c", 1342, __func__),0)));
    ((void)((node->pkt->pkttype == PKT_SIGNATURE) || (__assert_fail("node->pkt->pkttype == PKT_SIGNATURE", "import.c", 1343, __func__),0)));
    ((void)((node->pkt->pkt.signature->sig_class == 0x20) || (__assert_fail("node->pkt->pkt.signature->sig_class == 0x20", "import.c", 1344, __func__),0)));
    ...
    if( rc == 9 )
      {
 g10_log_error(gettext ("key %s: no public key -" " can't apply revocation certificate\n"), keystr(keyid));
 ...
 goto leave;
    ...
    if( rc )
      {
 g10_log_error( gettext ("key %s: invalid revocation certificate" ": %s - rejected\n"), keystr(keyid), g10_errstr(rc));
 ...
 g10_log_error (gettext ("error writing keyring `%s': %s\n"),
    ...
    if( !opt.quiet )
      {
        char *p=get_user_id_native (keyid);
 g10_log_info( gettext ("key %s: \"%s\" revocation certificate imported\n"),
    keystr(keyid),p);
 xfree(p);
      }
    stats->n_revoc++;
...
chk_self_sigs( const char *fname, KBNODE keyblock,
        PKT_public_key *pk, u32 *keyid, int *non_self )
{
    KBNODE n,knode=((void*)0);
    PKT_signature *sig;
    int rc;
    u32 bsdate=0,rsdate=0;
    KBNODE bsnode=((void*)0),rsnode=((void*)0);
 ...
 if( keyid[0] == sig->keyid[0] && keyid[1] == sig->keyid[1] ) {
     ...
     if(!opt.no_sig_cache)
       check_key_signature(keyblock,n,((void*)0));

     if( (((sig)->sig_class & ~3) == 0x10) || ((sig)->sig_class == 0x30) )
       {
  KBNODE unode = find_prev_kbnode( keyblock, n, PKT_USER_ID );
  if( !unode )
    {
      g10_log_error( gettext ("key %s: no user ID for signature\n"),
          keystr(keyid));
      return -1;
    }
  ...
  if(!(unode->flag&1)) {
    rc = check_key_signature( keyblock, n, ((void*)0));
    if( rc )
      {
        if( opt.verbose )
   {
     ...
     g10_log_info( rc == 4 ?
        gettext ("key %s: unsupported public key " "algorithm on user ID \"%s\"\n"):

        gettext ("key %s: invalid self-signature " "on user ID \"%s\"\n"),
                ...
                if ( rc )
                  {
                    if (opt.verbose)
                      g10_log_info (rc == 4 ?
                                gettext ("key %s: unsupported public key algorithm\n"):
                                gettext ("key %s: invalid direct key signature\n"),
                                keystr (keyid));
                    n->flag |= 4;
                  }
              }
  ...
  if( !knode )
    {
      if(opt.verbose)
        g10_log_info( gettext ("key %s: no subkey for key binding\n"),
    keystr(keyid));
      n->flag |= 4;
    }
  else
   ...
   if(opt.verbose)
     g10_log_info(rc == 4 ?
       ...
       gettext ("key %s: invalid subkey binding\n"),
       keystr(keyid));
   n->flag|=4;
        }
      else
     ...
     if(bsnode)
       {
         ...
         if(opt.verbose)
    g10_log_info(gettext ("key %s: removed multiple subkey" " binding\n"),keystr(keyid));
  ...
  if( !knode )
    {
      if(opt.verbose)
        g10_log_info( gettext ("key %s: no subkey for key revocation\n"),
    keystr(keyid));
      n->flag |= 4;
    }
  else
   ...
   if(opt.verbose)
     g10_log_info(rc == 4 ?
       ...
       gettext ("key %s: invalid subkey revocation\n"),
       keystr(keyid));
   n->flag|=4;
        }
      else
   ...
   if(sig->timestamp>=rsdate)
    ...
    if(opt.verbose)
      g10_log_info(gettext ("key %s: removed multiple subkey" " revocation\n"),keystr(keyid));
 ...
 if( node->pkt->pkttype == PKT_USER_ID ) {
  ...
  if( opt.verbose )
    {
      char *p=utf8_to_native(node->pkt->pkt.user_id->name,
        node->pkt->pkt.user_id->len,0);
      g10_log_info( gettext ("key %s: skipped user ID \"%s\"\n"),
         keystr(keyid),p);
      xfree(p);
    }
  delete_kbnode( node );
  ...
  nvalid++;
   ...
   || node->pkt->pkttype == PKT_SECRET_SUBKEY ) {
     if( (node->flag & 2) || !(node->flag & 1) ) {
  if( opt.verbose )
    g10_log_info( gettext ("key %s: skipped subkey\n"),keystr(keyid));
  ...
  while( node->next
   ...
   && node->pkt->pkt.signature->pubkey_algo != 1 )
     delete_kbnode( node );
 else if( node->pkt->pkttype == PKT_SIGNATURE &&
   ...
   seckey_available( node->pkt->pkt.signature->keyid ) )
   {
     ...
     if(opt.verbose)
       g10_log_info( gettext ("key %s: non exportable signature" " (class 0x%02X) - skipped\n"),
   ...
   && node->pkt->pkt.signature->sig_class == 0x20 ) {
     if( uid_seen )
       {
         if(opt.verbose)
    g10_log_info( gettext ("key %s: revocation certificate" " at wrong place - skipped\n"),keystr(keyid));
  ...
  {
    int rc = check_key_signature( keyblock, node, ((void*)0));
    if( rc )
      {
        if(opt.verbose)
   g10_log_info( gettext ("key %s: invalid revocation" " certificate: %s - skipped\n"),
   ...
   {
     if(opt.verbose)
       g10_log_info( gettext ("key %s: subkey signature" " in wrong place - skipped\n"), keystr(keyid));

     delete_kbnode( node );
   }
 else if( node->pkt->pkttype == PKT_SIGNATURE
   && !(((node->pkt->pkt.signature)->sig_class == 0x1f) || (((node->pkt->pkt.signature)->sig_class & ~3) == 0x10) || ((node->pkt->pkt.signature)->sig_class == 0x18) || ((node->pkt->pkt.signature)->sig_class == 0x20) || ((node->pkt->pkt.signature)->sig_class == 0x30) || ((node->pkt->pkt.signature)->sig_class == 0x28)))
   {
     if(opt.verbose)
       g10_log_info(gettext ("key %s: unexpected signature class (0x%02X) -" " skipped\n"),keystr(keyid),
...
collapse_uids( KBNODE *keyblock )
{
 ...
 key=keystr_from_sk(uid1->pkt->pkt.secret_key);
      ...
      g10_log_info(gettext ("key %s: duplicated user ID detected - merged\n"),key);
    }
       ...
       for(inode=keyblock->next;inode;inode=inode->next)
  {
    ...
    if(inode->pkt->pkttype==PKT_SIGNATURE &&
       ...
       inode->pkt->pkt.signature->keyid[1]==keyid[1])
      {
        ...
        rc=get_pubkey_byfprint_fast (((void*)0),sig->revkey[idx]->fpr,
                                                   20);
        if(rc==9 || rc==53)
   {
     ...
     & (1<<3)))
       {
         g10_log_info(gettext ("WARNING: key %s may be revoked:" " fetching revocation key %s\n"),
         ...
         rc=get_pubkey_byfprint_fast (((void*)0),
           sig->revkey[idx]->fpr,
           20);
       }

     if(rc==9 || rc==53)
       g10_log_info(gettext ("WARNING: key %s may be revoked:" " revocation key %s not present.\n"),
       ...
       u32 *keyid, int *n_uids, int *n_sigs, int *n_subk )
{
 ...
 if( node->pkt->pkttype == PKT_USER_ID )
  ...
  if(!opt.quiet)
    {
      char *p=get_user_id_native (keyid);
      g10_log_info(gettext ("key %s: \"%s\" revocation" " certificate added\n"), keystr(keyid),p);
 ...
 if( node->pkt->pkttype == PKT_USER_ID )
  ...
  n2->flag |= 1;
                ++*n_sigs;
  if(!opt.quiet)
    g10_log_info( gettext ("key %s: direct key signature added\n"),
       keystr(keyid));
       }
 }
    }
...
append_uid( KBNODE keyblock, KBNODE node, int *n_sigs,
       const char *fname, u32 *keyid )
{
    KBNODE n, n_where=((void*)0);

    ((void)((node->pkt->pkttype == PKT_USER_ID) || (__assert_fail("node->pkt->pkttype == PKT_USER_ID", "import.c", 2169, __func__),0)));
        ...
        const char *fname, u32 *keyid )
{
    ...
    ((void)((dst->pkt->pkttype == PKT_USER_ID) || (__assert_fail("dst->pkt->pkttype == PKT_USER_ID", "import.c", 2216, __func__),0)));
    ((void)((src->pkt->pkttype == PKT_USER_ID) || (__assert_fail("src->pkt->pkttype == PKT_USER_ID", "import.c", 2217, __func__),0)));
        ...
        const char *fname, u32 *keyid )
{
    ...
    ((void)((dst->pkt->pkttype == PKT_PUBLIC_SUBKEY || dst->pkt->pkttype == PKT_SECRET_SUBKEY) || (__assert_fail("dst->pkt->pkttype == PKT_PUBLIC_SUBKEY || dst->pkt->pkttype == PKT_SECRET_SUBKEY", "import.c", 2258, __func__),0)));
       ...
       const char *fname, u32 *keyid )
{
    ...
    ((void)((node->pkt->pkttype == PKT_PUBLIC_SUBKEY || node->pkt->pkttype == PKT_SECRET_SUBKEY) || (__assert_fail("node->pkt->pkttype == PKT_PUBLIC_SUBKEY || node->pkt->pkttype == PKT_SECRET_SUBKEY", "import.c", 2310, __func__),0)));
  ...
  while((node = walk_kbnode (sec_keyblock, &walkctx, 0)))
    {
      ...
      if (sk->is_protected == 1 && sk->protect.s2k.mode == 1001)
      ...
      else if (sk->is_protected == 1 && sk->protect.s2k.mode == 1002)
        {
          ...
          for (sk->protect.ivlen=0; sk->protect.ivlen < 16 && *s && s[1];
               sk->protect.ivlen++, s += 2)
            if (sk->protect.iv[sk->protect.ivlen] != (((*(s) <= '9'? (*(s)- '0'): *(s) <= 'F'? (*(s)-'A'+10):(*(s)-'a'+10)) * 16) + (*((s)+1) <= '9'? (*((s)+1)- '0'): *((s)+1) <= 'F'? (*((s)+1)-'A'+10):(*((s)+1)-'a'+10))))
              {
                g10_log_info (gettext ("NOTE: a key's S/N does not " "match the card's one\n"));
        ...
        {
          if (node->pkt->pkttype != PKT_SECRET_KEY)
            g10_log_info (gettext ("NOTE: primary key is online and stored on card\n"));
          else
            g10_log_info (gettext ("NOTE: secondary key is online and stored on card\n"));
        }
    }
  ...
  if (!rc)
    {
      ...
      if (rc)
        {
          g10_log_error (gettext ("error reading keyblock: %s\n"), g10_errstr(rc) );
          rc = 1;
        }
      else
        {
