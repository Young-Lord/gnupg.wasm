...
make_control ( byte *buf, int code, int operation )
{
    ...
    if ( sesmarklen > 20 )
        g10_log_bug0( "pipemode.c" , 71, __FUNCTION__ );
          ...
          IOBUF a, byte *buf, size_t *ret_len)
{
    ...
    if( control == 3 ) {
        *ret_len = 0;
        ...
        if ( size <= 30 +(9+2+2) )
            g10_log_bug0( "pipemode.c" , 99, __FUNCTION__ );
        size -= 30 +(9+2+2);
        ...
        while ( n < size ) {
            ...
            int c = ( ((a)->nofast || (a)->d.start >= (a)->d.len )? iobuf_readbyte((a)) : ( (a)->nbytes++, (a)->d.buf[(a)->d.start++] ) );
            if (c == -1) {
                if ( stx->state != STX_init ) {
                    g10_log_error ("EOF encountered at wrong state\n");
                    stx->stop = 1;
                    return -1;
                }
                break;
            ...
            if ( esc ) {
                switch (c) {
                  case '@':
                    ...
                    g10_log_error ("@@ not allowed in current state\n");
                    return -1;
                  case '<':
                    if ( stx->state != STX_init ) {
                        g10_log_error ("nested begin of stream\n");
                        stx->stop = 1;
                        return -1;
                    }
                    ...
                    stx->unarmor_ctx = ((void*)0);
                    break;
                   case '>':
                     if ( stx->state != STX_wait_init ) {
                        g10_log_error ("invalid state for @>\n");
                        stx->stop = 1;
                        return -1;
                    }
                    stx->state = STX_init;
                  ...
                  case 'D':
                    if ( stx->state != STX_wait_operation ) {
                        g10_log_error ("invalid state for operation code\n");
                        stx->stop = 1;
                        return -1;
                    }
                    stx->operation = c;
                  ...
                  case 't':
                    ...
                    if ( stx->state == STX_detached_signature ) {
                        if ( stx->operation != 'B' ) {
                            g10_log_error ("invalid operation for this state\n");
                            stx->stop = 1;
                            return -1;
                        }
                        ...
                        g10_log_error ("invalid state for @t\n");
                        stx->stop = 1;
                        return -1;
                    }
                    break;
                 ...
                 default:
                    g10_log_error ("invalid escape sequence 0x%02x in stream\n",
                               c);
                    stx->stop = 1;
                    return -1;
                }
