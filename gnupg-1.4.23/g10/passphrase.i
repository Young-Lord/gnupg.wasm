...
static char *fd_passwd = ((void*)0);
static char *next_pw = ((void*)0);
static char *last_pw = ((void*)0);
                ...
                const char *cacheid, int *canceled)
{
 ...
 agent_died:
  if ( opt.use_agent )
    {
      pw = agent_get_passphrase (((void*)0), 0, cacheid,
    ...
    {
      g10_log_error(gettext ("can't query passphrase in batch mode\n"));
      pw = ((void*)0);
    }
  else {
    if (tryagain_text)
    ...
    pw = cpr_get_hidden(promptid? promptid : "passphrase.ask",
                        prompt?prompt : gettext ("Enter passphrase: ") );
    tty_kill_prompt();
  }
                   ...
                   const char *tryagain_text, int *canceled)
{
 ...
 tty_printf("\n");
 tty_printf(gettext ("You need a passphrase to unlock the secret key for\n" "user: \"%s\"\n"),p);

 xfree(p);

 if( !get_pubkey( pk, keyid ) ) {
     const char *s = pubkey_algo_to_string( pk->pubkey_algo );
     tty_printf( gettext ("%u-bit %s key, ID %s, created %s"),
  ...
  if(keystrlen()>10)
    {
      tty_printf("\n");
      tty_printf(gettext ("         (subkey on main key ID %s)"),
          keystr(&keyid[2]) );
    }
  else
    tty_printf( gettext (" (main key ID %s)"), keystr(&keyid[2]) );
       }
     tty_printf("\n");
 }
 ...
 next_pw = ((void*)0);
        ...
        pw = agent_get_passphrase ( keyid, mode == 2? 1: 0, ((void*)0),
                                    tryagain_text, ((void*)0), ((void*)0), canceled );
        if (!pw)
          {
            if (!opt.use_agent)
              goto agent_died;
        ...
        if( *pw && mode == 2 )
   {
     ...
     for(i=0;i<opt.passwd_repeat;i++)
       {
  char *pw2 = agent_get_passphrase ( keyid, 2, ((void*)0), ((void*)0), ((void*)0),
         ((void*)0), canceled );
  if (!pw2)
    {
      if (!opt.use_agent)
 ...
 pw = cpr_get_hidden("passphrase.enter", gettext ("Enter passphrase: ") );
 tty_kill_prompt();
...
hash_passphrase( DEK *dek, char *pw, STRING2KEY *s2k, int create )
{
    ...
    ((void)((s2k->hash_algo) || (__assert_fail("s2k->hash_algo", "passphrase.c", 1040, __func__),0)));
    dek->keylen = cipher_get_keylen( dek->algo ) / 8;
    if( !(dek->keylen > 0 && dek->keylen <= (sizeof(dek->key)/sizeof((dek->key)[0]))) )
 g10_log_bug0( "passphrase.c" , 1043, __FUNCTION__ );
