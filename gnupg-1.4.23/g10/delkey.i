...
do_delete_key( const char *username, int secret, int force, int *r_sec_avail )
{
    ...
    *r_sec_avail = 0;
    ...
    rc = desc.mode? keydb_search2((hd),(&desc),(1),((void*)0)):35;
    if (rc) {
 g10_log_error (gettext ("key \"%s\" not found: %s\n"), username, g10_errstr (rc));
 write_status_text( 45, "1" );
 goto leave;
    }
 ...
 goto leave;
    ...
    if( !node ) {
 g10_log_error("Oops; key not found anymore!\n");
 rc = 1;
 goto leave;
    }
  ...
  *r_sec_avail = 1;
  ...
  goto leave;
       ...
       g10_log_error("%s: get secret key: %s\n", username, g10_errstr(rc) );
     else
       rc = 0;
   }
      }
 ...
 g10_log_error(gettext ("can't do this in batch mode\n"));
        g10_log_info (gettext ("(unless you specify the key by fingerprint)\n"));
      }
    else if( opt.batch && opt.answer_yes )
 okay++;
    else if( opt.batch )
      {
 g10_log_error(gettext ("can't do this in batch mode without \"--yes\"\n"));
        g10_log_info (gettext ("(unless you specify the key by fingerprint)\n"));
      }
    else {
        if( secret )
 ...
 yes = cpr_get_answer_is_yes( secret? "delete_key.secret.okay"
        : "delete_key.okay",
         gettext ("Delete this key from the keyring? (y/N) "));
 if( !cpr_enabled() && secret && yes ) {
    ...
    gettext ("This is a secret key! - really delete? (y/N) "));
 }
 if( yes )
     okay++;
    }
 ...
 if (rc) {
     g10_log_error (gettext ("deleting keyblock failed: %s\n"), g10_errstr(rc) );
     goto leave;
 }
        ...
        if (!secret && pk && clear_ownertrusts (pk)) {
          if (opt.verbose)
            g10_log_info (gettext ("ownertrust information cleared\n"));
        }
    }
...
delete_keys( STRLIST names, int secret, int allow_both )
{
  ...
  if ( allow_both ) {
    ...
    g10_log_error(gettext ("there is a secret key for public key \"%s\"!\n"),names->d);

    g10_log_info(gettext ("use option \"--delete-secret-keys\" to delete it first.\n"));

    write_status_text( 45, "2" );
    return rc;
  }
       }

       if(rc) {
  g10_log_error("%s: delete key failed: %s\n", names->d, g10_errstr(rc) );
  return rc;
       }
    }
