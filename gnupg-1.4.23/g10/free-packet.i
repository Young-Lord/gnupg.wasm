...
copy_public_key ( PKT_public_key *d, PKT_public_key *s)
{
 ...
 for(i=0; i < n; i++ )
    ...
    if( !s->revkey && s->numrevkeys )
        g10_log_bug0( "free-packet.c" , 171, __FUNCTION__ );
    if( s->numrevkeys ) {
...
cp_pka_info (const pka_info_t *s)
{
  ...
  d->valid = s->valid;
  d->checked = s->checked;
  d->uri = s->uri? xstrdup (s->uri):((void*)0);
...
copy_signature( PKT_signature *d, PKT_signature *s )
{
 ...
 for(i=0; i < n; i++ )
    ...
    d->pka_info = s->pka_info? cp_pka_info (s->pka_info) : ((void*)0);
...
{
    ((void)((uid->ref > 0) || (__assert_fail("uid->ref > 0", "free-packet.c", 321, __func__),0)));
    if (--uid->ref)
        return;
...
free_compressed( PKT_compressed *zd )
{
 ...
 while( iobuf_read( zd->buf, ((void*)0), 1<<30 ) != -1 )
     ;
    }
    xfree(zd);
}
...
free_encrypted( PKT_encrypted *ed )
{
    if( ed->buf ) {
 if( ed->is_partial ) {
     while( iobuf_read( ed->buf, ((void*)0), 1<<30 ) != -1 )
  ;
 }
 else {
    while( ed->len ) {
        int n = iobuf_read( ed->buf, ((void*)0), ed->len );
        if( n == -1 )
     ed->len = 0;
        else
     ed->len -= n;
...
free_plaintext( PKT_plaintext *pt )
{
    if( pt->buf ) {
 if( pt->is_partial ) {
     while( iobuf_read( pt->buf, ((void*)0), 1<<30 ) != -1 )
  ;
 }
 else {
    while( pt->len ) {
        int n = iobuf_read( pt->buf, ((void*)0), pt->len );
        if( n == -1 )
     pt->len = 0;
        else
     pt->len -= n;
...
free_packet( PACKET *pkt )
{
 ...
 g10_log_debug("free_packet() type=%d\n", pkt->pkttype );
