...
parse_export_options(char *str,unsigned int *options,int noisy)
{
  struct parse_options export_opts[]=
    {
      {"export-local-sigs",(1<<0),((void*)0),
       ("export signatures that are marked as local-only")},
      {"export-attributes",(1<<1),((void*)0),
       ("export attribute user IDs (generally photo IDs)")},
      {"export-sensitive-revkeys",(1<<2),((void*)0),
       ("export revocation keys marked as \"sensitive\"")},
      {"export-reset-subkey-passwd",(1<<3),((void*)0),
       ("remove the passphrase from exported subkeys")},
      {"export-clean",(1<<5),((void*)0),
       ("remove unusable parts from key during export")},
      {"export-minimal",(1<<4)|(1<<5),((void*)0),
       ("remove as much as possible from key during export")},

      {"include-local-sigs",(1<<0),((void*)0),((void*)0)},
      {"include-attributes",(1<<1),((void*)0),((void*)0)},
      {"include-sensitive-revkeys",(1<<2),((void*)0),((void*)0)},

      {"export-unusable-sigs",0,((void*)0),((void*)0)},
      {"export-clean-sigs",0,((void*)0),((void*)0)},
      {"export-clean-uids",0,((void*)0),((void*)0)},
...
do_export( STRLIST users, int secret, unsigned int options )
{
 ...
 iobuf_push_filter( out, armor_filter, &afx );
    ...
    rc = do_export_stream( out, users, secret, ((void*)0), options, &any );
    if( rc || !any )
 iobuf_cancel(out);
    else
 iobuf_close(out);
    ...
    KBNODE *keyblock_out, unsigned int options, int *any )
{
    ...
    KEYDB_SEARCH_DESC *desc = ((void*)0);
    subkey_list_t subkey_list = ((void*)0);
    KEYDB_HANDLE kdbhd;
    STRLIST sl;
 ...
 if((node=find_kbnode(keyblock,PKT_SECRET_KEY)))
   {
     ...
     if( secret == 2 && !sk->is_protected )
       {
  g10_log_info(gettext ("key %s: not protected - skipped\n"),
    keystr(sk_keyid));
  continue;
       }
     ...
     if( secret == 2 && sk->version == 3 )
       {
  g10_log_info(gettext ("key %s: PGP 2.x style key - skipped\n"),
    keystr(sk_keyid));
  continue;
       }
            ...
            if (secret == 2 && sk->is_protected
              ...
              {
  g10_log_info(gettext ("key %s: key material on-card - skipped\n"),
    keystr(sk_keyid));
  continue;
              }
   }
 else
   {
     ...
     if(options&(1<<5))
       clean_key(keyblock,opt.verbose,options&(1<<4),((void*)0),((void*)0));
   }
...
# 431 "export.c"
  ...
  node->pkt->pkt.secret_key->protect.s2k.mode = save_mode;
                     ...
                     && (opt.export_options&(1<<3)))
              {
                ...
                sk = copy_secret_key (((void*)0), sk_save);
                node->pkt->pkt.secret_key = sk;

                g10_log_info (gettext ("about to export an unprotected subkey\n"));
                ...
                if (rc)
                  {
                    ...
                    free_secret_key (sk);
                    g10_log_error (gettext ("failed to unprotect the subkey: %s\n"),
                               g10_errstr (rc));
                    goto leave;
                  }
  ...
  if(!sha1_warned && opt.simple_sk_checksum &&
     ...
     node->pkt->pkt.secret_key->protect.sha1chk)
    {
      ...
      g10_log_info(gettext ("WARNING: secret key %s does not have a " "simple SK checksum\n"),keystr(sk_keyid));
 ...
 rc = 0;
  ...
  leave:
    ...
    xfree(desc);
    keydb_release (kdbhd);
    if(rc || keyblock_out==((void*)0))
      release_kbnode( keyblock );
    if( !*any )
 g10_log_info(gettext ("WARNING: nothing exported\n"));
    return rc;
}
