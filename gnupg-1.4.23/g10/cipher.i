...
write_header( cipher_filter_context_t *cfx, IOBUF a )
{
 ...
 g10_log_fatal("unsupported blocksize %u\n", blocksize );
    ...
    ed.new_ctb = !ed.len && !(opt.compliance==CO_RFC1991 || opt.compliance==CO_PGP2);
    if( cfx->dek->use_mdc ) {
 ed.mdc_method = 2;
 cfx->mdc_hash = md_open( 2, 0 );
 if ( (opt.debug & 512) )
 ...
 g10_log_bug("build_packet(ENCR_DATA) failed\n");
    nprefix = blocksize;
    ...
    cipher_setiv( cfx->cipher_hd, ((void*)0), 0 );
        ...
        IOBUF a, byte *buf, size_t *ret_len)
{
 ...
 ((void)((a) || (__assert_fail("a", "cipher.c", 114, __func__),0)));
 if( !cfx->header ) {
     write_header( cfx, a );
 }
 if( cfx->mdc_hash )
 ...
 if( cfx->mdc_hash ) {
     ...
     ((void)((hashlen == 20) || (__assert_fail("hashlen == 20", "cipher.c", 130, __func__),0)));

     temp[0] = 0xd3;
     temp[1] = 0x14;
     do { if( (cfx->mdc_hash)->bufcount == (cfx->mdc_hash)->bufsize ) md_write( (cfx->mdc_hash), ((void*)0), 0 ); (cfx->mdc_hash)->buffer[(cfx->mdc_hash)->bufcount++] = (temp[0]) & 0xff; } while(0);
     do { if( (cfx->mdc_hash)->bufcount == (cfx->mdc_hash)->bufsize ) md_write( (cfx->mdc_hash), ((void*)0), 0 ); (cfx->mdc_hash)->buffer[(cfx->mdc_hash)->bufcount++] = (temp[1]) & 0xff; } while(0);

     md_final( cfx->mdc_hash );
     hash = md_read( cfx->mdc_hash, 0 );
     memcpy(temp+2, hash, 20);
     cipher_encrypt( cfx->cipher_hd, temp, temp, 22 );
     md_close( cfx->mdc_hash ); cfx->mdc_hash = ((void*)0);
     if( iobuf_write( a, temp, 22 ) )
  g10_log_error("writing MDC packet failed\n" );
 }
