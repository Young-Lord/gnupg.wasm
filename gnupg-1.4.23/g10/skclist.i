               ...
               int unlock, unsigned int use )
{
 ...
 sk->req_usage = use;
 if( (rc = get_seckey_byname( sk, ((void*)0), unlock )) ) {
   free_secret_key( sk ); sk = ((void*)0);
   g10_log_error("no default secret key: %s\n", g10_errstr(rc) );
 }
 else if( !(rc=check_pubkey_algo2(sk->pubkey_algo, use)) )
   {
     ...
     if( random_is_faked() && !is_insecure( sk ) )
       {
  g10_log_info(gettext ("key is not flagged as insecure - " "can't use it with the faked RNG!\n"));
  ...
  sk_list = r;
   ...
   {
     free_secret_key( sk ); sk = ((void*)0);
     g10_log_error("invalid default secret key: %s\n", g10_errstr(rc) );
   }
      }
    else {
 ...
 for(; locusr; locusr = locusr->next ) {
     PKT_secret_key *sk;
       ...
       {
  g10_log_info (gettext ("skipped \"%s\": duplicated\n"), locusr->d );
                continue;
       }
     sk = xmalloc_clear( sizeof *sk );
     sk->req_usage = use;
     if( (rc = get_seckey_byname( sk, locusr->d, 0 )) )
       {
  free_secret_key( sk ); sk = ((void*)0);
  g10_log_error(gettext ("skipped \"%s\": %s\n"),
     locusr->d, g10_errstr(rc) );
       }
            else if ( key_present_in_sk_list(sk_list, sk) == 0) {
                free_secret_key(sk); sk = ((void*)0);
                g10_log_info(gettext ("skipped: secret key already present\n"));
            }
  ...
  if( sk->version == 4 && (use & 1)
      && sk->pubkey_algo == 16 )
    {
      g10_log_info(gettext ("skipped \"%s\": %s\n"),locusr->d,
        gettext ("this is a PGP generated Elgamal key which" " is not secure for signatures!"));
  ...
  g10_log_error("skipped \"%s\": %s\n", locusr->d, g10_errstr(rc) );
    ...
    if( !rc && !sk_list ) {
 g10_log_error("no valid signators\n");
 rc = 16;
    }
