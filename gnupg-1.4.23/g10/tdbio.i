...
take_write_lock (void)
{
  ...
  if (!lockhandle)
    g10_log_fatal ( gettext ("can't create lock for `%s'\n"), db_name );

  if (!is_locked)
    {
      if (dotlock_take (lockhandle, -1) )
        g10_log_fatal ( gettext ("can't lock `%s'\n"), db_name );
      else
        is_locked = 1;
      return 0;
    }
...
write_cache_item( CACHE_CTRL r )
{
    ...
    if( lseek( db_fd, r->recno * 40, (0) ) == -1 ) {
 g10_log_error(gettext ("trustdb rec %lu: lseek failed: %s\n"),
         r->recno, strerror((*__errno_location())) );
 return 22;
    }
    n = write( db_fd, r->data, 40);
    if( n != 40 ) {
 g10_log_error(gettext ("trustdb rec %lu: write failed (n=%d): %s\n"),
         r->recno, n, strerror((*__errno_location())) );
 return 22;
    }
    r->flags.dirty = 0;
    return 0;
...
put_record_into_cache( ulong recno, const char *data )
{
    ...
    for( unused = ((void*)0), r = cache_list; r; r = r->next ) {
 if( !r->flags.used ) {
     if( !unused )
  unused = r;
 }
  ...
  if( memcmp(r->data, data, 40 ) ) {
      r->flags.dirty = 1;
      cache_is_dirty = 1;
  }
     }
     memcpy( r->data, data, 40 );
     return 0;
 }
 if( r->flags.used ) {
     if( r->flags.dirty )
 ...
 ((void)((unused) || (__assert_fail("unused", "tdbio.c", 242, __func__),0)));
 r = unused;
 r->flags.used = 1;
 r->recno = recno;
 memcpy( r->data, data, 40 );
 ...
 if( cache_entries < 10000 ) {
     if( opt.debug && !(cache_entries % 100) )
  g10_log_debug("increasing tdbio cache size\n");
     r = xmalloc( sizeof *r );
     r->flags.used = 1;
     r->recno = recno;
     memcpy( r->data, data, 40 );
 ...
 g10_log_info(gettext ("trustdb transaction too large\n"));
 return 31;
    }
    if( dirty_count ) {
 int n = dirty_count / 5;
 ...
 for( unused = ((void*)0), r = cache_list; r; r = r->next ) {
  ...
  if( !--n )
      break;
        ...
        release_write_lock ();
 ((void)((unused) || (__assert_fail("unused", "tdbio.c", 292, __func__),0)));
 r = unused;
 r->flags.used = 1;
 r->recno = recno;
 memcpy( r->data, data, 40 );
 ...
 return 0;
    ...
    g10_log_bug0( "tdbio.c" , 302, __FUNCTION__ );
}
...
tdbio_sync()
{
 ...
 open_db();
    if( in_transaction )
 g10_log_bug("tdbio: syncing while in transaction\n");
...
tdbio_set_dbname( const char *new_dbname, int create, int *r_nofile)
{
 ...
 initialized = 1;
    ...
    *r_nofile = 0;
    ...
    if(new_dbname==((void*)0))
      fname=make_filename(opt.homedir,"trustdb" "." "gpg", ((void*)0));
    else if (*new_dbname != '/' )
      {
 if (strchr(new_dbname, '/') )
   fname = make_filename (new_dbname, ((void*)0));
 else
   fname = make_filename (opt.homedir, new_dbname, ((void*)0));
      }
    else
    ...
    p = strrchr( fname, '/' );
    ((void)((p) || (__assert_fail("p", "tdbio.c", 532, __func__),0)));
    *p = 0;
    if( access( fname, 0 ) ) {
      try_make_homedir( fname );
      if (access (fname, 0 ))
        g10_log_fatal (gettext ("%s: directory does not exist!\n"), p);
    }
    *p = '/';
    ...
    if( access( fname, 4 ) ) {
        ...
        if( (*__errno_location()) != (44) )
            g10_log_fatal( gettext ("can't access `%s': %s\n"), fname, strerror((*__errno_location())) );
        ...
        umask(oldmask);
        if( !fp )
            g10_log_fatal( gettext ("can't create `%s': %s\n"), fname, strerror((*__errno_location())) );
        fclose(fp);
        db_fd = open( db_name, 02 | 0 );
        if( db_fd == -1 )
            g10_log_fatal( gettext ("can't open `%s': %s\n"), db_name, strerror((*__errno_location())) );

        rc = create_version_record ();
        if( rc )
             g10_log_fatal( gettext ("%s: failed to create version record: %s"),
                                                   fname, g10_errstr(rc));

        if( tdbio_read_record( 0, &rec, 1 ) )
            g10_log_fatal( gettext ("%s: invalid trustdb created\n"), db_name );

        if( !opt.quiet )
            g10_log_info(gettext ("%s: trustdb created\n"), db_name);
    }
...
open_db()
{
  ...
  ((void)((db_fd == -1) || (__assert_fail("db_fd == -1", "tdbio.c", 600, __func__),0)));

  db_fd = open (db_name, 02 | 0 );
  if (db_fd == -1 && ((*__errno_location()) == (2)

                      || (*__errno_location()) == (69)

                      )
      ) {
      db_fd = open (db_name, 00 | 0 );
      if (db_fd != -1)
          g10_log_info (gettext ("NOTE: trustdb not writable\n"));
  }
  if ( db_fd == -1 )
    g10_log_fatal( gettext ("can't open `%s': %s\n"), db_name, strerror((*__errno_location())) );
  ...
  if (tdbio_read_record (0, &rec, 1 ) )
    g10_log_fatal( gettext ("%s: invalid trustdb\n"), db_name );
}
...
create_hashtable( TRUSTREC *vr, int type )
{
    ...
    offset = lseek( db_fd, 0, (2) );
    if( offset == -1 )
 g10_log_fatal("trustdb: lseek to end failed: %s\n", strerror((*__errno_location())) );
    recnum = offset / 40;
    ((void)((recnum) || (__assert_fail("recnum", "tdbio.c", 647, __func__),0)));

    if( !type )
 vr->r.ver.trusthashtbl = recnum;


    n = (256+((40 -2)/4)-1) / ((40 -2)/4);
  ...
  rc = tdbio_write_record( &rec );
  if( rc )
      g10_log_fatal( gettext ("%s: failed to create hashtable: %s\n"),
 ...
 rc = tdbio_sync();
    if( rc )
 g10_log_fatal( gettext ("%s: error updating version record: %s\n"),
        db_name, g10_errstr(rc));
}
  ...
  if( yes_no == -1 )
    {
      ...
      rc = tdbio_read_record( 0, &vr, 1 );
      if( rc )
 g10_log_fatal( gettext ("%s: error reading version record: %s\n"),
     db_name, g10_errstr(rc) );
...
tdbio_write_nextcheck (ulong stamp)
{
 ...
 g10_log_fatal( gettext ("%s: error reading version record: %s\n"),
    ...
    rc = tdbio_write_record( &vr );
    if( rc )
 g10_log_fatal( gettext ("%s: error writing version record: %s\n"),
           db_name, g10_errstr(rc) );
    return 1;
}
...
upd_hashtable( ulong table, byte *key, int keylen, ulong newrecnum )
{
  ...
  next_level:
    msb = key[level];
    hashrec += msb / ((40 -2)/4);
    rc = tdbio_read_record( hashrec, &rec, 10 );
    if( rc ) {
 g10_log_error("upd_hashtable: read failed: %s\n", g10_errstr(rc) );
 return rc;
    }

    item = rec.r.htbl.item[msb % ((40 -2)/4)];
    if( !item ) {
 rec.r.htbl.item[msb % ((40 -2)/4)] = newrecnum;
 rc = tdbio_write_record( &rec );
 if( rc ) {
     g10_log_error("upd_hashtable: write htbl failed: %s\n",
           g10_errstr(rc) );
     return rc;
 }
    }
    else if( item != newrecnum ) {
 lastrec = rec;
 rc = tdbio_read_record( item, &rec, 0 );
 if( rc ) {
     g10_log_error( "upd_hashtable: read item failed: %s\n",
           g10_errstr(rc) );
     return rc;
 }
 ...
 if( rec.rectype == 10 ) {
     ...
     level++;
     if( level >= keylen ) {
  g10_log_error( "hashtable has invalid indirections.\n");
  return 33;
     }
     goto next_level;
 }
 else if( rec.rectype == 11 ) {
  ...
  if( rec.r.hlst.next ) {
      rc = tdbio_read_record( rec.r.hlst.next,
             &rec, 11);
      if( rc ) {
   g10_log_error( "upd_hashtable: read hlst failed: %s\n",
            g10_errstr(rc) );
   return rc;
      }
  }
  ...
  for(i=0; i < ((40 -6)/5); i++ ) {
   ...
   rc = tdbio_write_record( &rec );
   if( rc )
       g10_log_error( "upd_hashtable: write hlst failed: %s\n",
             g10_errstr(rc) );
   return rc;
      }
  }
  if( rec.r.hlst.next ) {
   ...
   return rc;
      ...
      rc = tdbio_write_record( &rec );
      if( rc )
   g10_log_error( "upd_hashtable: write ext hlst failed: %s\n",
         g10_errstr(rc) );
      return rc;
  }
     }
 }
 else if( rec.rectype == 12 ) {
  ...
  return 0;
     ...
     rec.r.hlst.rnum[0] = item;
     rec.r.hlst.rnum[1] = newrecnum;
     rc = tdbio_write_record( &rec );
     if( rc ) {
  g10_log_error( "upd_hashtable: write new hlst failed: %s\n",
        g10_errstr(rc) );
  return rc;
     }

     lastrec.r.htbl.item[msb % ((40 -2)/4)] = rec.recnum;
     rc = tdbio_write_record( &lastrec );
     if( rc )
  g10_log_error( "upd_hashtable: update htbl failed: %s\n",
            g10_errstr(rc) );
     return rc;
 }
 else {
     g10_log_error( "hashtbl %lu: %lu/%d points to an invalid record %lu\n",
         table, hashrec, (msb % ((40 -2)/4)), item);
     list_trustdb(((void*)0));
     return 33;
 }
    }
...
drop_from_hashtable( ulong table, byte *key, int keylen, ulong recnum )
{
  ...
  next_level:
    ...
    rc = tdbio_read_record( hashrec, &rec, 10 );
    if( rc ) {
 g10_log_error("drop_from_hashtable: read failed: %s\n",
       g10_errstr(rc) );
 return rc;
    }
 ...
 return 0;
    ...
    if( item == recnum ) {
 rec.r.htbl.item[msb % ((40 -2)/4)] = 0;
 rc = tdbio_write_record( &rec );
 if( rc )
     g10_log_error("drop_from_hashtable: write htbl failed: %s\n",
           g10_errstr(rc) );
 return rc;
    }
    ...
    rc = tdbio_read_record( item, &rec, 0 );
    if( rc ) {
 g10_log_error( "drop_from_hashtable: read item failed: %s\n",
       g10_errstr(rc) );
 return rc;
    }
 ...
 for(;;) {
  ...
  rc = tdbio_read_record( rec.r.hlst.next,
         &rec, 11);
  if( rc ) {
      g10_log_error( "drop_from_hashtable: read hlst failed: %s\n",
        g10_errstr(rc) );
      return rc;
  }
     }
  ...
  return 0;
    ...
    g10_log_error( "hashtbl %lu: %lu/%d points to wrong record %lu\n",
      table, hashrec, (msb % ((40 -2)/4)), item);
    return 33;
}
# 1017 "tdbio.c"
                  ...
                  const void *cmpdata, TRUSTREC *rec )
{
  ...
  next_level:
    ...
    rc = tdbio_read_record( hashrec, rec, 10 );
    if( rc ) {
 g10_log_error("lookup_hashtable failed: %s\n", g10_errstr(rc) );
 return rc;
    }
 ...
 return -1;
    ...
    rc = tdbio_read_record( item, rec, 0 );
    if( rc ) {
 g10_log_error( "hashtable read failed: %s\n", g10_errstr(rc) );
 return rc;
    }
    if( rec->rectype == 10 ) {
 hashrec = item;
 ...
 for(;;) {
  ...
  if( rec->r.hlst.rnum[i] ) {
      ...
      if( rc ) {
   g10_log_error( "lookup_hashtable: read item failed: %s\n",
             g10_errstr(rc) );
   return rc;
      }
   ...
   *rec = tmp;
   return 0;
     ...
     if( rec->r.hlst.next ) {
  rc = tdbio_read_record( rec->r.hlst.next, rec, 11 );
  if( rc ) {
      g10_log_error( "lookup_hashtable: read hlst failed: %s\n",
        g10_errstr(rc) );
      return rc;
  }
     }
...
tdbio_dump_record( TRUSTREC *rec, FILE *fp )
{
 ...
 break;
      case 254: fprintf(fp, "free, next=%lu\n", rec->r.free.next );
 break;
      case 10:
 fprintf(fp, "htbl,");
 for(i=0; i < ((40 -2)/4); i++ )
...
tdbio_read_record( ulong recnum, TRUSTREC *rec, int expected )
{
 ...
 open_db();
    ...
    if( !buf ) {
 if( lseek( db_fd, recnum * 40, (0) ) == -1 ) {
     g10_log_error(gettext ("trustdb: lseek failed: %s\n"), strerror((*__errno_location())) );
     return 21;
 }
 n = read( db_fd, readbuf, 40);
 if( !n ) {
     return -1;
 }
 else if( n != 40 ) {
     g10_log_error(gettext ("trustdb: read failed (n=%d): %s\n"), n,
       strerror((*__errno_location())) );
     return 21;
 }
 buf = readbuf;
 ...
 g10_log_error("%lu: read expected rec type %d, got %d\n",
 ...
 if( memcmp(buf+1, "gpg", 3 ) ) {
     g10_log_error( gettext ("%s: not a trustdb file\n"), db_name );
     rc = 33;
 }
 p += 2;
 rec->r.ver.version = *p++;
 ...
 if( recnum ) {
     g10_log_error( gettext ("%s: version record with recnum %lu\n"), db_name,
            (ulong)recnum );
     rc = 33;
 }
 else if( rec->r.ver.version != 3 ) {
     g10_log_error( gettext ("%s: invalid file version %d\n"), db_name,
       rec->r.ver.version );
     rc = 33;
 }
 break;
 ...
 break;
      default:
 g10_log_error( "%s: invalid record type %d at recnum %lu\n",
...
tdbio_write_record( TRUSTREC *rec )
{
 ...
 break;
      case 1:
 if( recnum )
     g10_log_bug0( "tdbio.c" , 1302, __FUNCTION__ );
 ...
 *p++ = rec->r.ver.min_cert_level;
 p += 2;
 do { ((byte*)p)[0] = rec->r.ver.created >> 24; ((byte*)p)[1] = rec->r.ver.created >> 16; ((byte*)p)[2] = rec->r.ver.created >> 8; ((byte*)p)[3] = rec->r.ver.created ; } while(0); p += 4;
 do { ((byte*)p)[0] = rec->r.ver.nextcheck >> 24; ((byte*)p)[1] = rec->r.ver.nextcheck >> 16; ((byte*)p)[2] = rec->r.ver.nextcheck >> 8; ((byte*)p)[3] = rec->r.ver.nextcheck ; } while(0); p += 4;
 p += 4;
 p += 4;
 do { ((byte*)p)[0] = rec->r.ver.firstfree >> 24; ((byte*)p)[1] = rec->r.ver.firstfree >> 16; ((byte*)p)[2] = rec->r.ver.firstfree >> 8; ((byte*)p)[3] = rec->r.ver.firstfree ; } while(0); p += 4;
 p += 4;
 do { ((byte*)p)[0] = rec->r.ver.trusthashtbl >> 24; ((byte*)p)[1] = rec->r.ver.trusthashtbl >> 16; ((byte*)p)[2] = rec->r.ver.trusthashtbl >> 8; ((byte*)p)[3] = rec->r.ver.trusthashtbl ; } while(0); p += 4;
 break;

      case 254:
 do { ((byte*)p)[0] = rec->r.free.next >> 24; ((byte*)p)[1] = rec->r.free.next >> 16; ((byte*)p)[2] = rec->r.free.next >> 8; ((byte*)p)[3] = rec->r.free.next ; } while(0); p += 4;
 break;


      case 10:
 for(i=0; i < ((40 -2)/4); i++ ) {
     do { ((byte*)p)[0] = rec->r.htbl.item[i] >> 24; ((byte*)p)[1] = rec->r.htbl.item[i] >> 16; ((byte*)p)[2] = rec->r.htbl.item[i] >> 8; ((byte*)p)[3] = rec->r.htbl.item[i] ; } while(0); p += 4;
 }
 break;

      case 11:
 do { ((byte*)p)[0] = rec->r.hlst.next >> 24; ((byte*)p)[1] = rec->r.hlst.next >> 16; ((byte*)p)[2] = rec->r.hlst.next >> 8; ((byte*)p)[3] = rec->r.hlst.next ; } while(0); p += 4;
 for(i=0; i < ((40 -6)/5); i++ ) {
     do { ((byte*)p)[0] = rec->r.hlst.rnum[i] >> 24; ((byte*)p)[1] = rec->r.hlst.rnum[i] >> 16; ((byte*)p)[2] = rec->r.hlst.rnum[i] >> 8; ((byte*)p)[3] = rec->r.hlst.rnum[i] ; } while(0); p += 4;
 }
 break;
 ...
 *p++ = rec->r.trust.min_ownertrust;
        p++;
 do { ((byte*)p)[0] = rec->r.trust.validlist >> 24; ((byte*)p)[1] = rec->r.trust.validlist >> 16; ((byte*)p)[2] = rec->r.trust.validlist >> 8; ((byte*)p)[3] = rec->r.trust.validlist ; } while(0); p += 4;
 break;

      case 13:
 memcpy( p, rec->r.valid.namehash, 20); p += 20;
 *p++ = rec->r.valid.validity;
 do { ((byte*)p)[0] = rec->r.valid.next >> 24; ((byte*)p)[1] = rec->r.valid.next >> 16; ((byte*)p)[2] = rec->r.valid.next >> 8; ((byte*)p)[3] = rec->r.valid.next ; } while(0); p += 4;
 *p++ = rec->r.valid.full_count;
 *p++ = rec->r.valid.marginal_count;
 break;

      default:
 g10_log_bug0( "tdbio.c" , 1356, __FUNCTION__ );
    }
...
tdbio_new_recnum()
{
 ...
 rc = tdbio_read_record( recnum, &rec, 254 );
 if( rc ) {
     g10_log_error( gettext ("%s: error reading free record: %s\n"),
        db_name, g10_errstr(rc) );
     return rc;
 }
 ...
 rc = tdbio_write_record( &vr );
 if( rc ) {
     g10_log_error( gettext ("%s: error writing dir record: %s\n"),
           db_name, g10_errstr(rc) );
     return rc;
 }
 ...
 rc = tdbio_write_record( &rec );
 if( rc )
     g10_log_fatal(gettext ("%s: failed to zero a record: %s\n"),
           db_name, g10_errstr(rc));
    }
    else {
 offset = lseek( db_fd, 0, (2) );
 if( offset == -1 )
     g10_log_fatal("trustdb: lseek to end failed: %s\n", strerror((*__errno_location())) );
 recnum = offset / 40;
 ((void)((recnum) || (__assert_fail("recnum", "tdbio.c", 1448, __func__),0)));
 ...
 if( lseek( db_fd, recnum * 40, (0) ) == -1 ) {
     g10_log_error(gettext ("trustdb rec %lu: lseek failed: %s\n"),
      recnum, strerror((*__errno_location())) );
     rc = 22;
 }
 else {
     int n = write( db_fd, &rec, 40);
     if( n != 40 ) {
  g10_log_error(gettext ("trustdb rec %lu: write failed (n=%d): %s\n"),
       recnum, n, strerror((*__errno_location())) );
  rc = 22;
     }
 }

 if( rc )
     g10_log_fatal(gettext ("%s: failed to append a record: %s\n"),
        db_name, g10_errstr(rc));
    }
    return recnum ;
}
...
tdbio_invalid(void)
{
  g10_log_error (gettext ("Error: The trustdb is corrupted.\n"));
...
migrate_from_v2 ()
{
  ...
  ottable_used = 0;
# 1553 "tdbio.c"
  if (lseek (db_fd, 0, (0) ) == -1 )
      g10_log_fatal ("migrate_from_v2: lseek failed: %s\n", strerror ((*__errno_location())));
  for (recno=0;;recno++)
    {
      do
      ...
      if (!n)
        break;
      if (n != 40)
        g10_log_fatal ("migrate_vfrom_v2: read error or short read\n");
  ...
  g10_log_info ("found %d ownertrust records\n", ottable_used);
  ...
  for (recno=0;;recno++)
    {
      ...
      if (!n)
        break;
      if (n != 40)
        g10_log_fatal ("migrate_from_v2: read error or short read\n");
  ...
  if (ftruncate (db_fd, 0))
    g10_log_fatal ("can't truncate `%s': %s\n", db_name, strerror ((*__errno_location())) );
  if (create_version_record ())
    g10_log_fatal ("failed to recreate version record of `%s'\n", db_name);
  ...
  for (i=0; i < ottable_used; i++)
    {
      ...
      if (tdbio_write_record (&rec))
        g10_log_fatal ("failed to write trust record of `%s'\n", db_name);
      count++;
    }
  ...
  rc = tdbio_sync ();
  if (rc)
    g10_log_fatal ("failed to sync `%s'\n", db_name);
  g10_log_info ("migrated %d version 2 ownertrusts\n", count);
  xfree (ottable);
}
