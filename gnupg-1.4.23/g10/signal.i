...
init_one_signal (int sig, void (*handler)(int), int check_ign )
{
    ...
    if (check_ign) {
        ...
        sigaction (sig, ((void*)0), &oact );
        if (oact.__sa_handler.sa_handler == ((void (*)(int))-2) )
            return;
    }

    nact.__sa_handler.sa_handler = handler;
    sigemptyset (&nact.sa_mask);
    nact.sa_flags = 0;
    sigaction ( sig, &nact, ((void*)0));
# 78 "signal.c"
}
...
# 124 "signal.c"
    init_one_signal (sig, ((void (*)(int)) 0), 0);
...
init_signals()
{
    init_one_signal (2, got_fatal_signal, 1 );
    init_one_signal (1, got_fatal_signal, 1 );
    init_one_signal (15, got_fatal_signal, 1 );
    init_one_signal (3, got_fatal_signal, 1 );
    init_one_signal (11, got_fatal_signal, 1 );
    init_one_signal (10, got_usr_signal, 0 );
    init_one_signal (13, ((void (*)(int))-2), 0 );
}
...
pause_on_sigusr( int which )
{
    ...
    ((void)((which == 1) || (__assert_fail("which == 1", "signal.c", 159, __func__),0)));
    sigemptyset( &mask );
    sigaddset( &mask, 10 );

    sigprocmask( 0, &mask, &oldmask );
    while( !caught_sigusr1 )
 sigsuspend( &oldmask );
    caught_sigusr1 = 0;
    sigprocmask( 1, &mask, ((void*)0) );
# 176 "signal.c"
}
