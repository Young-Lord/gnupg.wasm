...
static byte sym_prefs[30];
static int nsym_prefs;
static byte hash_prefs[30];
static int nhash_prefs;
static byte zip_prefs[30];
...
print_status_key_created (int letter, PKT_public_key *pk, const char *handle)
{
  ...
  buf = xmalloc (20*2+31 + strlen (handle) + 1);
  ...
  if (*handle)
    {
      *p++ = ' ';
      for (i=0; handle[i] && i < 100; i++)
        *p++ = __isspace((unsigned int)handle[i])? '_':handle[i];
    }
  *p = 0;
  write_status_text ((letter || pk)?58:75,
                     buf);
  xfree (buf);
}
...
{
  print_status_key_created (0, ((void*)0), handle);
}
...
set_one_pref (int val, int type, const char *item, byte *buf, int *nbuf)
{
    ...
    for (i=0; i < *nbuf; i++ )
      if (buf[i] == val)
 {
   g10_log_info (gettext ("preference `%s' duplicated\n"), item);
   return -1;
        }
    ...
    if (*nbuf >= 30)
      {
 if(type==1)
   g10_log_info(gettext ("too many cipher preferences\n"));
 else if(type==2)
   g10_log_info(gettext ("too many digest preferences\n"));
 else if(type==3)
   g10_log_info(gettext ("too many compression preferences\n"));
 else
   g10_log_bug0( "keygen.c" , 277, __FUNCTION__ );
...
{
    byte sym[30], hash[30], zip[30];
    int nsym=0, nhash=0, nzip=0, val, rc=0;
    int mdc=1, modify=0;
    char dummy_string[20*4+1];
 ...
 if (opt.def_preference_list)
 ...
 else
   {
     dummy_string[0]='\0';
# 324 "keygen.c"
     if(!check_cipher_algo(9))
       strcat(dummy_string,"S9 ");
     if(!check_cipher_algo(8))
       strcat(dummy_string,"S8 ");
     if(!check_cipher_algo(7))
       strcat(dummy_string,"S7 ");
     if(!check_cipher_algo(3))
       strcat(dummy_string,"S3 ");
     strcat(dummy_string,"S2 ");
# 341 "keygen.c"
     if((opt.compliance==CO_PGP2) && !check_cipher_algo(1))
       strcat(dummy_string,"S1 ");
# 353 "keygen.c"
     if (!check_digest_algo (8))
       strcat (dummy_string, "H8 ");

     strcat (dummy_string,"H2 ");

     if (!check_digest_algo (9))
       strcat (dummy_string, "H9 ");

     if (!check_digest_algo (10))
       strcat (dummy_string, "H10 ");

     if (!check_digest_algo (11))
 ...
 while((tok=strsep(&prefstring," ,")))
  ...
  if(set_one_pref(val,3,tok,zip,&nzip))
    rc=-1;
       ...
       {
  g10_log_info (gettext ("invalid item `%s' in preference string\n"),tok);
  rc=-1;
       }
   }
 ...
 if(personal)
  ...
  if(nsym==0)
    opt.personal_cipher_prefs=((void*)0);
  else
    {
      int i;
  ...
  if(nhash==0)
    opt.personal_digest_prefs=((void*)0);
  else
    {
      int i;
  ...
  if(nzip==0)
    opt.personal_compress_prefs=((void*)0);
  else
    {
      int i;
...
keygen_upd_std_prefs( PKT_signature *sig, void *opaque )
{
 ...
 delete_sig_subpkt (sig->unhashed, SIGSUBPKT_PREF_HASH);
    ...
    keygen_add_keyserver_url(sig,((void*)0));
...
keygen_add_revkey(PKT_signature *sig, void *opaque)
{
  ...
  buf[0]=revkey->class;
  buf[1]=revkey->algid;
  memcpy(&buf[2],revkey->fpr,20);

  build_sig_subpkt(sig,SIGSUBPKT_REV_KEY,buf,2+20);
              ...
              u32 timestamp)
{
  ...
  rc= make_keysig_packet (&backsig, pk, ((void*)0), sub_pk, sub_sk, 0x19,
                          0, 0, timestamp, 0, ((void*)0), ((void*)0));
  if(rc)
    g10_log_error("make_keysig_packet failed for backsig: %s\n",g10_errstr(rc));
  else
    {
      ...
      do { (&backsig_pkt)->pkttype = 0; (&backsig_pkt)->pkt.generic = ((void*)0); } while(0);
      backsig_pkt.pkttype=PKT_SIGNATURE;
      backsig_pkt.pkt.signature=backsig;
      rc=build_packet(backsig_out,&backsig_pkt);
      free_packet(&backsig_pkt);
      if(rc)
 g10_log_error("build_packet failed for backsig: %s\n",g10_errstr(rc));
      else
 {
   size_t pktlen=0;
   byte *buf=( (backsig_out)->d.buf );
  ...
  g10_log_bug0( "keygen.c" , 840, __FUNCTION__ );
     }
   else
     {
       int mark=1;
  ...
  {
  case 3:
    g10_log_bug0( "keygen.c" , 849, __FUNCTION__ );
    break;
    ...
    struct revocation_key *revkey, u32 timestamp)
{
    ...
    if( opt.verbose )
 g10_log_info(gettext ("writing direct signature\n"));


    node = find_kbnode( pub_root, PKT_PUBLIC_KEY );
    if( !node )
 g10_log_bug0( "keygen.c" , 893, __FUNCTION__ );
    ...
    rc = make_keysig_packet (&sig, pk, ((void*)0), ((void*)0), sk, 0x1F,
                             0, 0, timestamp, 0,
  ...
  unsigned int use, u32 timestamp)
{
    ...
    if( opt.verbose )
 g10_log_info(gettext ("writing self signature\n"));


    node = find_kbnode( pub_root, PKT_USER_ID );
    if( !node )
 g10_log_bug0( "keygen.c" , 933, __FUNCTION__ );
    uid = node->pkt->pkt.user_id;

    node = find_kbnode( pub_root, PKT_PUBLIC_KEY );
    if( !node )
 g10_log_bug0( "keygen.c" , 938, __FUNCTION__ );
    pk = node->pkt->pkt.public_key;
    pk->pubkey_usage = use;
    ...
    rc = make_keysig_packet (&sig, pk, uid, ((void*)0), sk, 0x13,
                             0, 0, timestamp, 0,
                  ...
                  unsigned int use, u32 timestamp)
{
    ...
    if( opt.verbose )
 g10_log_info(gettext ("writing key binding signature\n"));


    node = find_kbnode( pub_root, PKT_PUBLIC_KEY );
    if( !node )
 g10_log_bug0( "keygen.c" , 984, __FUNCTION__ );
 ...
 if( node->pkt->pkttype == PKT_PUBLIC_SUBKEY )
    ...
    if( !sub_pk )
 g10_log_bug0( "keygen.c" , 997, __FUNCTION__ );


    oduap.usage = use;
    oduap.pk = sub_pk;
    rc=make_keysig_packet(&sig, pri_pk, ((void*)0), sub_pk, pri_sk, 0x18,
                          0, 0, timestamp, 0,
 ...
 u32 expireval, int is_subkey)
{
    ...
    ((void)((((algo)==16)) || (__assert_fail("is_ELGAMAL(algo)", "keygen.c", 1038, __func__),0)));

    if (nbits < 1024) {
 nbits = 2048;
 g10_log_info(gettext ("keysize invalid; using %u bits\n"), nbits );
    }
    else if (nbits > 4096) {
        nbits = 4096;
        g10_log_info(gettext ("keysize invalid; using %u bits\n"), nbits );
 ...
 g10_log_info(gettext ("keysize rounded up to %u bits\n"), nbits );
    }

    rc = pubkey_generate( algo, nbits, skey, &factors );
    if( rc ) {
 g10_log_error("pubkey_generate failed: %s\n", g10_errstr(rc) );
 return rc;
    }
    ...
    if( ret_sk )
 *ret_sk = copy_secret_key( ((void*)0), sk );
...
# 1166 "keygen.c"
    ...
    if(qbits!=160)
      g10_log_info("WARNING: some OpenPGP programs can't"
        " handle a DSA key with this digest size\n");

    rc = dsa2_generate( 17, nbits, qbits, skey, &factors );
    if( rc )
      {
 g10_log_error("dsa2_generate failed: %s\n", g10_errstr(rc) );
 return rc;
      }
    ...
    sk->pubkey_algo = pk->pubkey_algo = 17;
         pk->pkey[0] = mpi_copy( skey[0] );
         pk->pkey[1] = mpi_copy( skey[1] );
         pk->pkey[2] = mpi_copy( skey[2] );
 ...
 u32 expireval, int is_subkey)
{
    ...
    ((void)((((algo)==1 || (algo)==2 || (algo)==3 )) || (__assert_fail("is_RSA(algo)", "keygen.c", 1256, __func__),0)));
...
{
  if(flags&1)
    tty_printf("%s ",gettext ("Sign"));

  if(flags&4)
    tty_printf("%s ",gettext ("Certify"));

  if(flags&2)
    tty_printf("%s ",gettext ("Encrypt"));

  if(flags&8)
    tty_printf("%s ",gettext ("Authenticate"));
}
...
{
# 1388 "keygen.c"
  const char *togglers=gettext ("SsEeAaQq");
  char *answer=((void*)0);
  ...
  for(;;)
    {
      tty_printf("\n");
      tty_printf(gettext ("Possible actions for a %s key: "),
   pubkey_algo_to_string(algo));
      print_key_flags(possible);
      tty_printf("\n");
      tty_printf(gettext ("Current allowed actions: "));
      print_key_flags(current);
      tty_printf("\n\n");

      if(possible&1)
 tty_printf(gettext ("   (%c) Toggle the sign capability\n"),
     togglers[0]);
      if(possible&2)
 tty_printf(gettext ("   (%c) Toggle the encrypt capability\n"),
     togglers[2]);
      if(possible&8)
 tty_printf(gettext ("   (%c) Toggle the authenticate capability\n"),
     togglers[4]);

      tty_printf(gettext ("   (%c) Finished\n"),togglers[6]);
      tty_printf("\n");

      xfree(answer);
      answer = cpr_get("keygen.flags",gettext ("Your selection? "));
      cpr_kill_prompt();
...
ask_algo (int addmode, int *r_subkey_algo, unsigned int *r_usage)
{
  ...
  tty_printf(gettext ("Please select what kind of key you want:\n"));
  if (!addmode)
    tty_printf (gettext ("   (%d) RSA and RSA (default)\n"), 1 );
  if ( !addmode )
    tty_printf (gettext ("   (%d) DSA and Elgamal\n"), 2 );

  tty_printf( gettext ("   (%d) DSA (sign only)\n"), 3 );
  tty_printf( gettext ("   (%d) RSA (sign only)\n"), 4 );

  if (addmode)
    {
      tty_printf (gettext ("   (%d) Elgamal (encrypt only)\n"), 5 );
      tty_printf (gettext ("   (%d) RSA (encrypt only)\n"), 6 );
    }
  if (opt.expert)
    {
      tty_printf (gettext ("   (%d) DSA (set your own capabilities)\n"), 7 );
      tty_printf (gettext ("   (%d) RSA (set your own capabilities)\n"), 8 );
    }
  ...
  for (;;)
    {
      ...
      *r_subkey_algo = 0;
      xfree (answer);
      answer = cpr_get ("keygen.algo", gettext ("Your selection? "));
      cpr_kill_prompt ();
...
ask_keysize (int algo, unsigned int primary_keysize)
{
  ...
  if (primary_keysize && !opt.expert)
    {
      ...
      if (algo == 17 && primary_keysize > 3072)
        nbits = 3072;
  ...
  tty_printf(gettext ("%s keys may be between %u and %u bits long.\n"),
  ...
  for(;;)
    {
      ...
      if (for_subkey)
        prompt = xasprintf (gettext ("What keysize do you want " "for the subkey? (%u) "), def);

      else
        prompt = xasprintf (gettext ("What keysize do you want? (%u) "), def);
 ...
 tty_printf(gettext ("%s keysizes must be in the range %u-%u\n"),
     pubkey_algo_to_string(algo),min,max);
      else
 break;
    }

  tty_printf(gettext ("Requested keysize is %u bits\n"), nbits );
  ...
  if( algo == 17 && (nbits % 64) )
    {
      nbits = ((nbits + 63) / 64) * 64;
      if (!autocomp)
        tty_printf (gettext ("rounded up to %u bits\n"), nbits);
    }
  else if( (nbits % 32) )
    {
      nbits = ((nbits + 31) / 32) * 32;
...
ask_expire_interval(u32 timestamp,int object,const char *def_expire)
{
    ...
    switch(object)
      ...
      case 0:
 if(def_expire)
   g10_log_bug0( "keygen.c" , 1721, __FUNCTION__ );
 tty_printf(gettext ("Please specify how long the key should be valid.\n" "         0 = key does not expire\n" "      <n>  = key expires in n days\n" "      <n>w = key expires in n weeks\n" "      <n>m = key expires in n months\n" "      <n>y = key expires in n years\n"));
 ...
 break;
      ...
      case 1:
 if(!def_expire)
   g10_log_bug0( "keygen.c" , 1732, __FUNCTION__ );
 tty_printf(gettext ("Please specify how long the signature should be valid.\n" "         0 = signature does not expire\n" "      <n>  = signature expires in n days\n" "      <n>w = signature expires in n weeks\n" "      <n>m = signature expires in n months\n" "      <n>y = signature expires in n years\n"));
 ...
 break;
      ...
      default:
 g10_log_bug0( "keygen.c" , 1742, __FUNCTION__ );
      }
 ...
 xfree(answer);
 if(object==0)
   answer = cpr_get("keygen.valid",gettext ("Key is valid for? (0) "));
 else
   {
     char *prompt;
     ...
     prompt=xmalloc(strlen(gettext ("Signature is valid for? (%s) "))+strlen(def_expire)+1);
     sprintf(prompt,gettext ("Signature is valid for? (%s) "),def_expire);
 ...
 if( interval == (u32)-1 )
   {
     tty_printf(gettext ("invalid value\n"));
     continue;
   }
 ...
 if( !interval )
   {
            tty_printf((object==0)
                       ? gettext ("Key does not expire at all\n")
                       : gettext ("Signature does not expire at all\n"));
   }
 else
   {
     tty_printf(object==0
         ? gettext ("Key expires at %s\n")
         : gettext ("Signature expires at %s\n"),
...
ask_user_id( int mode )
{
 ...
 tty_printf( gettext ("\n""You need a user ID to identify your key; " "the software constructs the user ID\n""from the Real Name, Comment and Email Address in this form:\n""    \"Heinrich Heine (Der Dichter) <heinrichh@duesseldorf.de>\"\n\n") );
    ...
    uid = aname = acomment = amail = ((void*)0);
    for(;;) {
 char *p;
 int fail=0;
 ...
 if( !aname ) {
     for(;;) {
  xfree(aname);
  aname = cpr_get("keygen.name",gettext ("Real name: "));
  trim_spaces(aname);
  cpr_kill_prompt();
  ...
  if( strpbrk( aname, "<>" ) )
      tty_printf(gettext ("Invalid character in name\n"));
  else if( (*(aname) >= '0' && *(aname) <= '9') )
      tty_printf(gettext ("Name may not start with a digit\n"));
  else if( strlen(aname) < 5 )
      tty_printf(gettext ("Name must be at least 5 characters long\n"));
  else
      break;
     }
 }
 if( !amail ) {
     for(;;) {
  xfree(amail);
  amail = cpr_get("keygen.email",gettext ("Email address: "));
  trim_spaces(amail);
  cpr_kill_prompt();
  if( !*amail || opt.allow_freeform_uid )
      break;
  else if ( !is_valid_mailbox (amail) )
                    tty_printf(gettext ("Not a valid email address\n"));
  else
      break;
     }
 }
 if( !acomment ) {
     for(;;) {
  xfree(acomment);
  acomment = cpr_get("keygen.comment",gettext ("Comment: "));
  trim_spaces(acomment);
  cpr_kill_prompt();
  if( !*acomment )
      break;
  else if( strpbrk( acomment, "()" ) )
      tty_printf(gettext ("Invalid character in comment\n"));
  else
      break;
     }
 }
 ...
 for(p=uid; *p; p++ ) {
     if( *p & 0x80 ) {
  tty_printf(gettext ("You are using the `%s' character set.\n"),
      get_native_charset() );
  break;
     }
 }

 tty_printf(gettext ("You selected this USER-ID:\n    \"%s\"\n\n"), uid);

 if( !*amail && !opt.allow_freeform_uid
     && (strchr( aname, '@' ) || strchr( acomment, '@'))) {
     fail = 1;
     tty_printf(gettext ("Please don't put the email address " "into the real name or the comment\n") );

 }

 for(;;) {
# 1926 "keygen.c"
     const char *ansstr = gettext ("NnCcEeOoQq");

     if( strlen(ansstr) != 10 )
  g10_log_bug0( "keygen.c" , 1929, __FUNCTION__ );
     if( cpr_enabled() ) {
  ...
  answer = cpr_get("keygen.userid.cmd", fail?
    gettext ("Change (N)ame, (C)omment, (E)mail or (Q)uit? ") :
    gettext ("Change (N)ame, (C)omment, (E)mail or (O)kay/(Q)uit? "));
  cpr_kill_prompt();
     }
     if( strlen(answer) > 1 )
  ;
     else if( *answer == ansstr[0] || *answer == ansstr[1] ) {
  xfree(aname); aname = ((void*)0);
  break;
     }
     else if( *answer == ansstr[2] || *answer == ansstr[3] ) {
  xfree(acomment); acomment = ((void*)0);
  break;
     }
     else if( *answer == ansstr[4] || *answer == ansstr[5] ) {
  xfree(amail); amail = ((void*)0);
  break;
     }
     else if( *answer == ansstr[6] || *answer == ansstr[7] ) {
  if( fail ) {
      tty_printf(gettext ("Please correct the error first\n"));
  }
  else {
      xfree(aname); aname = ((void*)0);
      xfree(acomment); acomment = ((void*)0);
...
do_ask_passphrase( STRING2KEY **ret_s2k )
{
    ...
    tty_printf(gettext ("You need a Passphrase to protect your secret key.\n\n") );

    s2k = xmalloc_secure( sizeof *s2k );
    for(;;) {
 s2k->mode = opt.s2k_mode;
 s2k->hash_algo = (opt.s2k_digest_algo?opt.s2k_digest_algo:2);
 dek = passphrase_to_dek( ((void*)0), 0, opt.s2k_cipher_algo, s2k,2,
                                 errtext, ((void*)0));
 if( !dek ) {
     errtext = ("passphrase not correctly repeated; try again");
     tty_printf(gettext ("%s.\n"), gettext (errtext));
 }
 else if( !dek->keylen ) {
     xfree(dek); dek = ((void*)0);
     xfree(s2k); s2k = ((void*)0);
    ...
    u32 expiredate, int is_subkey )
{
  ...
  if( !opt.batch )
    tty_printf(gettext ("We need to generate a lot of random bytes. It is a good idea to perform\n""some other action (type on the keyboard, move the mouse, utilize the\n""disks) during the prime generation; this gives the random number\n""generator a better chance to gain enough entropy.\n") );
  ...
  if( algo == 16 )
   ...
   expiredate, is_subkey);
    ...
    g10_log_bug0( "keygen.c" , 2049, __FUNCTION__ );
...
get_parameter_value( struct para_data_s *para, enum para_name key )
{
    ...
    return (r && *r->u.value)? r->u.value : ((void*)0);
}
 ...
 return 0;
    ...
    while ( (p = strsep (&pn, " \t,")) ) {
        if ( !*p)
            ...
            g10_log_error("%s:%d: invalid usage list\n", fname, r->lnr );
            return -1;
        }
    }
    r->u.usage = use;
        ...
        struct para_data_s *para, enum para_name key)
{
 ...
 goto fail;
  ...
  fail:
  g10_log_error("%s:%d: invalid revocation key\n", fname, r->lnr );
  return -1;
}
...
get_parameter_u32( struct para_data_s *para, enum para_name key )
{
  ...
  if (!r && key == pKEYCREATIONDATE)
    {
      ...
      if (r && *r->u.value)
        {
          ...
          if (!seconds)
            g10_log_error ("invalid creation date in line %d\n", r->lnr );
          else
            {
      ...
      r = get_parameter (para, key);
      ((void)((r) || (__assert_fail("r", "keygen.c", 2255, __func__),0)));
    }
 ...
 return r->u.usage;
    ...
    return (unsigned int)strtoul( r->u.value, ((void*)0), 10 );
}
                     ...
                     struct output_control_s *outctrl, int card )
{
  ...
  if(r)
    {
      ...
      if(check_pubkey_algo2(algo,1))
 {
   g10_log_error("%s:%d: invalid algorithm\n", fname, r->lnr );
   return -1;
 }
    }
  else
    {
      g10_log_error("%s: no Key-Type specified\n",fname);
      return -1;
    }
  ...
  if (!err)
  ...
  else
    {
      ...
      if (r && (r->u.usage & ~openpgp_pk_algo_usage (algo)))
        {
          g10_log_error ("%s:%d: specified Key-Usage not allowed for algo %d\n",
                     fname, r->lnr, algo);
          return -1;
        }
    }
 ...
 return -1;
      else
        {
          ...
          if (r && (r->u.usage & ~openpgp_pk_algo_usage (algo)))
            {
              g10_log_error ("%s:%d: specified Subkey-Usage not allowed"
                         " for algo %d\n", fname, r->lnr, algo);
              return -1;
            }
        }
      ...
      if( s1 || s2 || s3 )
 {
  ...
  if(!have_user_id)
    {
      g10_log_error("%s: no User-ID specified\n",fname);
      return -1;
    }
  ...
  if(s1)
    {
      ...
      spec=parse_keyserver_uri(s1,1,((void*)0),0);
      if(spec)
 {
 ...
 {
   g10_log_error("%s:%d: invalid keyserver url\n", fname, r->lnr );
   return -1;
 }
    }
  ...
  if( r && *r->u.value ) {
    ...
    ((void)((dek) || (__assert_fail("dek", "keygen.c", 2457, __func__),0)));
  ...
  if (r && *r->u.value)
    {
      ...
      if (!seconds)
 {
   g10_log_error ("%s:%d: invalid creation date\n", fname, r->lnr );
   return -1;
 }
      r->u.creation = seconds;
      r->key = pKEYCREATIONDATE;
  ...
  if( r && *r->u.value )
    {
      ...
      if (seconds == (u32)(-1))
 {
   g10_log_error("%s:%d: invalid expire date\n", fname, r->lnr );
   return -1;
 }
      r->u.expire = seconds;
      r->key = pKEYEXPIRE;
  ...
  if( !!outctrl->pub.newfname ^ !!outctrl->sec.newfname ) {
    g10_log_error("%s:%d: only one ring name is set\n", fname, outctrl->lnr );
    return -1;
  }
...
read_parameter_file( const char *fname )
{
 ...
 for( p = line; __isspace(*(byte*)p); p++ )
     ;
 if( !*p || *p == '#' )
     continue;
 keyword = p;
 if( *keyword == '%' ) {
     for( ; !__isspace(*(byte*)p); p++ )
  ;
     if( *p )
  *p++ = 0;
     for( ; __isspace(*(byte*)p); p++ )
  ...
  g10_log_info("skipping control `%s' (%s)\n", keyword, value );
 ...
 iobuf_close( outctrl.sec.stream );
        ...
        if (outctrl.pub.fname)
          iobuf_ioctl (((void*)0), 2, 0, (char*)outctrl.pub.fname);
        if (outctrl.sec.fname)
          iobuf_ioctl (((void*)0), 2, 0, (char*)outctrl.sec.fname);
                  ...
                  const char *backup_encryption_dir)
{
  ...
  int both = 0;
  u32 expire;
  struct para_data_s *para = ((void*)0);
  struct para_data_s *r;
  struct output_control_s outctrl;
  ...
  if( !uid )
    {
      g10_log_error(gettext ("Key generation canceled.\n"));
      release_parameter_list( para );
      return;
    }
  ...
  dek = card_serialno? ((void*)0) : do_ask_passphrase( &s2k );
  if( dek )
    {
      r = xmalloc_clear( sizeof *r );
      r->key = pPASSPHRASE_DEK;
                  ...
                  PKT_secret_key **r_sk_protected)
{
  ...
  ((void)((nskey <= 6 && npkey < nskey) || (__assert_fail("nskey <= PUBKEY_MAX_NSKEY && npkey < nskey", "keygen.c", 2948, __func__),0)));
  ...
  rc = pubkey_generate (algo, nbits, sk->skey, ((void*)0));
  if (rc)
    {
      g10_log_error("pubkey_generate failed: %s\n", g10_errstr(rc) );
      goto leave;
  ...
  if (r_sk_unprotected)
    *r_sk_unprotected = copy_secret_key (((void*)0), sk);
...
do_generate_keypair (struct para_data_s *para,struct output_control_s *outctrl,
       int card)
{
    KBNODE pub_root = ((void*)0);
    KBNODE sec_root = ((void*)0);
    PKT_secret_key *pri_sk = ((void*)0), *sub_sk = ((void*)0);
    const char *s;
    struct revocation_key *revkey;
    int rc;
    int did_sub = 0;
    ...
    if( outctrl->dryrun )
      {
 g10_log_info("dry-run mode - key generation skipped\n");
 return;
      }
 ...
 if( outctrl->pub.newfname ) {
     iobuf_close(outctrl->pub.stream);
     outctrl->pub.stream = ((void*)0);
            if (outctrl->pub.fname)
              iobuf_ioctl (((void*)0), 2, 0, (char*)outctrl->pub.fname);
     xfree( outctrl->pub.fname );
     outctrl->pub.fname = outctrl->pub.newfname;
     outctrl->pub.newfname = ((void*)0);
  ...
  g10_log_error(gettext ("can't create `%s': %s\n"), outctrl->pub.newfname,
 ...
 if( outctrl->sec.newfname ) {
     ...
     iobuf_close(outctrl->sec.stream);
     outctrl->sec.stream = ((void*)0);
            if (outctrl->sec.fname)
              iobuf_ioctl (((void*)0), 2, 0, (char*)outctrl->sec.fname);
     xfree( outctrl->sec.fname );
     outctrl->sec.fname = outctrl->sec.newfname;
     outctrl->sec.newfname = ((void*)0);
 ...
 ((void)((outctrl->pub.stream) || (__assert_fail("outctrl->pub.stream", "keygen.c", 3096, __func__),0)));
 ((void)((outctrl->sec.stream) || (__assert_fail("outctrl->sec.stream", "keygen.c", 3097, __func__),0)));
        if( opt.verbose ) {
            g10_log_info(gettext ("writing public key to `%s'\n"), outctrl->pub.fname );
            if (card)
              g10_log_info (gettext ("writing secret key stub to `%s'\n"),
                        outctrl->sec.fname);
            else
              g10_log_info(gettext ("writing secret key to `%s'\n"), outctrl->sec.fname );
        }
    }
# 3115 "keygen.c"
    start_tree(&pub_root);
...
# 3129 "keygen.c"
   ...
   timestamp,
      ...
      {
        rc = gen_card_key (1, 1, 1, pub_root, sec_root, ((void*)0),
                           &timestamp,
                           get_parameter_u32 (para, pKEYEXPIRE), para);
        if (!rc)
          {
            pri_sk = sec_root->next->pkt->pkt.secret_key;
            ((void)((pri_sk) || (__assert_fail("pri_sk", "keygen.c", 3148, __func__),0)));
          }
      }
...
# 3178 "keygen.c"
    if (!rc && card && get_parameter (para, pAUTHKEYTYPE))
      {
        rc = gen_card_key (1, 3, 0, pub_root, sec_root, ((void*)0),
                           &timestamp,
                           get_parameter_u32 (para, pKEYEXPIRE), para);
            ...
            if ((s = get_parameter_value (para, pBACKUPENCDIR)))
              {
                ...
                rc = gen_card_key_with_backup (1, 2, 0,
                                               pub_root, sec_root,
                                               timestamp,
              ...
              rc = gen_card_key (1, 2, 0, pub_root, sec_root,
     ((void*)0),
                                 &timestamp,
                                 get_parameter_u32 (para, pKEYEXPIRE), para);
          }
 ...
 if( rc )
     g10_log_error("can't write public key: %s\n", g10_errstr(rc) );
 if( !rc ) {
     rc = write_keyblock( outctrl->sec.stream, sec_root );
     if( rc )
  g10_log_error("can't write secret key: %s\n", g10_errstr(rc) );
 }
 ...
 KEYDB_HANDLE sec_hd = keydb_new (1);
        ...
        rc = keydb_locate_writable (pub_hd, ((void*)0));
        if (rc)
         g10_log_error (gettext ("no writable public keyring found: %s\n"),
                       g10_errstr (rc));

        if (!rc) {
            rc = keydb_locate_writable (sec_hd, ((void*)0));
            if (rc)
                g10_log_error (gettext ("no writable secret keyring found: %s\n"),
                           g10_errstr (rc));
        }
     ...
     rc = keydb_insert_keyblock (pub_hd, pub_root);
            if (rc)
                g10_log_error (gettext ("error writing public keyring `%s': %s\n"),
     ...
     rc = keydb_insert_keyblock (sec_hd, sec_root);
            if (rc)
                g10_log_error (gettext ("error writing secret keyring `%s': %s\n"),
                           keydb_get_resource_name (pub_hd), g10_errstr(rc));
        }
 ...
 if (!rc) {
            int no_enc_rsa =
                get_parameter_algo(para, pKEYTYPE) == 1
                && get_parameter_uint( para, pKEYUSAGE )
                && !(get_parameter_uint( para,pKEYUSAGE) & 2);
     ...
     update_ownertrust (pk,
          ((get_ownertrust (pk) & ~15)
    | 6 ));

     if (!opt.batch) {
                tty_printf(gettext ("public and secret key created and signed.\n") );
  tty_printf("\n");
  list_keyblock(pub_root,0,1,((void*)0));
            }


     if( !opt.batch
  && ( get_parameter_algo( para, pKEYTYPE ) == 17
                     || no_enc_rsa )
  && !get_parameter( para, pSUBKEYTYPE ) )
     {
  tty_printf(gettext ("Note that this key cannot be used for " "encryption.  You may want to use\n" "the command \"--edit-key\" to generate a " "subkey for this purpose.\n") );
    ...
    if( rc ) {
 if( opt.batch )
     g10_log_error("key generation failed: %s\n", g10_errstr(rc) );
 else
     tty_printf(gettext ("Key generation failed: %s\n"), g10_errstr(rc) );
        print_status_key_not_created ( get_parameter_value (para, pHANDLE) );
    }
    else {
        PKT_public_key *pk = find_kbnode (pub_root,
...
generate_subkeypair( KBNODE pub_keyblock, KBNODE sec_keyblock )
{
 ...
 goto leave;
    ...
    pri_sk = copy_secret_key( ((void*)0), node->pkt->pkt.secret_key );

    timestamp = make_timestamp();
    if( pri_sk->timestamp > timestamp ) {
 ulong d = pri_sk->timestamp - timestamp;
 g10_log_info( d==1 ? gettext ("key has been created %lu second " "in future (time warp or clock problem)\n")

         : gettext ("key has been created %lu seconds " "in future (time warp or clock problem)\n"), d );
 ...
 if( !opt.ignore_time_conflict ) {
    ...
    if (pri_sk->version < 4) {
        g10_log_info (gettext ("NOTE: creating subkeys for v3 keys " "is not OpenPGP compliant\n"));
 ...
 goto leave;
    ...
    algo = ask_algo (1, ((void*)0), &use);
    ((void)((algo) || (__assert_fail("algo", "keygen.c", 3430, __func__),0)));
    nbits = ask_keysize (algo, 0);
    expire = ask_expire_interval(timestamp,0,((void*)0));
    if( !cpr_enabled() && !cpr_get_answer_is_yes("keygen.sub.okay",
        gettext ("Really create? (y/N) ")))
 goto leave;
 ...
 rc = write_keybinding (sec_keyblock, pub_keyblock, pri_sk, sub_sk,
    ...
    if (!rc)
      {
 okay = 1;
        write_status_text (58, "S");
      }

  leave:
    if( rc )
 g10_log_error(gettext ("Key generation failed: %s\n"), g10_errstr(rc) );
    xfree( passphrase );
    xfree( s2k );
                          ...
                          int keyno, const char *serialno)
{
  int okay=0, rc=0;
  KBNODE node;
  PKT_secret_key *pri_sk = ((void*)0), *sub_sk;
  int algo;
  unsigned int use;
  ...
  ((void)((keyno >= 1 && keyno <= 3) || (__assert_fail("keyno >= 1 && keyno <= 3", "keygen.c", 3493, __func__),0)));
...
write_keyblock( IOBUF out, KBNODE node )
{
  for( ; node ; node = node->next )
    {
      if(!((node)->private_flag & 1))
 {
   int rc = build_packet( out, node->pkt );
   if( rc )
     {
       g10_log_error("build_packet(%d) failed: %s\n",
              ...
              u32 *timestamp, u32 expireval, struct para_data_s *para)
{
  ...
  ((void)((algo == 1) || (__assert_fail("algo == PUBKEY_ALGO_RSA", "keygen.c", 3631, __func__),0)));


  rc = agent_scd_genkey (&info, keyno, 1, ((void*)0), timestamp);
# 3645 "keygen.c"
  if (rc)
    {
      g10_log_error ("key generation failed: %s\n", g10_errstr ((rc)));
  ...
  if ( !info.n || !info.e )
    {
      g10_log_error ("communication error with SCD\n");
      mpi_free (info.n);
      mpi_free (info.e);
      return (1);
    }
  ...
  if (*timestamp != info.created_at)
    g10_log_info ("Note that the key does not use the suggested creation date\n");
  *timestamp = info.created_at;
  ...
  sk->skey[2] = mpi_set_opaque (((void*)0), xstrdup ("dummydata"), 10);
  sk->is_protected = 1;
  ...
  if (s)
    {
      for (sk->protect.ivlen=0; sk->protect.ivlen < 16 && *s && s[1];
        ...
        sk->protect.iv[sk->protect.ivlen] = (((*(s) <= '9'? (*(s)- '0'): *(s) <= 'F'? (*(s)-'A'+10):(*(s)-'a'+10)) * 16) + (*((s)+1) <= '9'? (*((s)+1)- '0'): *((s)+1) <= 'F'? (*((s)+1)-'A'+10):(*((s)+1)-'a'+10)));
    }
                          ...
                          const char *backup_dir)
{
  ...
  if (rc)
    {
      g10_log_error (gettext ("storing key onto card failed: %s\n"), g10_errstr (rc));
  ...
  sk->skey[i] = mpi_set_opaque (((void*)0), xstrdup ("dummydata"), 10);
  sk->is_protected = 1;
  sk->protect.s2k.mode = 1002;
  s = get_parameter_value (para, pSERIALNO);
  ((void)((s) || (__assert_fail("s", "keygen.c", 3766, __func__),0)));
  for (sk->protect.ivlen=0; sk->protect.ivlen < 16 && *s && s[1];
       sk->protect.ivlen++, s += 2)
    sk->protect.iv[sk->protect.ivlen] = (((*(s) <= '9'? (*(s)- '0'): *(s) <= 'F'? (*(s)-'A'+10):(*(s)-'a'+10)) * 16) + (*((s)+1) <= '9'? (*((s)+1)- '0'): *((s)+1) <= 'F'? (*((s)+1)-'A'+10):(*((s)+1)-'a'+10)));
...

  {
    ...
    keyid_from_sk (sk, ((void*)0));
    sprintf (name_buffer,"sk_%08lX%08lX.gpg",
             (ulong)sk->keyid[0], (ulong)sk->keyid[1]);

    fname = make_filename (backup_dir, name_buffer, ((void*)0));
    oldmask = umask (077);
    ...
    if (!fp)
      {
 g10_log_error (gettext ("can't create backup file `%s': %s\n"),
                   fname, strerror((*__errno_location())) );
        xfree (fname);
        free_secret_key (sk_unprotected);
        free_secret_key (sk_protected);
    ...
    if (rc)
      {
        g10_log_error("build packet failed: %s\n", g10_errstr(rc) );
        iobuf_cancel (fp);
      }
    else
      {
        ...
        g10_log_info (gettext ("NOTE: backup of card key saved to `%s'\n"), fname);

        fingerprint_from_sk (sk, array, &n);
        p = fprbuf = xmalloc (20*2 + 1 + 1);
        for (i=0; i < n ; i++, p += 2)
          sprintf (p, "%02X", array[i]);
        *p++ = ' ';
        *p = 0;

        write_status_text_and_buffer (81,
                                      fprbuf,
                                      fname, strlen (fname),
                                      0);
        xfree (fprbuf);
...
{
  int rc;
  unsigned char *rsa_n = ((void*)0);
  unsigned char *rsa_e = ((void*)0);
  unsigned char *rsa_p = ((void*)0);
  unsigned char *rsa_q = ((void*)0);
  unsigned int rsa_n_len, rsa_e_len, rsa_p_len, rsa_q_len;
  unsigned char *sexp = ((void*)0);
  unsigned char *p;
  char numbuf[55], numbuf2[50];

  ((void)((((sk->pubkey_algo)==1 || (sk->pubkey_algo)==2 || (sk->pubkey_algo)==3 )) || (__assert_fail("is_RSA (sk->pubkey_algo)", "keygen.c", 3888, __func__),0)));
  ((void)((!sk->is_protected) || (__assert_fail("!sk->is_protected", "keygen.c", 3889, __func__),0)));


  rsa_n = mpi_get_secure_buffer (sk->skey[0], &rsa_n_len, ((void*)0));
  rsa_e = mpi_get_secure_buffer (sk->skey[1], &rsa_e_len, ((void*)0));
  rsa_p = mpi_get_secure_buffer (sk->skey[3], &rsa_p_len, ((void*)0));
  rsa_q = mpi_get_secure_buffer (sk->skey[4], &rsa_q_len, ((void*)0));
  ...
  rc = agent_scd_writekey (keyno, ((void*)0), sexp, p - sexp);
