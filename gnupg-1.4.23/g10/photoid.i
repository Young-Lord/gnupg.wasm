...
generate_photo_id(PKT_public_key *pk,const char *photo_name)
{
  ...
  if(photo_name && *photo_name)
    filename=make_filename(photo_name,(void *)((void*)0));
  else
    {
      ...
      if(filename==((void*)0))
 {
   ...
   tempname=cpr_get("photoid.jpeg.add",
      gettext ("Enter JPEG filename for photo ID: "));

                           ;

   filename=make_filename(tempname,(void *)((void*)0));
   ...
   if(strlen(filename)==0)
     goto scram;
      ...
      if(!file)
 {
   g10_log_error(gettext ("unable to open JPEG file `%s': %s\n"),
 ...
 {
   tty_printf( gettext ("This JPEG is really large (%d bytes) !\n"),len);
   if(!cpr_get_answer_is_yes("photoid.jpeg.size",
       gettext ("Are you sure you want to use it? (y/N) ")))
   {
     iobuf_close(file);
     xfree(filename);
     filename=((void*)0);
 ...
 {
   g10_log_error(gettext ("`%s' is not a JPEG file\n"),filename);
   xfree(photo);
   photo=((void*)0);
   xfree(filename);
   filename=((void*)0);
   continue;
      ...
      make_attribute_uidname(uid, 71);



      if(opt.command_fd==-1)
 {
   show_photos(uid->attribs,uid->numattribs,pk,((void*)0),uid);
   switch(cpr_get_answer_yes_no_quit("photoid.jpeg.okay",
      gettext ("Is this photo correct (y/N/q)? ")))
     {
     case -1:
       goto scram;
     case 0:
...
{
# 282 "photoid.c"
  if(path_access("xloadimage",1)==0)
    return "xloadimage -fork -quiet -title 'KeyID 0x%k' stdin";
  else if(path_access("eog",1)==0)
    return "eog %i";
  else if(path_access("display",1)==0)
     ...
     PKT_user_id *uid)
{
 ...
 if(!*opt.photo_viewer)
   {
     g10_log_info(gettext ("no photo viewer set\n"));
     goto fail;
   }
 ...
 name=xmalloc(16+strlen(".")+
       strlen(image_type_to_string(args.imagetype,0))+1);
# 355 "photoid.c"
 sprintf(name,"%08lX%08lX" "." "%s",(ulong)kid[0],(ulong)kid[1],
  image_type_to_string(args.imagetype,0));


 if(exec_write(&spawn,((void*)0),command,name,1,1)!=0)
   {
     xfree(name);
     goto fail;
   }
 ...
 fail:
  g10_log_error(gettext ("unable to display photo ID!\n"));
