...
hash_public_key( MD_HANDLE md, PKT_public_key *pk )
{
  ...
  if(npkey==0 && pk->pkey[0] && ((pk->pkey[0]) && (mpi_get_flags (pk->pkey[0])&4)))
    {
 ...
 n += 2 + nn[i];
  ...
  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (0x99) & 0xff; } while(0);
  ...
  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (pk->version) & 0xff; } while(0);

  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (pk->timestamp >> 24) & 0xff; } while(0);
  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (pk->timestamp >> 16) & 0xff; } while(0);
  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (pk->timestamp >> 8) & 0xff; } while(0);
  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (pk->timestamp) & 0xff; } while(0);
 ...
 days=(u16)((pk->expiredate - pk->timestamp) / 86400L);
      ...
      do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (days >> 8) & 0xff; } while(0);
      do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (days) & 0xff; } while(0);
    }

  do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (pk->pubkey_algo) & 0xff; } while(0);
  ...
  if(npkey==0 && pk->pkey[0] && ((pk->pkey[0]) && (mpi_get_flags (pk->pkey[0])&4)))
    ...
    for(i=0; i < npkey; i++ )
      {
 do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (nb[i]>>8) & 0xff; } while(0);
 do { if( (md)->bufcount == (md)->bufsize ) md_write( (md), ((void*)0), 0 ); (md)->buffer[(md)->bufcount++] = (nb[i]) & 0xff; } while(0);
        if (pp[i])
...
keystrlen(void)
{
  switch(opt.keyid_format)
    ...
    default:
      g10_log_bug0( "keyid.c" , 180, __FUNCTION__ );
    }
}
...
keystr(u32 *keyid)
{
 ...
 sprintf(keyid_str,"0x%08lX",(ulong)keyid[1]);
    ...
    default:
      g10_log_bug0( "keyid.c" , 214, __FUNCTION__ );
    }
...
keystr_from_desc(KEYDB_SEARCH_DESC *desc)
{
 ...
 return keystr(keyid);
    ...
    default:
      g10_log_bug0( "keyid.c" , 258, __FUNCTION__ );
    }
}
...
keyid_from_sk( PKT_secret_key *sk, u32 *keyid )
{
  ...
  if( sk->keyid[0] || sk->keyid[1] )
    ...
    {
      if( ((sk->pubkey_algo)==1 || (sk->pubkey_algo)==2 || (sk->pubkey_algo)==3 ) )
 {
   lowbits = pubkey_get_npkey(sk->pubkey_algo) ?
     mpi_get_keyid( sk->skey[0], keyid ) : 0;
   sk->keyid[0]=keyid[0];
   sk->keyid[1]=keyid[1];
 }
      else
...
keyid_from_pk( PKT_public_key *pk, u32 *keyid )
{
      ...
      if( ((pk->pubkey_algo)==1 || (pk->pubkey_algo)==2 || (pk->pubkey_algo)==3 ) )
 {
   lowbits = pubkey_get_npkey(pk->pubkey_algo) ?
     mpi_get_keyid( pk->pkey[0], keyid ) : 0 ;
   pk->keyid[0] = keyid[0];
   pk->keyid[1] = keyid[1];
 }
      else
...
keyid_from_fingerprint( const byte *fprint, size_t fprint_len, u32 *keyid )
{
 ...
 if( rc ) {
     g10_log_error("Oops: keyid_from_fingerprint: no pubkey\n");
     keyid[0] = 0;
     keyid[1] = 0;
 }
 else
...
fingerprint_from_pk( PKT_public_key *pk, byte *array, size_t *ret_len )
{
 ...
 if( ((pk->pubkey_algo)==1 || (pk->pubkey_algo)==2 || (pk->pubkey_algo)==3 ) )
   {
     ...
     if( pubkey_get_npkey( pk->pubkey_algo ) > 1 ) {
       p = buf = mpi_get_buffer( pk->pkey[0], &n, ((void*)0) );
       md_write( md, p, n );
       xfree(buf);
       p = buf = mpi_get_buffer( pk->pkey[1], &n, ((void*)0) );
       md_write( md, p, n );
       xfree(buf);
     }
     md_final(md);
 ...
 ((void)((len <= 20) || (__assert_fail("len <= MAX_FINGERPRINT_LEN", "keyid.c", 684, __func__),0)));
 if( !array )
     array = xmalloc( len );
 memcpy(array, dp, len );
 ...
 if( ((sk->pubkey_algo)==1 || (sk->pubkey_algo)==2 || (sk->pubkey_algo)==3 ) )
   {
     ...
     if( pubkey_get_npkey( sk->pubkey_algo ) > 1 ) {
       p = buf = mpi_get_buffer( sk->skey[0], &n, ((void*)0) );
       md_write( md, p, n );
       xfree(buf);
       p = buf = mpi_get_buffer( sk->skey[1], &n, ((void*)0) );
       md_write( md, p, n );
       xfree(buf);
     }
     md_final(md);
 ...
 if(md)
   {
     dp = md_read( md, 0 );
     len = md_digest_length( md_get_algo( md ) );
     ((void)((len <= 20) || (__assert_fail("len <= MAX_FINGERPRINT_LEN", "keyid.c", 743, __func__),0)));
     if( !array )
       array = xmalloc( len );
     memcpy(array, dp, len );
     md_close(md);
