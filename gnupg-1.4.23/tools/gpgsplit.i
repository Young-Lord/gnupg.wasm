...
main( int argc, char **argv )
{
  ...
  pargs.argv = &argv;
  pargs.flags= 1;
  while (optfile_parse( ((void*)0), ((void*)0), ((void*)0), &pargs, opts))
    {
      switch (pargs.r_opt)
        {
...
create_filename (int pkttype)
{
  ...
  ((void)((pkttype < 1000 && pkttype >= 0) || (__assert_fail("pkttype < 1000 && pkttype >= 0", "gpgsplit.c", 208, __func__),0)));
  partno++;
...
{
  if ( putc ((0xc0 | (pkttype & 0x3f)), fp) == (-1) )
    return -1;
  ...
  if (len < 192)
    ...
    {
      len -= 192;
      if (putc ((len/256)+192, fp) == (-1))
        return -1;
      if (putc ((len%256), fp) == (-1))
        return -1;
    }
  else
    {
...
handle_zlib(int algo,FILE *fpin,FILE *fpout)
{
      ...
      if (zs.avail_in < inbufsize)
 {
   ...
   zs.avail_in = n;
      ...
      if (!zinit_done)
 {
   zrc = (algo == 1? inflateInit2_((&zs), (-13), "1.1.4", sizeof(z_stream))
   : inflateInit_((&zs), "1.1.4", sizeof(z_stream)));
   if (zrc != 0)
     {
             ...
             int pkttype, int partial, unsigned char *hdr, size_t hdrlen)
{
  ...
  if (opt_no_split)
    ...
    {
      if (opt_verbose)
        g10_log_info ("writing `%s'\n", outname);
      fpout = fopen (outname, "wb");
      if (!fpout)
        {
          g10_log_error ("error creating `%s': %s\n", outname, strerror((*__errno_location())));
  ...
  if (opt_secret_to_public
      && (pkttype == PKT_SECRET_KEY || pkttype == PKT_SECRET_SUBKEY))
    {
      ...
      if (!len)
        {
          g10_log_error ("error calcualting public key length\n");
          g10_exit (1);
        }
      if ( (hdr[0] & 0x40) )
        {
      ...
      if (partial == 1)
        {
          if (first )
            {
              c = pktlen;
              ((void)((c >= 224 && c < 255) || (__assert_fail("c >= 224 && c < 255", "gpgsplit.c", 613, __func__),0)));
              first = 0;
            }
          else if ((c = getc (fpin)) == (-1) )
            goto read_error;
          else
            hdr[hdrlen++] = c;
        ...
        {
          ((void)((!pktlen) || (__assert_fail("!pktlen", "gpgsplit.c", 664, __func__),0)));
  ...
  {
    g10_log_error("invalid compression algorithm (%d)\n",c);
    goto read_error;
  }
            }
          else
 ...
 write_error:
  g10_log_error ("error writing `%s': %s\n", outname, strerror ((*__errno_location())));
  if (!opt_no_split)
    fclose (fpout);
  return 2;
...
split_packets (const char *fname)
{
  ...
  if ( rc > 0 )
    ...
    g10_log_error ("error reading `%s': %s\n", fname, strerror ((*__errno_location())));
  else
    g10_log_error ("premature EOF while reading `%s'\n", fname );
