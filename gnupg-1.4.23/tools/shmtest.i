...
main(int argc, char **argv)
{
 ...
 strcat(p, argv[i] );
    ...
    xfree( p );
    if( !fp )
 g10_log_error("popen failed: %s\n", strerror((*__errno_location())));

    while ( fgets (buf, sizeof (buf) - 1, fp ) != ((void*)0) ) {
 ...
 if( len >= 9 && !memcmp( buf, "[GNUPG:] ", 9 ) ) {
  ...
  if( word==1 && !strcmp(p,"SHM_INFO") ) {
   ...
   g10_log_error("duplicate SHM_INFO ignored\n" );
  }
      ...
      *p2++ = 0;
      ...
      if( !strcmp(p, "pv" ) ) {
   if( atoi(p2) != 1 )
       g10_log_fatal("invalid protocol version %d\n", val );
   is_info = 2;
      }
   ...
   shm_id = val;
      ...
      g10_log_info("Status: %s\n", p);
     }
     if( is_info ) {
  if( is_info < 2 )
      g10_log_fatal("SHM info without protocol version\n");
  if( serverpid == -1 )
      g10_log_fatal("SHM info without server's pid\n");
  if( shm_id == -1 )
      g10_log_fatal("SHM info without id\n");
  g10_log_info("Shared memory info: server=%d shm_id=%d\n",
           serverpid, shm_id);
  area = shmat( shm_id, 0, 0 );
  if( area == (void*)-1 )
      g10_log_fatal("attach to shared memory failed: %s\n",
           strerror((*__errno_location())));
     }
 }
 else
