  ...
  switch(c) {
   ...
   do_add();
    ...
    case '-':
      if( (c=getc((stdin))) == '-' )
   do_dec();
      else if( (0 ? isdigit(c) : ((unsigned)(c)-'0') < 10) || (c >='A' && c <= 'F') ) {
   state = 1;
   ungetc(c, (stdin));
   strbuf[0] = '-';
   strbuf[1] = '0';
   ...
   mpi_free(stack[i]), stack[i] = ((void*)0);
      stackidx = 0;
      break;
    case 'p':
      if( !stackidx )
