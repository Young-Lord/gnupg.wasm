...

{
 ...
 g10_log_debug("mpi_alloc(%u)\n", nlimbs*(8*(4)) );




    a = xmalloc( sizeof *a );
    a->d = nlimbs? mpi_alloc_limb_space( nlimbs, 0 ) : ((void*)0);
...

{
    ...
    if( memory_debug_mode )
 g10_log_debug("mpi_alloc_secure(%u)\n", nlimbs*(8*(4)) );




    a = xmalloc( sizeof *a );
    a->d = nlimbs? mpi_alloc_limb_space( nlimbs, 1 ) : ((void*)0);
...

{
    ...
    if( memory_debug_mode )
 g10_log_debug("mpi_alloc_limb_space(%u)\n", (unsigned)len*8 );
# 146 "mpiutil.c"
...

{
 ...
 return;
    if( memory_debug_mode )
 g10_log_debug("mpi_free_limb_space of size %lu\n", (ulong)m_size(a)*8 );
# 187 "mpiutil.c"
    xfree(a);
}
...

{
 ...
 mpi_free_limb_space(a->d);
    ...
    if( a->flags & ~7 )
 g10_log_bug("invalid flag value in mpi\n");
    xfree(a);
}
...
mpi_set_secure( MPI a )
{
 ...
 return;
    ...
    ap = a->d;
    if( !a->nlimbs ) {
 ((void)((!ap) || (__assert_fail("!ap", "mpiutil.c", 278, __func__),0)));
 return;
    }
    ...
    do { mpi_size_t _i; for( _i = 0; _i < (a->nlimbs); _i++ ) (bp)[_i] = (ap)[_i]; } while(0);
    a->d = bp;
...
{
    if( !(a->flags & 4) )
 g10_log_bug("mpi_get_opaque on normal mpi\n");
    if( len )
 *len = a->nbits;
    return a->d;
}
...

{
 ...
 b = mpi_set_opaque( ((void*)0), p, a->nbits );
    }
    else if( a ) {
 ...
 b = ((a) && (mpi_get_flags (a)&1))? mpi_alloc_secure( a->nlimbs )
       : mpi_alloc( a->nlimbs );
...
mpi_set( MPI w, MPI u)
{
    ...
    do { if( (w)->alloced < (usize) ) mpi_resize((w), (usize)); } while(0);
    wp = w->d;
    up = u->d;
    do { mpi_size_t _i; for( _i = 0; _i < (usize); _i++ ) (wp)[_i] = (up)[_i]; } while(0);
    w->nlimbs = usize;
    w->nbits = u->nbits;
    w->flags = u->flags;
    w->sign = usign;
...
mpi_set_cond( MPI w, MPI u, unsigned long set)
{
    ...
    if (w->alloced != u->alloced)
      g10_log_bug ("mpi_set_cond: different sizes\n");
...
{
    do { if( (w)->alloced < (1) ) mpi_resize((w), (1)); } while(0);
    w->d[0] = u;
    w->nlimbs = u? 1:0;
    w->sign = 0;
    w->nbits = 0;
