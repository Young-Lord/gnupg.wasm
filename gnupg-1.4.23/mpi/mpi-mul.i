...
mpi_mul_2exp( MPI w, MPI u, unsigned long cnt)
{
 ...
 do { mpi_size_t _i; for( _i = (usize)-1; _i >= 0; _i--) (wp + limb_cnt)[_i] = (u->d)[_i]; } while(0);
    }



    do { int _i; for( _i = 0; _i < (limb_cnt); _i++ ) (wp)[_i] = 0; } while (0);
...
mpi_mul( MPI w, MPI u, MPI v)
{
 ...
 usize = v->nlimbs;
 usign = v->sign;
 usecure = ((v) && (mpi_get_flags (v)&1));
 up = v->d;
 vsize = u->nlimbs;
 vsign = u->sign;
 vsecure = ((u) && (mpi_get_flags (u)&1));
 vp = u->d;
    }
    else {
 usize = u->nlimbs;
 usign = u->sign;
 usecure = ((u) && (mpi_get_flags (u)&1));
 up = u->d;
 vsize = v->nlimbs;
 vsign = v->sign;
 vsecure = ((v) && (mpi_get_flags (v)&1));
 vp = v->d;
    }
    ...
    wsize = usize + vsize;
    if ( !((w) && (mpi_get_flags (w)&1)) && (((u) && (mpi_get_flags (u)&1)) || ((v) && (mpi_get_flags (v)&1))) ) {
 ...
 if( wp == up || wp == vp ) {
     wp = mpi_alloc_limb_space( wsize, ((w) && (mpi_get_flags (w)&1)) );
     assign_wp = 1;
 }
 else {
     mpi_resize(w, wsize );
 ...
 if( wp == up ) {
  ...
  vp = up;
     ...
     do { mpi_size_t _i; for( _i = 0; _i < (usize); _i++ ) (up)[_i] = (wp)[_i]; } while(0);
 }
 else if( wp == vp ) {

     vp = tmp_limb = mpi_alloc_limb_space( vsize, vsecure );

     do { mpi_size_t _i; for( _i = 0; _i < (vsize); _i++ ) (vp)[_i] = (wp)[_i]; } while(0);
 }
    }
 ...
 wsize -= cy? 0:1;
    ...
    if( assign_wp ) {
     ...
     do { mpi_size_t _i; for( _i = 0; _i < (wsize); _i++ ) (tmp_wp)[_i] = (wp)[_i]; } while(0);
