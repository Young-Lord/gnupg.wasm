...
mpi_write( IOBUF out, MPI a )
{
    ...
    if( nbits > 16384 )
 g10_log_bug("mpi_encode: mpi too large (%u bits)\n", nbits);

    iobuf_writebyte(out,(nbits >>8));
    iobuf_writebyte(out,(nbits));

    p = buf = mpi_get_buffer( a, &n, ((void*)0) );
...

{
 ...
 goto leave;
    ...
    if( nbits > 16384 ) {
 g10_log_error("mpi too large for this implementation (%u bits)\n", nbits);
 goto leave;
    }
 ...
 for(; i < (4); i++ ) {
     ...
     a <<= 8;
     a |= ( ((inp)->nofast || (inp)->d.start >= (inp)->d.len )? iobuf_readbyte((inp)) : ( (inp)->nbytes++, (inp)->d.buf[(inp)->d.start++] ) ) & 0xff; nread++;
 }
 i = 0;
 val->d[j-1] = a;
    }
  ...
  leave:
    *ret_nread = nread;
    return val;
  overflow:
    g10_log_error ("mpi larger than indicated length (%u bytes)\n", nmax);
    *ret_nread = nread;
    return val;
}
...
mpi_read_from_buffer(byte *buffer, unsigned int *ret_nread, int secure)
{
 ...
 goto leave;
    ...
    if( nbits > 16384 ) {
 g10_log_info ("mpi too large (%u bits)\n", nbits);
 goto leave;
    }
    buffer += 2;
    nread = 2;
 ...
 for(; i < (4); i++ ) {
  ...
  g10_log_info ("mpi larger than buffer\n");
                mpi_free (val);
                val = ((void*)0);
                goto leave;
          }
...
mpi_fromstr(MPI val, const char *str)
{
 ...
 for(; i < (4); i++ ) {
  ...
  c1 = *str++;
     ((void)((c1) || (__assert_fail("c1", "mpicoder.c", 237, __func__),0)));
     c2 = *str++;
     ((void)((c2) || (__assert_fail("c2", "mpicoder.c", 239, __func__),0)));
     if( c1 >= '0' && c1 <= '9' )
  c = c1 - '0';
     else if( c1 >= 'a' && c1 <= 'f' )
  c = c1 - 'a' + 10;
...
# 301 "mpicoder.c"
 for(i=a->nlimbs; i > 0 ; i-- ) {
     n += fprintf(fp, i!=a->nlimbs? "%0" "8" "lX":"%lX", (ulong)a->d[i-1]);
 ...
 *sign = a->sign;
    *nbytes = n = a->nlimbs * (4);
    if (!n)
      n++;
    p = buffer = force_secure || ((a) && (mpi_get_flags (a)&1)) ? xmalloc_secure(n)
        : xmalloc(n);
...
mpi_set_buffer( MPI a, const byte *buffer, unsigned nbytes, int sign )
{
    ...
    nlimbs = (nbytes + (4) - 1) / (4);
    do { if( (a)->alloced < (nlimbs) ) mpi_resize((a), (nlimbs)); } while(0);
    a->sign = sign;

    for(i=0, p = buffer+nbytes-1; p >= buffer+(4); ) {
...
# 468 "mpicoder.c"
 a->d[i++] = alimb;
    ...
    a->nlimbs = i;
    ((void)((i == nlimbs) || (__assert_fail("i == nlimbs", "mpicoder.c", 471, __func__),0)));
}
