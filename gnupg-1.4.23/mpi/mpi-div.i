...
{
    mpi_tdiv_qr(((void*)0), rem, num, den );
}
...
mpi_tdiv_qr( MPI quot, MPI rem, MPI num, MPI den)
{
 ...
 qp = rp + dsize;
    ...
    do { UWtype __xr = (dp[dsize - 1]); UWtype __a; if ((8*(4)) <= 32) { __a = __xr < ((UWtype) 1 << 2*((8*(4)) / 4)) ? (__xr < ((UWtype) 1 << ((8*(4)) / 4)) ? 0 : ((8*(4)) / 4)) : (__xr < ((UWtype) 1 << 3*((8*(4)) / 4)) ? 2*((8*(4)) / 4) : 3*((8*(4)) / 4)); } else { for (__a = (8*(4)) - 8; __a > 0; __a -= 8) if (((__xr >> __a) & 0xff) != 0) break; } (normalization_steps) = (8*(4)) - (__clz_tab[__xr >> __a] + __a); } while (0);
...
mpi_tdiv_q_2exp( MPI w, MPI u, unsigned count )
{
 ...
 do { if( (w)->alloced < (wsize) ) mpi_resize((w), (wsize)); } while(0);
 wp = w->d;
 up = u->d;
 ...
 if( count ) {
     ...
     do { mpi_size_t _i; for( _i = 0; _i < (wsize); _i++ ) (wp)[_i] = (up + limb_cnt)[_i]; } while (0);
 }
