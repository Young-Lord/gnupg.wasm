      ...
      MPI g, MPI **ret_factors )
{
 ...
 ;
    n--;
    if( !n || (mode==1 && n < 2) )
 g10_log_fatal(gettext ("can't gen prime with pbits=%u qbits=%u\n"),
                  pbits, qbits );
    if( mode == 1 ) {
 n--;
 ...
 qbits = pbits - n*fbits;
    ...
    if( g10c_debug_mode )
 g10_log_debug("gen prime: pbits=%u qbits=%u fbits=%u/%u n=%d\n",
      pbits, req_qbits, qbits, fbits, n );
    prime = mpi_alloc ( mpi_nlimb_hint_from_nbits (pbits) );
    q = gen_prime( qbits, 0, 0 );
    q_factor = mode==1? gen_prime( req_qbits, 0, 0 ) : ((void*)0);
 ...
 if( nprime > pbits ) {
  ...
  goto next_try;
    ...
    if( g10c_debug_mode ) {
 progress('\n');
 g10_log_mpidump( "prime    : ", prime );
 g10_log_mpidump( "factor  q: ", q );
 if( mode == 1 )
     g10_log_mpidump( "factor q0: ", q_factor );
 for(i=0; i < n; i++ )
     g10_log_mpidump( "factor pi: ", factors[i] );
 g10_log_debug("bit sizes: prime=%u, q=%u", mpi_get_nbits(prime), mpi_get_nbits(q) );
 if( mode == 1 )
 ...
 *ret_factors = xmalloc_clear( (n+2) * sizeof **ret_factors);
 ...
 if( mode == 1 )
     g10_log_bug0( "primegen.c" , 251, __FUNCTION__ );
 factors[n] = q;
...
gen_prime( unsigned int nbits, int secret, int randomlevel )
{
    ...
    if( 0 && g10c_debug_mode )
 g10_log_debug("generate a prime of %u bits ", nbits );

    if (nbits < 16)
      {
        g10_log_error (gettext ("can't generate a prime with less than %d bits\n"), 16);
        exit (2);
      }
 ...
 for(i=0; (x = small_prime_numbers[i]); i++ )
     mods[i] = mpi_fdiv_r_ui(((void*)0), prime, x);
 ...
 for(step=0; step < 20000; step += 2 ) {
  ...
  if( is_prime(ptest, 5, &count2 ) ) {
   ...
   progress('\n');
   g10_log_debug("overflow in prime generation\n");
   break;
      }
...
is_prime( MPI n, int steps, int *count )
{
 ...
 if( !i ) {
  ...
  mpi_clear_bit( x, nbits-2 );
     ...
     ((void)((mpi_cmp( x, nminus1 ) < 0 && mpi_cmp_ui( x, 1 ) > 0) || (__assert_fail("mpi_cmp( x, nminus1 ) < 0 && mpi_cmp_ui( x, 1 ) > 0", "primegen.c", 479, __func__),0)));
 }
 mpi_powm( y, x, q, n);
...
m_out_of_n( char *array, int m, int n )
{
 ...
 g10_log_bug0( "primegen.c" , 524, __FUNCTION__ );
    }
