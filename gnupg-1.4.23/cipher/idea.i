...
static void
invert_key( u16 *ek, u16 dk[(6*8 +4)] )
{
    int i;
    u16 t1, t2, t3;
    u16 temp[(6*8 +4)];
 ...
 *--p = t1;
    ...
    *--p = t1;
    ...
    do { volatile char *_vptr=(volatile char *)(temp); size_t _vlen=(sizeof(temp)); while(_vlen) { *_vptr=(0); _vptr++; _vlen--; } } while(0);
}
...
# 199 "idea.c"
    ...
    do {
 do {u16 _t16; u32 _t32; if( (_t16 = (*key++)) ) { if( (x1 = (x1)&0xffff) ) { _t32 = (u32)x1 * _t16; x1 = _t32 & 0xffff; _t16 = _t32 >> 16; x1 = ((x1)-_t16) + (x1<_t16?1:0); } else { x1 = 1 - _t16; } } else { x1 = 1 - x1; } } while(0);
 x2 += *key++;
 x3 += *key++;
 do {u16 _t16; u32 _t32; if( (_t16 = (*key++)) ) { if( (x4 = (x4)&0xffff) ) { _t32 = (u32)x4 * _t16; x4 = _t32 & 0xffff; _t16 = _t32 >> 16; x4 = ((x4)-_t16) + (x4<_t16?1:0); } else { x4 = 1 - _t16; } } else { x4 = 1 - x4; } } while(0);

 s3 = x3;
 x3 ^= x1;
 do {u16 _t16; u32 _t32; if( (_t16 = (*key++)) ) { if( (x3 = (x3)&0xffff) ) { _t32 = (u32)x3 * _t16; x3 = _t32 & 0xffff; _t16 = _t32 >> 16; x3 = ((x3)-_t16) + (x3<_t16?1:0); } else { x3 = 1 - _t16; } } else { x3 = 1 - x3; } } while(0);
 s2 = x2;
 x2 ^=x4;
 x2 += x3;
 do {u16 _t16; u32 _t32; if( (_t16 = (*key++)) ) { if( (x2 = (x2)&0xffff) ) { _t32 = (u32)x2 * _t16; x2 = _t32 & 0xffff; _t16 = _t32 >> 16; x2 = ((x2)-_t16) + (x2<_t16?1:0); } else { x2 = 1 - _t16; } } else { x2 = 1 - x2; } } while(0);
 x3 += x2;
...
do_setkey( IDEA_context *c, const byte *key, unsigned int keylen )
{
 ...
 return 50;
    ...
    ((void)((keylen == 16) || (__assert_fail("keylen == 16", "idea.c", 267, __func__),0)));
    c->have_dk = 0;
    expand_key( key, c->ek );
