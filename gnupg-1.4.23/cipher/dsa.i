...
gen_k( MPI q )
{
    ...
    if( g10c_debug_mode )
 g10_log_debug("choosing a random k ");
    for(;;) {
 if( g10c_debug_mode )
     progress('.');
...
test_keys( DSA_secret_key *sk, unsigned qbits )
{
 ...
 g10_log_fatal("DSA:: sign, verify failed\n");
   ...
   MPI **ret_factors )
{
    ...
    ((void)((nbits >= 512) || (__assert_fail("nbits >= 512", "dsa.c", 181, __func__),0)));
    ((void)((qbits >= 160) || (__assert_fail("qbits >= 160", "dsa.c", 182, __func__),0)));
    ((void)((qbits %8 == 0) || (__assert_fail("qbits %8 == 0", "dsa.c", 183, __func__),0)));

    p = generate_elg_prime( 1, nbits, qbits, ((void*)0), ret_factors );

    q = mpi_copy((*ret_factors)[0]);
    if( mpi_get_nbits(q) != qbits )
 g10_log_bug0( "dsa.c" , 189, __FUNCTION__ );
    ...
    h = mpi_alloc_set_ui( 1 );
    do {
 mpi_add_ui( h, h, 1 );

 mpi_powm( g, h, e, p );
    } while( !mpi_cmp_ui( g, 1 ) );
    ...
    if( g10c_debug_mode )
 g10_log_debug("choosing a random x ");
 ...
 mpi_clear_highbit( x, qbits+1 );
    ...
    if( g10c_debug_mode ) {
 progress('\n');
 g10_log_mpidump("dsa  p= ", p );
 g10_log_mpidump("dsa  q= ", q );
 g10_log_mpidump("dsa  g= ", g );
 g10_log_mpidump("dsa  y= ", y );
 g10_log_mpidump("dsa  x= ", x );
    }
...
verify(MPI r, MPI s, MPI hash, DSA_public_key *pkey )
{
 ...
 return 0;
    ...
    base[2] = ((void*)0); exponent[2] = ((void*)0);
        ...
        int *use )
{
    ...
    *nsig = 2;
    ...
    switch( algo ) {
      case 17: *use = 1; return "DSA";
      default: *use = 0; return ((void*)0);
    }
}
