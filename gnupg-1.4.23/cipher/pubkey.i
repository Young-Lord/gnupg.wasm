...
setup_pubkey_table(void)
{
 ...
 g10_log_bug0( "pubkey.c" , 115, __FUNCTION__ );
    i++;
    pubkey_table[i].algo = 17;
    ...
    if( !pubkey_table[i].name )
 g10_log_bug0( "pubkey.c" , 132, __FUNCTION__ );
    i++;


    pubkey_table[i].algo = 1;
    ...
    if( !pubkey_table[i].name )
 g10_log_bug0( "pubkey.c" , 151, __FUNCTION__ );
    i++;
    pubkey_table[i].algo = 2;
    pubkey_table[i].name = rsa_get_info( pubkey_table[i].algo,
      &pubkey_table[i].npkey,
      &pubkey_table[i].nskey,
      &pubkey_table[i].nenc,
    ...
    pubkey_table[i].get_nbits = rsa_get_nbits;
    if( !pubkey_table[i].name )
 g10_log_bug0( "pubkey.c" , 168, __FUNCTION__ );
    i++;
    pubkey_table[i].algo = 3;
    pubkey_table[i].name = rsa_get_info( pubkey_table[i].algo,
      &pubkey_table[i].npkey,
      &pubkey_table[i].nskey,
      &pubkey_table[i].nenc,
    ...
    pubkey_table[i].get_nbits = rsa_get_nbits;
    if( !pubkey_table[i].name )
 g10_log_bug0( "pubkey.c" , 185, __FUNCTION__ );
    i++;


    for( ; i < 10; i++ )
 pubkey_table[i].name = ((void*)0);
}
...
disable_pubkey_algo( int algo )
{
 ...
 if( !disabled_algos[i] || disabled_algos[i] == algo ) {
    ...
    g10_log_fatal("can't disable pubkey algo %d: table full\n", algo );
}
...
check_pubkey_algo2( int algo, unsigned use )
{
 ...
 for(i=0; pubkey_table[i].name; i++ )
     if( pubkey_table[i].algo == algo ) {
  if( (use & 1)
      && !(pubkey_table[i].use & 1) )
      return 41;
  if( (use & 2)
      && !(pubkey_table[i].use & 2) )
      return 41;
...
pubkey_encrypt( int algo, MPI *resarr, MPI data, MPI *pkey )
{
    ...
    if( g10c_debug_mode ) {
 g10_log_debug("pubkey_encrypt: algo=%d\n", algo );
 for(i=0; i < pubkey_get_npkey(algo); i++ )
     g10_log_mpidump("  pkey:", pkey[i] );
 g10_log_mpidump("  data:", data );
    }
 ...
 for(i=0; pubkey_table[i].name; i++ )
  ...
  goto ready;
    ...
    rc = 4;
  ready:
    if( !rc && g10c_debug_mode ) {
 for(i=0; i < pubkey_get_nenc(algo); i++ )
     g10_log_mpidump("  encr:", resarr[i] );
    }
    return rc;
}
# 504 "pubkey.c"
...
pubkey_decrypt( int algo, MPI *result, MPI *data, MPI *skey )
{
    ...
    *result = ((void*)0);
    if( g10c_debug_mode ) {
 g10_log_debug("pubkey_decrypt: algo=%d\n", algo );
 for(i=0; i < pubkey_get_nskey(algo); i++ )
     g10_log_mpidump("  skey:", skey[i] );
 for(i=0; i < pubkey_get_nenc(algo); i++ )
     g10_log_mpidump("  data:", data[i] );
    }
...
pubkey_sign( int algo, MPI *resarr, MPI data, MPI *skey )
{
    ...
    if( g10c_debug_mode ) {
 g10_log_debug("pubkey_sign: algo=%d\n", algo );
 for(i=0; i < pubkey_get_nskey(algo); i++ )
     g10_log_mpidump("  skey:", skey[i] );
 g10_log_mpidump("  data:", data );
    }
