...
static struct cipher_table_s cipher_table[14];
static int disabled_algos[14];
...
setup_cipher_table(void)
{
 ...
 g10_log_bug0( "cipher.c" , 109, __FUNCTION__ );
    i++;
    cipher_table[i].algo = 8;
    cipher_table[i].name = rijndael_get_info( cipher_table[i].algo,
      &cipher_table[i].keylen,
      &cipher_table[i].blocksize,
      &cipher_table[i].contextsize,
      ...
      &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 120, __FUNCTION__ );
    i++;
    cipher_table[i].algo = 9;
    cipher_table[i].name = rijndael_get_info( cipher_table[i].algo,
      &cipher_table[i].keylen,
      &cipher_table[i].blocksize,
      &cipher_table[i].contextsize,
      ...
      &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 131, __FUNCTION__ );
    i++;



    cipher_table[i].algo = 10;
    cipher_table[i].name = twofish_get_info( cipher_table[i].algo,
      ...
      &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 145, __FUNCTION__ );
    i++;



    cipher_table[i].algo = 4;
    cipher_table[i].name = blowfish_get_info( cipher_table[i].algo,
      ...
      &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 159, __FUNCTION__ );
    i++;



    cipher_table[i].algo = 3;
    cipher_table[i].name = cast5_get_info( cipher_table[i].algo,
      ...
      &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 173, __FUNCTION__ );
    i++;


    cipher_table[i].algo = 2;
    cipher_table[i].name = des_get_info( cipher_table[i].algo,
      ...
      &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 186, __FUNCTION__ );
    i++;


    cipher_table[i].algo = 11;
    cipher_table[i].name = camellia_get_info( cipher_table[i].algo,
           ...
           &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 199, __FUNCTION__ );
    i++;
    cipher_table[i].algo = 12;
    cipher_table[i].name = camellia_get_info( cipher_table[i].algo,
           &cipher_table[i].keylen,
           &cipher_table[i].blocksize,
           &cipher_table[i].contextsize,
           ...
           &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 210, __FUNCTION__ );
    i++;
    cipher_table[i].algo = 13;
    cipher_table[i].name = camellia_get_info( cipher_table[i].algo,
           &cipher_table[i].keylen,
           &cipher_table[i].blocksize,
           &cipher_table[i].contextsize,
           ...
           &cipher_table[i].decrypt );
    if( !cipher_table[i].name )
 g10_log_bug0( "cipher.c" , 221, __FUNCTION__ );
    i++;



    cipher_table[i].algo = 1;
...
disable_cipher_algo( int algo )
{
    ...
    for(i=0; i < (sizeof(disabled_algos)/sizeof((disabled_algos)[0])); i++ ) {
 if( !disabled_algos[i] || disabled_algos[i] == algo ) {
    ...
    g10_log_fatal("can't disable cipher algo %d: table full\n", algo );
}
...
cipher_get_keylen( int algo )
{
 ...
 for(i=0; cipher_table[i].name; i++ ) {
  ...
  if( !len )
      g10_log_bug("cipher %d w/o key length\n", algo );
  return len;
     }
 }
    } while( load_cipher_modules() );
    g10_log_bug("cipher %d not found\n", algo );
    return 0;
}
...
cipher_get_blocksize( int algo )
{
 ...
 for(i=0; cipher_table[i].name; i++ ) {
  ...
  if( !len )
      g10_log_bug("cipher %d w/o blocksize\n", algo );
  return len;
     }
 }
    } while( load_cipher_modules() );
...
cipher_open( int algo, int mode, int secure )
{
 ...
 for(i=0; cipher_table[i].name; i++ )
  ...
  break;
    ...
    if( !cipher_table[i].name ) {
 g10_log_fatal("cipher_open: algorithm %d not available\n", algo );
 return ((void*)0);
    }
...
cipher_setiv( CIPHER_HANDLE c, const byte *iv, unsigned ivlen )
{
 ...
 if( ivlen != c->blocksize )
     g10_log_info("WARNING: cipher_setiv: ivlen=%u blklen=%u\n",
...
cipher_encrypt( CIPHER_HANDLE c, byte *outbuf, byte *inbuf, unsigned nbytes )
{
    switch( c->mode ) {
      case 1:
 ((void)((!(nbytes%c->blocksize)) || (__assert_fail("!(nbytes%c->blocksize)", "cipher.c", 754, __func__),0)));
 do_ecb_encrypt(c, outbuf, inbuf, nbytes/c->blocksize );
 break;
      case 6:
 ((void)((!(nbytes%c->blocksize)) || (__assert_fail("!(nbytes%c->blocksize)", "cipher.c", 758, __func__),0)));
 ...
 break;
      ...
      default: g10_log_fatal("cipher_encrypt: invalid mode %d\n", c->mode );
    }
}
...
cipher_decrypt( CIPHER_HANDLE c, byte *outbuf, byte *inbuf, unsigned nbytes )
{
    switch( c->mode ) {
      case 1:
 ((void)((!(nbytes%c->blocksize)) || (__assert_fail("!(nbytes%c->blocksize)", "cipher.c", 786, __func__),0)));
 do_ecb_decrypt(c, outbuf, inbuf, nbytes/c->blocksize );
 break;
      case 6:
 ((void)((!(nbytes%c->blocksize)) || (__assert_fail("!(nbytes%c->blocksize)", "cipher.c", 790, __func__),0)));
 ...
 break;
      ...
      default: g10_log_fatal("cipher_decrypt: invalid mode %d\n", c->mode );
    }
}
