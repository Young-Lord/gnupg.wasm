       ...
       size_t length, int level )
{
 ...
 do { int __i; unsigned long *__b=(&rfds)->fds_bits; for(__i=sizeof (fd_set)/sizeof (long); __i; __i--) *__b++=0; } while(0);
 tv.tv_sec = 3;
 tv.tv_usec = 0;
        if (fd < 1024)
          {
            ((&rfds)->fds_bits[(fd)/(8*sizeof(long))] |= (1UL<<((fd)%(8*sizeof(long)))));
            if( !(rc=select(fd+1, &rfds, ((void*)0), ((void*)0), &tv)) ) {
              if( !warn )
  tty_printf(
gettext ("\n""Not enough random bytes available.  Please do some other work to give\n""the OS a chance to collect more entropy! (Need %d more bytes)\n"), (int)length );
 ...
 length -= n;
    ...
    do { volatile char *_vptr=(volatile char *)(buffer); size_t _vlen=(sizeof(buffer)); while(_vlen) { *_vptr=(0); _vptr++; _vlen--; } } while(0);
