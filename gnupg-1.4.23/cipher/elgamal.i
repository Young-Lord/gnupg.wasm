...
test_keys( ELG_secret_key *sk, unsigned int nbits )
{
 ...
 g10_log_fatal("Elgamal operation: encrypt, decrypt failed\n");
...
gen_k( MPI p, int small_k )
{
    ...
    if (small_k)
      {
        ...
        if( nbits >= orig_nbits )
          g10_log_bug0( "elgamal.c" , 178, __FUNCTION__ );
      }
    else
      nbits = orig_nbits;
    ...
    nbytes = (nbits+7)/8;
    if( g10c_debug_mode )
 g10_log_debug("choosing a random k of %u bits", nbits);
    mpi_sub_ui( p_1, p, 1);
    for(;;) {
 if( !rndbuf || nbits < 32 ) {
     xfree(rndbuf);
...
# 274 "elgamal.c"
    xbits = qbits * 3 / 2;
    if( xbits >= nbits )
 g10_log_bug0( "elgamal.c" , 276, __FUNCTION__ );
    x = mpi_alloc_secure ( mpi_nlimb_hint_from_nbits (xbits) );
    if( g10c_debug_mode )
 g10_log_debug("choosing a random x of size %u", xbits );
    rndbuf = ((void*)0);
    do {
 if( g10c_debug_mode )
     progress('.');
 ...
 mpi_clear_highbit( x, xbits+1 );
    ...
    if( g10c_debug_mode ) {
 progress('\n');
 g10_log_mpidump("elg  p= ", p );
 g10_log_mpidump("elg  g= ", g );
 g10_log_mpidump("elg  y= ", y );
 g10_log_mpidump("elg  x= ", x );
    }
