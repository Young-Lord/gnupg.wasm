...
# 10 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/sys/time.h" 2 3
...
# 1 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/sys/times.h" 1 3
...
# 1 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/bits/alltypes.h" 1 3
# 10 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/sys/times.h" 2 3
...
# 17 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/sys/resource.h" 3
# 1 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/bits/alltypes.h" 1 3
...
initialize(void)
{
    ...
    rndpool = secure_alloc ? xmalloc_secure_clear((30*20)+64)
      : xmalloc_clear((30*20)+64);
    keypool = secure_alloc ? xmalloc_secure_clear((30*20)+64)
      : xmalloc_clear((30*20)+64);
    is_initialized = 1;
}
...
random_dump_stats()
{
 ...
 (30*20), rndstats.mixrnd, rndstats.slowpolls, rndstats.fastpolls,
...
randomize_mpi (MPI mpi, size_t nbits, int level)
{
  ...
  buffer = get_random_bits (nbits, level, ((mpi) && (mpi_get_flags (mpi)&1)));
...
get_random_bits( size_t nbits, int level, int secure )
{
 ...
 level = 1;
    do {if( level > 2 ) level = 2; else if( level < 0 ) level = 0; } while(0);
    if( level == 1 ) {
 ...
 rndstats.ngetbytes2++;
    ...
    for( p = buf; nbytes > 0; ) {
 size_t n = nbytes > (30*20)? (30*20) : nbytes;
 read_pool( p, n, level );
 nbytes -= n;
 p += n;
    }
...
mix_pool(byte *pool)
{
    ...
    pend = pool + (30*20);
    memcpy(hashbuf, pend - 20, 20 );
    memcpy(hashbuf+20, pool, 64 -20);
...
{
    if( seed_file_name )
 g10_log_bug0( "random.c" , 396, __FUNCTION__ );
...
lock_seed_file (int fd, const char *fname, int for_write)
{
  ...
  lck.l_type = for_write? 1 : 0;
  lck.l_whence = (0);

  while (fcntl (fd, 13, &lck) == -1)
    {
      if ((*__errno_location()) != (6) && (*__errno_location()) != (2))
        {
          g10_log_info (gettext ("can't lock `%s': %s\n"), fname, strerror ((*__errno_location())));
          return -1;
        }

      if (backoff > 2)
        g10_log_info( gettext ("waiting for lock on `%s'...\n"), fname);
...
read_seed_file(void)
{
 ...
 return 0;
    ...
    fd = open( seed_file_name, 00 );

    if( fd == -1 && (*__errno_location()) == (44)) {
 allow_seed_file_update = 1;
 return 0;
    }

    if( fd == -1 ) {
 g10_log_info(gettext ("can't open `%s': %s\n"), seed_file_name, strerror((*__errno_location())) );
 return 0;
    }
    ...
    if( fstat( fd, &sb ) ) {
 g10_log_info(gettext ("can't stat `%s': %s\n"), seed_file_name, strerror((*__errno_location())) );
 close(fd);
 return 0;
    }
    if( !(((sb.st_mode) & 0170000) == 0100000) ) {
 g10_log_info(gettext ("`%s' is not a regular file - ignored\n"), seed_file_name );
 close(fd);
 return 0;
    }
    if( !sb.st_size ) {
 g10_log_info(gettext ("note: random_seed file is empty\n") );
 close(fd);
 allow_seed_file_update = 1;
 return 0;
    }
    if( sb.st_size != (30*20) ) {
 g10_log_info(gettext ("WARNING: invalid size of random_seed file - not used\n") );
 close(fd);
 return 0;
    }

    do {
 n = read( fd, buffer, (30*20) );
    ...
    else if( n == -1 && (*__errno_location()) == (44)) {


 g10_log_info(gettext ("can't read `%s': %s\n"), seed_file_name, strerror((*__errno_location())));
        allow_seed_file_update = 1;
 close(fd);
 return 0;
    }
 ...
 g10_log_fatal(gettext ("can't read `%s': %s\n"), seed_file_name,strerror((*__errno_location())) );
 close(fd);
 return 0;
    }
    else if ( n != (30*20) ) {
 ...
 return 0;
    ...
    add_randomness( buffer, (30*20), 0 );

    { pid_t x = getpid();
 add_randomness( &x, sizeof(x), 0 );
    }
    { time_t x = time(((void*)0));
 add_randomness( &x, sizeof(x), 0 );
    }
    { clock_t x = clock();
 add_randomness( &x, sizeof(x), 0 );
...
update_random_seed_file()
{
 ...
 g10_log_info(gettext ("note: random_seed file not updated\n"));
 return;
    }
 ...
 *dp = *sp + 0xa5a5a5a5;
    ...
    fd = open( seed_file_name, 01|0100, 0400|0200 );
# 621 "random.c"
    if( fd == -1 ) {
 g10_log_info(gettext ("can't create `%s': %s\n"), seed_file_name, strerror((*__errno_location())));
 return;
    }
    ...
    if (ftruncate (fd, 0))
      {
 g10_log_info(gettext ("can't write `%s': %s\n"), seed_file_name, strerror((*__errno_location())));
        close (fd);
        return;
      }
    ...
    do {
 i = write( fd, keypool, (30*20) );
    } while( i == -1 && (*__errno_location()) == (27) );
    if( i != (30*20) ) {
 g10_log_info(gettext ("can't write `%s': %s\n"), seed_file_name, strerror((*__errno_location())) );
    }
    if( close(fd) )
 g10_log_info(gettext ("can't close `%s': %s\n"), seed_file_name, strerror((*__errno_location())) );
}
...
read_pool( byte *buffer, size_t length, int level )
{
    ...
    if( length > (30*20) ) {
 g10_log_bug("too many random bits requested\n");
    }
 ...
 if( needed < (30*20)/2 )
     needed = (30*20)/2;
 else if( needed > (30*20) )
     g10_log_bug0( "random.c" , 676, __FUNCTION__ );
 ...
 needed = length - pool_balance;
 if( needed > (30*20) )
     g10_log_bug0( "random.c" , 690, __FUNCTION__ );
 read_random_source( 3, needed, 2 );
 pool_balance += needed;
    }
 ...
 do { volatile char *_vptr=(volatile char *)(keypool); size_t _vlen=((30*20)); while(_vlen) { *_vptr=(0); _vptr++; _vlen--; } } while(0);
    }
}
# 752 "random.c"
static void
...
{
    rndstats.slowpolls++;
    read_random_source( 2, (30*20)/5, 1 );
}
...
fast_random_poll()
{
    static int (*fnc)( void (*)(const void*, size_t, int), int) = ((void*)0);
...
# 810 "random.c"
 ...
 if( gettimeofday( &tv, ((void*)0) ) )
     g10_log_bug0( "random.c" , 812, __FUNCTION__ );
...
read_random_source( int requester, size_t length, int level )
{
 ...
 g10_log_fatal("No way to gather entropy for the RNG\n");
}
       ...
       size_t length, int level )
{
    ...
    if( !initialized ) {
 g10_log_info(gettext ("WARNING: using insecure random number generator!!\n"));
 tty_printf(gettext ("The random number generator is only a kludge to let\n" "it run - it is in no way a strong RNG!\n\n" "DON'T USE ANY DATA GENERATED BY THIS PROGRAM!!\n\n"));
 ...
 srand(make_timestamp()*getpid());
    ...
    while( n-- )
 *p++ = ((unsigned)(1 + (int) (256.0*rand()/((0x7fffffff)+1.0)))-1);
