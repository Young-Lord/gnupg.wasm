...
load_digest_module (void)
{
  ...
  if (!new_list_item (10, sha512_get_info))
    g10_log_bug0( "md.c" , 100, __FUNCTION__ );
  if (!new_list_item (9, sha384_get_info))
    g10_log_bug0( "md.c" , 102, __FUNCTION__ );


  if (!new_list_item (8, sha256_get_info))
    g10_log_bug0( "md.c" , 106, __FUNCTION__ );
  if (!new_list_item (11, sha224_get_info))
    g10_log_bug0( "md.c" , 108, __FUNCTION__ );

  if (!new_list_item (1, md5_get_info))
    g10_log_bug0( "md.c" , 111, __FUNCTION__ );
  if (!new_list_item (3, rmd160_get_info))
    g10_log_bug0( "md.c" , 113, __FUNCTION__ );
  if (!new_list_item (2, sha1_get_info))
    g10_log_bug0( "md.c" , 115, __FUNCTION__ );
...
md_enable( MD_HANDLE h, int algo )
{
 ...
 for(r = digest_list; r; r = r->next )
  ...
  break;
    ...
    if( !r ) {
 g10_log_error("md_enable: algorithm %d not available\n", algo );
 return;
    }
...
md_write( MD_HANDLE a, const byte *inbuf, size_t inlen)
{
 ...
 if( a->bufcount && fwrite(a->buffer, a->bufcount, 1, a->debug ) != 1 )
     g10_log_bug0( "md.c" , 314, __FUNCTION__ );
 if( inlen && fwrite(inbuf, inlen, 1, a->debug ) != 1 )
     g10_log_bug0( "md.c" , 316, __FUNCTION__ );
    }
    for(r=a->list; r; r = r->next ) {
...
md_read( MD_HANDLE a, int algo )
{
 ...
 if( (r=a->list) ) {
     if( r->next )
  g10_log_debug("more than algorithm in md_read(0)\n");
 ...
 for(r=a->list; r; r = r->next )
  ...
  return (*r->read)( &r->context.c );
    ...
    g10_log_bug0( "md.c" , 366, __FUNCTION__ );
    return ((void*)0);
}
# 380 "md.c"
int
...
{
    struct md_digest_list_s *r = ((void*)0);
    char *context;
    char *digest;
 ...
 md_write( a, ((void*)0), 0 );
    ...
    if( !algo ) {
 if( (r=a->list) && r->next )
     g10_log_debug("more than algorithm in md_digest(0)\n");
    }
    else {
 for(r=a->list; r; r = r->next )
     if( r->algo == algo )
  break;
    ...
    if( !r )
 g10_log_bug0( "md.c" , 400, __FUNCTION__ );
...
md_get_algo( MD_HANDLE a )
{
    ...
    if( (r=a->list) ) {
 if( r->next )
     g10_log_error("WARNING: more than algorithm in md_get_algo()\n");
 return r->algo;
    }
    return 0;
}
...
md_digest_length( int algo )
{
 ...
 for(r = digest_list; r; r = r->next ) {
  ...
  return r->mdlen;
    ...
    g10_log_error("WARNING: no length for md algo %d\n", algo);
    return 0;
}
...
md_asn_oid( int algo, size_t *asnlen, size_t *mdlen )
{
 ...
 for(r = digest_list; r; r = r->next ) {
  ...
  return r->asnoid;
    ...
    g10_log_bug("no asn for md algo %d\n", algo);
    return ((void*)0);
}
...
md_start_debug( MD_HANDLE md, const char *suffix )
{
    ...
    if( md->debug ) {
 g10_log_debug("Oops: md debug already started\n");
 return;
    }
    idx++;
    ...
    if( !md->debug )
 g10_log_debug("md debug: can't open %s\n", buf );
}
