...
# 9 "/home/yang/wasm/emsdk/upstream/emscripten/cache/sysroot/include/iconv.h" 2 3
...
static struct hash_node * volatile struniq_hash_table[257]
                             ;
...
struniq (const char *string)
{
  ...
  size_t slot = hashcode % 257;
  size_t size;
  struct hash_node *new_node;
  struct hash_node *p;
  for (p = struniq_hash_table[slot]; p != ((void*)0); p = p->next)
    if (strcmp (p->contents, string) == 0)
      return p->contents;
  size = strlen (string) + 1;
  new_node =
    (struct hash_node *)
    malloc (__builtin_offsetof(struct hash_node, contents[0]) + size);
  if (new_node == ((void*)0))
...
# 2679 "localename.c"
const char *
_nl_locale_name_thread_unsafe (int category, const char *categoryname)
{

  {
    locale_t thread_locale = uselocale (((void*)0));
    if (thread_locale != ((locale_t)-1))
      {
# 2727 "localename.c"
      }
  }
...
const char *
_nl_locale_name_thread (int category, const char *categoryname)
{

  const char *name = _nl_locale_name_thread_unsafe (category, categoryname);
  if (name != ((void*)0))
    return struniq (name);
# 2764 "localename.c"
  return ((void*)0);
...
{
# 2802 "localename.c"
  return _nl_locale_name_environ (category, categoryname);

}

const char *
_nl_locale_name_environ (int category, const char *categoryname)
{
  const char *retval;
...
_nl_locale_name (int category, const char *categoryname)
{
  ...
  retval = _nl_locale_name_thread (category, categoryname);
  if (retval != ((void*)0))
    return retval;
